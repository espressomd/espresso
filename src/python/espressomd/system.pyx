...
cdef class System(object):
    """ The central class for the simulation system.
    ...
    def smaller_time_step(self):
        """
        Setting this property to a positive integer value turns on the
        multi-timestepping algorithm. The ratio
    ...
    def smaller_time_step(self, double _smaller_time_step):
        IF MULTI_TIMESTEP:
    ...
    @time_step.setter
    def time_step(self, double _time_step):
        IF LB:
            global lbpar
        IF LB_GPU:
            global lbpar_gpu
    ...
    def random_number_generator_state(self, rng_state):
        _state_size_plus_one = self._get_PRNG_state_size() + 1
    ...
    IF LEES_EDWARDS == 1:
        @property
        def lees_edwards_offset(self):
            """Defines the lees edwards offset.

            """
            return lees_edwards_offset

        @lees_edwards_offset.setter
        def lees_edwards_offset(self, double _lees_edwards_offset):
