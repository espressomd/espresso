...
IF REACTION_ENSEMBLE:
    ...
    cdef class reaction_ensemble(object):
        ...
        def _get_params_from_es_core(self):
            params = {
                "temperature": RE.m_current_reaction_system.temperature_reaction_ensemble,
        ...
        def _set_params_in_es_core(self):
            RE.m_current_reaction_system.temperature_reaction_ensemble = self._params[
                "temperature"]
            # setting a volume is a side effect, sets the default volume of the
            # reaction ensemble as the volume of the cuboid simulation box.
            # this volume can be altered by the command "reaction ensemble
            # volume <volume>" if one wants to simulate e.g. in a system with
            # constraint (e.g. cuboid box with cylinder constraint, so that the
            # particles are only contained in the volume of the cylinder)
            if(RE.m_current_reaction_system.volume < 0):
                RE.set_cuboid_reaction_ensemble_volume()
            RE.m_current_reaction_system.standard_pressure_in_simulation_units = self._params[
                "standard_pressure"]
        ...
        def acceptance_rate_configurational_moves(self):
            ...
            """
            return (1.0 * RE.m_accepted_configurational_MC_moves) / \
                RE.m_tried_configurational_MC_moves
        ...
        def _set_params_in_es_core_add(self):
            ...
            RE.add_reaction(
                self._params["equilibrium_constant"],
                reactant_types,
                reactant_coefficients,
                product_types,
                product_coefficients)
            RE.add_reaction(
                1.0 / self._params["equilibrium_constant"],
                product_types,
                product_coefficients,
                reactant_types,
                reactant_coefficients)
        ...
        def set_default_charges(self, *args, **kwargs):
            ...
            for key in self._params["dictionary"]:
                if(RE.find_index_of_type(int(key)) >= 0):
                    RE.m_current_reaction_system.charges_of_types[RE.find_index_of_type(
                        int(key))] = self._params["dictionary"][key]
        ...
        def global_mc_move_for_one_particle_of_type(type_mc):
            ...
            """
            RE.do_global_mc_move_for_particles_of_type(
        ...
        def get_status(self):
            ...
            if(RE.check_reaction_ensemble() == es_error):
                ...
                reactions = []
                for single_reaction_i in range(
                        RE.m_current_reaction_system.nr_single_reactions):
                    reactant_types = []
                    for i in range(
                            RE.m_current_reaction_system.reactions[single_reaction_i].len_reactant_types):
                        reactant_types.append(
                            RE.m_current_reaction_system.reactions[single_reaction_i].reactant_types[i])
                    reactant_coefficients = []
                    for i in range(
                            RE.m_current_reaction_system.reactions[single_reaction_i].len_reactant_types):
                        reactant_coefficients.append(
                            RE.m_current_reaction_system.reactions[single_reaction_i].reactant_coefficients[i])

                    product_types = []
                    for i in range(
                            RE.m_current_reaction_system.reactions[single_reaction_i].len_product_types):
                        product_types.append(
                            RE.m_current_reaction_system.reactions[single_reaction_i].product_types[i])
                    product_coefficients = []
                    for i in range(
                            RE.m_current_reaction_system.reactions[single_reaction_i].len_product_types):
                        product_coefficients.append(
                            RE.m_current_reaction_system.reactions[single_reaction_i].product_coefficients[i])
                    reaction = {
                        ...
                        "reactant_types": reactant_types,
                        "equilibrium_constant": RE.m_current_reaction_system.reactions[single_reaction_i].equilibrium_constant}
        ...
        def add_collective_variable_degree_of_association(
                self, *args, **kwargs):
            """
            ...
            RE.add_new_CV_degree_of_association(
        ...
        def set_wang_landau_parameters(self, *args, **kwargs):
            ...
            RE.m_current_wang_landau_system.final_wang_landau_parameter = self._params[
                "final_wang_landau_parameter"]
            RE.m_current_wang_landau_system.wang_landau_steps = self._params["wang_landau_steps"]
            RE.m_current_wang_landau_system.output_filename = strdup(
                self._params["filename"])
            RE.m_current_wang_landau_system.do_not_sample_reaction_partition_function = self._params[
        ...
        def reaction_wang_landau(self):
            ...
            if(status_wang_landau < 0):
                raise wang_landau_has_converged(
        ...
        def global_mc_move_for_one_particle_of_type_wang_landau(self, type_mc):
            ...
            RE.do_global_mc_move_for_particles_of_type(
                type_mc,
                RE.m_current_wang_landau_system.polymer_start_id,
                RE.m_current_wang_landau_system.polymer_end_id,
                1,
                True)
