...
cdef class PScriptInterface(object):
    ...
    def call_method(self, method, **kwargs):
        ...
        for name in kwargs:
            parameters[to_char_pointer(
                name)] = self.python_object_to_variant(kwargs[name])

        return self.variant_to_python_object(
            self.sip.get().call_method(
                to_char_pointer(method), parameters))
        ...
        for pname in kwargs:
            ...
            if < int > type in [ < int > INT_VECTOR, < int > DOUBLE_VECTOR]:
                ...
                if n_elements != 0 and not (len(kwargs[pname]) == n_elements):
                    raise ValueError(
                        "Value of %s expected to be %i elements" %
                        (name, n_elements))
            ...
            if v.which() == <int > type:
                ...
                raise ValueError("Wrong type for parameter '%s': Expected %s, but got %s" % (
                    pname, get_type_label(type), get_type_label(v)))
        ...
        if < int > type == <int > OBJECTID:
            ...
            try:
                ...
                if ptr != shared_ptr[ScriptInterfaceBase]():
                    ...
                    # Fallback class, if nothing omre specific is registered
