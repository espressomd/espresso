    ...
    def checkBondOrThrowException(self, bond):
        ...
        * If the number of bond partners fits the bond type
        ...
        if not hasattr(bond, "__getitem__"):
            raise ValueError(
                "Elements of the bond list have to be tuples of the form (bondType,bondPartner)")
        if len(bond) < 2:
            raise ValueError(
                "Elements of the bond list have to be tuples of the form (bondType,bondPartner)")
