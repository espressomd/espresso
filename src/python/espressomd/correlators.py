...
class Correlator(ScriptInterfaceHelper):
    """Calculates correlations based on results from observables.
       ...
       obs1 and obs2: instances of :class:`espressomd.observables.Observable`
         are the observables A and B that are to be correlated. 
         If `obs2` is omitted, autocorrelation of `obs1` is calculated by 
         default.
       corr_operation: string
         The operation that is performed on :math:`A(t)` and :math:`B(t+\tau)` to obtain 
         :math:`C(\tau)`. 
         The following operations are currently available:

            * `scalar_product`: Scalar product of :math:`A` and :math:`B`, i.e., :math:`C=\sum\limits_{i} A_i B_i`

            * `componentwise_product`: Componentwise product of :math:`A` and :math:`B`, i.e., :math:`C_i = A_i B_i`

            * `square_distance_componentwise`: Each component of the correlation vector is the square of the difference between the 
              corresponding components of the observables, i.E., :math:`C_i = (A_i-B_i)^2`. 
              Example: when :math:`A` is `ParticlePositions`, it produces the mean square displacement
              (for each component separately).

            * `tensor_product`: Tensor product of :math:`A` and :math:`B`, i.e., :math:`C_{i \cdot l_B + j} = A_i B_j` with :math:`l_B` the length of :math:`B`.

            * `complex_conjugate_product`: todo: Complex conjugate product must be defined.

            * `fcs_acf`: todo: How to set the extra args in py?
              Fluorescence Correlation Spectroscopy (FCS) autocorrelation function, i.e.,
              :math:`G_i(\tau) = \frac{1}{N} \Bigl< \exp \Bigl( - \frac{\Delta x_i^2(\tau) }{w_x^2} - \frac{\Delta y_i^2(\tau)}{w_y^2} - \frac{\Delta z_i^2(\tau)}{w_z^2} \Bigr) \Bigr>\,,`
              where :math:`\Delta x_i^2(\tau) = \bigl(x_i(0) - x_i(\tau) \bigr)^2` is the square displacement 
              of particle :math:`i` in the :math:`x` direction, and :math:`w_x` is the beam waist of the intensity profile 
              of the exciting laser beam,
              :math:`W(x,y,z) = I_0 \exp \Bigl( - \frac{2x^2}{w_x^2} - \frac{2y^2}{w_y^2} - \frac{2z^2}{w_z^2} \Bigr)\,.`
              The equations are a generalization of the formula presented by 
              Hoefling et. al. :cite:`hofling11a`. For more information, see references therein.
              ...
              `ParticlePositions`, the physical meaning of the result is unclear.

       dt: float
         The time interval of sampling data points. When autoupdate is used, `dt` has
         to be a multiple of timestep. It is also used to produce time axis in real units.
         Warning: if `dt` is close to the timestep, autoupdate is strongly recommended.
         Otherwise cpu time is wasted on passing the control between the script and kernel.

       tau_max: float
         This is the maximum value of :math:`\tau` for which the correlation should be computed.
         Warning: Unless you are using the multiple tau correlator, choosing `tau\_max`
         of more than 100`dt` will result in a huge computational overhead.
         In a multiple tau correlator with reasonable parameters, 
         `tau\_max` can span the entire simulation without
       ...
       tau_lin: int
          ...
          is set equal to `tau\_max` which results in the trivial linear
         correlator. By setting `tau\_lin` < `tau\_max` the multiple
       ...
       compress1 and compress2: string
         ...
         same function is used for both.  Default is `discard` which
