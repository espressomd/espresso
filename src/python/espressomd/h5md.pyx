...
#WRITE CLASS 
  class write_to_h5(object):
    ...
    def dipm(self,timestep=-1,groupname="particles/atoms/dipole_magnitude/",datasetname="value"):
      if timestep == -1:
        for i in range(0,self.n_part):
          self.h5md.WriteValue(-1,i,self.h5md.system.part[i].dipm,groupname+"/"+datasetname,(1,1),(None,1),'f8','value_time_independent',DIPOLES)        
      else:
        self.n_part=self.h5md.system.n_part
        for i in range(0,self.n_part):
          self.h5md.WriteValue(timestep,i,self.h5md.system.part[i].dipm,groupname+"/"+datasetname,(1,1,1),(None,None,1),'f8','value_time_dependent',DIPOLES)     
    #external force
...
#READ CLASS   
  class read_from_h5(object):
    ...
    def XXX(self,timestep,groupname="particles/atoms/XXX/",datasetname="value"):
      ...
      if len(self.h5md.value_dataset.shape) == 2:
        for i in range(self.h5md.value_dataset.shape[0]):
          self.h5md.system.part[i].XXX = self.h5md.value_dataset[i][0]  
      else:
        for i in range(self.h5md.value_dataset.shape[1]):
          self.h5md.system.part[i].XXX = self.h5md.value_dataset[timestep,i][0]
