...
BOOST_AUTO_TEST_CASE(matrix) {
  ...
  Utils::Mat<int, 3, 3> diag_mat = boost::qvm::diag_mat(diagonal);
  BOOST_CHECK((diag_mat(0, 0) = 1));
  BOOST_CHECK((*(diag_mat.data()) == 1));
}

BOOST_AUTO_TEST_CASE(identity_matrix) {
  auto const id_mat = Utils::Mat<int, 2, 2>::identity();
...
BOOST_AUTO_TEST_CASE(type_deduction) {
  static_assert(
      std::is_same<boost::qvm::deduce_vec2<Utils::Mat<int, 2, 2>,
...
BOOST_AUTO_TEST_CASE(matrix_matrix) {
  const Utils::Mat<int, 2, 2> a{{1, 2}, {3, 4}};
  const Utils::Mat<int, 2, 2> b{{5, 6}, {7, 8}};
  auto const res = a * b;
...
BOOST_AUTO_TEST_CASE(matrix_vector) {
  ...
  auto const mat2 = Utils::Mat<int, 2, 2>{{1, 2}, {3, 4}};
...
BOOST_AUTO_TEST_CASE(transposed) {
  auto const mat = Utils::Mat<int, 2, 2>{{1, 2}, {3, 4}};
  auto const mat_T = mat.transposed();
  auto const expected = Utils::Mat<int, 2, 2>{{1, 3}, {2, 4}};
  BOOST_CHECK((mat_T == expected));
}

BOOST_AUTO_TEST_CASE(inversed) {
  auto constexpr eps = std::numeric_limits<double>::epsilon();
  auto const mat = Utils::Mat<double, 2, 2>{{1, 2}, {3, 4}};
  auto const mat_inv = mat.inversed();
  auto const expected = Utils::Mat<double, 2, 2>{{-2, 1.0}, {1.5, -0.5}};
...
BOOST_AUTO_TEST_CASE(shape) {
  auto const mat = Utils::Mat<int, 2, 3>{{1, 2, 3}, {4, 5, 6}};
