...
BOOST_AUTO_TEST_CASE(energy) {
  ...
  // check the minimum
  BOOST_CHECK(lennard_jones.U(std::pow(2.0, 1./6) * lennard_jones.sigma) == -lennard_jones.epsilon);
  // check the root
...
BOOST_AUTO_TEST_CASE(force) {
  ...
  auto const force = lennard_jones.F(minimum, Utils::Vector3d{minimum, 0.0, 0.0});
  ...
  auto const force1 = lennard_jones.F(minimum - epsilon, Utils::Vector3d{minimum - epsilon, 0.0, 0.0});
  auto const force2 = lennard_jones.F(minimum + epsilon, Utils::Vector3d{minimum + epsilon, 0.0, 0.0});
