...
int fft_init(double **data, int *ca_mesh_dim, int *ca_mesh_margin, int *ks_pnum)
{
  ...
  for(i=1; i<4;i++) {
    fft_plan[i].g_size=fft_find_comm_groups(n_grid[i-1], n_grid[i], n_id[i-1], n_id[i], 
    ...
    fft_plan[i].new_size = fft_calc_local_mesh(my_pos[i], n_grid[i], p3m.params.mesh,
					   p3m.params.mesh_off, fft_plan[i].new_mesh, 
					   fft_plan[i].start);  
	...
	= fft_calc_send_block(my_pos[i-1], n_grid[i-1], &(n_pos[i][3*node]), n_grid[i],
			  p3m.params.mesh, p3m.params.mesh_off, &(fft_plan[i].send_block[6*j]));
	...
	= fft_calc_send_block(my_pos[i], n_grid[i], &(n_pos[i-1][3*node]), n_grid[i-1],
			  p3m.params.mesh,p3m.params.mesh_off,&(fft_plan[i].recv_block[6*j]));
    ...
    for(j=0;j<n_nodes;j++) {
      ...
      if(j==this_node) FFT_TRACE(fft_print_fft_plan(fft_plan[i]));
    }
  }
...
//printf("fft_plan[%d].n_ffts=%d\n",i,fft_plan[i].n_ffts);
  ...
  for(i=1;i<4;i++) {
    ...
    fft_back[i].pack_function = fft_pack_block_permute1;
    FFT_TRACE(fprintf(stderr,"%d: back plan[%d] permute 1 \n",this_node,i));
  }
  if(fft_plan[1].row_dir==2) {
    fft_back[1].pack_function = fft_pack_block;
    FFT_TRACE(fprintf(stderr,"%d: back plan[%d] permute 0 \n",this_node,1));
  }
  else if(fft_plan[1].row_dir==1) {
    fft_back[1].pack_function = fft_pack_block_permute2;
...
void fft_perform_forw(double *data)
{
  ...
  */
  ...
  //fft_print_global_fft_mesh(fft_plan[3],data,1,0);
...
void forw_grid_comm(fft_forw_plan plan, double *in, double *out)
{
  ...
  for(i=0;i<plan.g_size;i++) {   
    ...
    fft_unpack_block(recv_buf, out, &(plan.recv_block[6*i]), 
...
void back_grid_comm(fft_forw_plan plan_f,  fft_back_plan plan_b, double *in, double *out)
{
  ...
  for(i=0;i<plan_f.g_size;i++) {
    ...
    fft_unpack_block(recv_buf, out, &(plan_f.send_block[6*i]), 
