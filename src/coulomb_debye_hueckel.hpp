...
*/
#ifndef COULOMB_DEBYE_HUECKEL_H
#define COULOMB_DEBYE_HUECKEL_H
...
typedef struct {
  ...
  double kappa;
} Coulomb_debye_hueckel_params;

/** Structure containing the Debye-Hueckel parameters. */
extern Coulomb_debye_hueckel_params dh_params;

/** \name Functions */
/************************************************************/
/*@{*/

int cdh_set_params(double kappa, double r_cut);
...
*/
inline void add_cdh_coulomb_pair_force(Particle *p1, Particle *p2, double d[3], double dist, double force[3])
{
  int j;
...
inline double cdh_coulomb_pair_energy(Particle *p1, Particle *p2, double dist)
{
  if(dist < dh_params.r_cut) {
    if(dh_params.kappa > 0.0)
