...
void calculate_pore_dist(Particle *p1, double ppos[3], Particle *c_p, Constraint_pore *c, double *dist, double *vec)
{
  ...
  if ( ( (r >= c1_r) && (z <= c1_z) && full_pore ) || ( ( z <= 0 ) && (r>=max(c1_r, c2_r)) && full_pore ) || ( (r >= c1_r) && (r <= c1_or) && (z <= c1_z) && !full_pore ) ) { 
  ...
  if ( ( (r >= c2_r) && (z <= c2_z) && full_pore ) || ( ( z >= 0 ) && (r>=max(c1_r, c2_r)) && full_pore ) || ( (r >= c2_r) && (r<c2_or) && (z >= c2_z) && !full_pore ) ) { 
    dist_vector_z=-z + c->length;
    dist_vector_r=0;
    *dist = +z - c->length;
    for (i=0; i<3; i++) vec[i]=-dist_vector_r*e_r[i] - dist_vector_z*e_z[i];
