...
//MDINLINE void calc_area_global_complicated(double *area, int molType){  // More complicated approach
	//double partArea=0.;
	//int rank, num_proc;
		...
		//for(i=0; i < np; i++) {				
			...
			//while(j<p1->bl.n){
				...
				//if(type == BONDED_IA_AREA_FORCE_GLOBAL && id == molType){ // BONDED_IA_AREA_FORCE_GLOBAL with correct molType  
					...
					//partArea += area_triangle_new(uu,vv);
				//}
				//else{
					//j+=n_partners;
				//}	
...
//MDINLINE void add_area_global_force_complicated(double area, int molType){ // more complicated approach
	//double force1[3], force2[3], force3[3];
	//int k;
	//double aa, uh[3], vh[3], rh[3], hn, uu[3],vv[3];
		...
		//for(i=0; i < np; i++) {				
			...
			//while(j<p1->bl.n){
				...
				//if(type == BONDED_IA_AREA_FORCE_GLOBAL && id == molType){ // BONDED_IA_VOLUME_FORCE with correct molType
					...
					//hn=normr(3,uh);
					//for(k=0;k<3;k++) {
						//force2[k] =  iaparams->p.area_force_global.ka_g * aa * uh[k]/hn;
					//}

					//hn=normr(3,vh);
					//for(k=0;k<3;k++) {
						//force3[k] =  iaparams->p.area_force_global.ka_g * aa * vh[k]/hn;
