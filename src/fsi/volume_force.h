...
//MDINLINE void calc_volume_complicated(double *volume, int molType){ // more complicated approach - worse results
	//double partVol=0.,normal[3],n,hz;
	 //int k; 
	//double A, rh[3];
	//double uu[3],vv[3];
		...
		//for(i=0; i < np; i++) {				
			...
			//while(j<p1->bl.n){
				...
				//if(type == BONDED_IA_VOLUME_FORCE && id == molType){ // BONDED_IA_VOLUME_FORCE with correct molType
					...
					//memcpy(ppos, p1->r.p, 3*sizeof(double));
					//memcpy(img, p1->l.i, 3*sizeof(int));
					//fold_position(ppos, img);
					
					//hz = ppos[2] - rh[2];
					//partVol += A * -1*normal[2]/n * hz;	
				//}
				//else{
					//j+=n_partners;
				//}	
...
//MDINLINE void add_volume_force_complicated(double volume, int molType){ // More complicated approach
	//double A,normal[3],n;
	////int rank, num_proc;
	//double uu[3],vv[3],vvv;
		...
		//for(i=0; i < np; i++) {				
			...
			//while(j<p1->bl.n){
				...
				//if(type == BONDED_IA_VOLUME_FORCE && id == molType){ // BONDED_IA_VOLUME_FORCE with correct molType
					...
					//vvv=(volume - iaparams->p.volume_force.V0)/iaparams->p.volume_force.V0;
		
					//for(k=0;k<3;k++) {
						//force[k]=iaparams->p.volume_force.kv * vvv * A * normal[k]/n * 1.0 / 3.0;
						//p1->f.f[k] += force[k]; 
						//p2->f.f[k] +=  force[k];
						//p3->f.f[k] +=  force[k];
					//}
