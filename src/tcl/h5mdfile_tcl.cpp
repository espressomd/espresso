...
class H5mdfile
{
    ...
    // Close simple dataspace
    int H5_Sclose_simple(int argc, char **argv, Tcl_Interp *interp);
    ...
    // Close data_type
    int H5_Tclose(int argc, char **argv, Tcl_Interp *interp);

    // Get open objects for debugging purposes
    int get_open_objects(int argc, char **argv, Tcl_Interp *interp);
};
...
int tclcommand_h5mdfile(ClientData data, Tcl_Interp *interp, int argc, char **argv)
{
    ...
    if (!strncmp(argv[1], "H5Sclose_simple", strlen(argv[1])))
    {
        if(argc!=2)
        {
            Tcl_AppendResult(interp, "\nExpected: h5mdfile H5Sclose_simple\n",(char *) NULL);
            return TCL_ERROR;
        }
        return h5mdfile.H5_Sclose_simple(argc, argv, interp);
    }
    ...
    if (!strncmp(argv[1], "get_open_objects", strlen(argv[1])))
    {
        if(argc!=2)
        {
            Tcl_AppendResult(interp, "\nExpected: h5mdfile get_open_objects\n",(char *) NULL);
            return TCL_ERROR;
        }
        return h5mdfile.get_open_objects(argc, argv, interp);
    }
    
    if (!strncmp(argv[1], "H5_Tclose", strlen(argv[1])))
    {
        if(argc!=2)
        {
            Tcl_AppendResult(interp, "\nExpected: h5mdfile H5_Tclose\n",(char *) NULL);
            return TCL_ERROR;
        }
        return h5mdfile.H5_Tclose(argc, argv, interp);
    }
    return TCL_ERROR;
}
...
int H5mdfile::H5_Dclose(int argc, char **argv, Tcl_Interp *interp)
{
    ...
    status = H5Tclose(dataset_type_id);
    status = H5Sclose(dataspace_id);
    status = H5Dclose(dataset_id);
    return TCL_OK;
}

int H5mdfile::H5_Tclose(int argc, char **argv, Tcl_Interp *interp)
{
    status = H5Tclose(dataset_type_id);
    return TCL_OK;
}
...
int H5mdfile::H5_Sclose_simple(int argc, char **argv, Tcl_Interp *interp)
{
...
int H5mdfile::H5_Screate_simple(int argc, char **argv, Tcl_Interp *interp)
{
  /*  Create simple dataspace for dataset */
    ...
    {
        dataset_type_id = H5Tcopy(H5T_NATIVE_FLOAT);
        dset_data=(float*) Utils::malloc(dset_data_size*sizeof(float));
        memset(dset_data,0,dset_data_size*sizeof(float));
    }
    else if(!strncmp(argv[3], "double", strlen(argv[3])))
    {
        dataset_type_id = H5Tcopy(H5T_NATIVE_DOUBLE);
        dset_data=(double*) Utils::malloc(dset_data_size*sizeof(double));
        memset(dset_data,0,dset_data_size*sizeof(double));
    }
    else if(!strncmp(argv[3], "int", strlen(argv[3])))
    {
        dataset_type_id = H5Tcopy(H5T_NATIVE_INT);
        dset_data=(int*) Utils::malloc(dset_data_size*sizeof(int));
        memset(dset_data,0,dset_data_size*sizeof(int));
    }
