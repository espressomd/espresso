...
public:
   ...
   int 		dataset_rank_total; // Rank of basic plus extended dataset (usually the same as dataset_rank)
   ...
   void 	*dset_data_ext; // Dataset memory for extended dataset
   int 		*dset_data_int_ext; // Dataset memory for extended dataset (type: integer)
   float 	*dset_data_float_ext; // Dataset memory for extended dataset (type: float)
   double 	*dset_data_double_ext; // Dataset memory for extended dataset (type: double)
   char 	**dset_data_string_ext; // Dataset memory for extended dataset (type: string)
   int 		dset_data_singlevalue_int_ext;// Single value from extended dataset memory (type: integer)
   float 	dset_data_singlevalue_float_ext; // Single value from extended dataset memory (type: float)
   double 	dset_data_singlevalue_double_ext; // Single value from extended dataset memory (type: double)
   double 	dset_data_singlevalue_string_ext; // Single value from extended dataset memory (type: string)
...
int tclcommand_h5mdfile(ClientData data, Tcl_Interp *interp, int argc, char **argv)
{
	...
	if (!strncmp(argv[1], "H5Dwrite_ext", strlen(argv[1])))
	{
		return h5mdfile.H5_Dwrite_ext(argv);
	}
	...
	if (!strncmp(argv[1], "H5Screate_simple_ext", strlen(argv[1])))
	{
		return h5mdfile.H5_Screate_simple_ext(argv);
	}
	if (!strncmp(argv[1], "H5Sselect_hyperslab", strlen(argv[1])))
	{
		return h5mdfile.H5_Sselect_hyperslab(argv);
	}
	if (!strncmp(argv[1], "H5_read_value_ext", strlen(argv[1])))
	{
	...
	if (!strncmp(argv[1], "H5_write_value_ext", strlen(argv[1])))
	{
		return h5mdfile.H5_write_value_ext(argv);
	}
}
...
int H5mdfile::H5_Dcreate2(char **argv)
{
   /* Create the dataset. */
	...
	printf("dataspace_id H5_Dcreate2 %i\n",dataspace_id);
	return TCL_OK;
}
int H5mdfile::H5_Dextend(char **argv)
{

   dataset_rank_total=0;
   dset_data_size_ext=1;

   while(!(argv[4+dataset_rank_total]==NULL))
   {
	   dset_data_size_ext*=atoi(argv[4+dataset_rank_total]);
	   dataset_rank_total++;
   }

   dimstotal = new hsize_t[atoi(argv[dataset_rank_total])];
   dimsext = new hsize_t[atoi(argv[dataset_rank_total])];

   for(int i=0;i<dataset_rank_total;i++)
   {
...
//	   dset_data=(float*) realloc(dset_data,dset_data_size_ext*sizeof(float));
//   }
//   if(!strncmp(argv[2], "double", strlen(argv[3])))
//   {
//	   dataset_type_id = H5T_NATIVE_DOUBLE;
//	   dset_data=(double*) realloc(dset_data,dset_data_size_ext*sizeof(double));
//   }
//   if(!strncmp(argv[2], "int", strlen(argv[3])))
//   {
//	   dataset_type_id = H5T_NATIVE_INT;
//	   dset_data=(int*) realloc(dset_data,dset_data_size_ext*sizeof(int));
//   }
//   if(!strncmp(argv[2], "str", strlen(argv[3])))
//   {
//  	   dataset_type_id = H5T_NATIVE_SCHAR;
//	   //dset_data=new char*[dset_data_size_ext];
//   }
...
int H5mdfile::H5_Dopen2(char **argv)
{
   ...
   dataset_rank = H5Sget_simple_extent_dims(dataspace_simple_id,dims,maxdims);
...
int H5mdfile::H5_Dread(char **argv)
{
   ...
   if(H5Tequal(dataset_type_id, H5T_NATIVE_SCHAR))
   {
	   //dataset_type_id = H5T_NATIVE_SCHAR;
...
int H5mdfile::H5_Dwrite_ext(char **argv)
{
	/* Write the dataset. */
	printf("write extendet\n");
	status = H5Dwrite(dataset_id, dataset_type_id, dataspace_simple_ext_id, dataspace_id, H5P_DEFAULT, dset_data_ext);
	return TCL_OK;
}
...
int H5mdfile::H5_Screate_simple_ext(char **argv)
{
   dataspace_simple_ext_id = H5Screate_simple(dataset_rank_total, dimsext, NULL);
   printf("dataspace_simple_id H5_Screate_simple_ext %i\n",dataspace_simple_ext_id);
   /* Data set array*/
   if(!strncmp(argv[2], "float", strlen(argv[3])))
   {
	   dataset_type_id = H5T_NATIVE_FLOAT;
	   dset_data_ext=(float*) malloc(dset_data_size_ext*sizeof(float));
	   memset(dset_data_ext,0,dset_data_size_ext*sizeof(float));
   }
   if(!strncmp(argv[2], "double", strlen(argv[3])))
   {
	   dataset_type_id = H5T_NATIVE_DOUBLE;
	   dset_data_ext=(double*) malloc(dset_data_size_ext*sizeof(double));
	   memset(dset_data_ext,0,dset_data_size_ext*sizeof(double));
   }
   if(!strncmp(argv[2], "int", strlen(argv[3])))
   {
	   dataset_type_id = H5T_NATIVE_INT;
	   dset_data_ext=(int*) malloc(dset_data_size_ext*sizeof(int));
	   memset(dset_data_ext,0,dset_data_size_ext*sizeof(int));
   }
   if(!strncmp(argv[2], "str", strlen(argv[3])))
   {
  	   dataset_type_id = H5T_NATIVE_SCHAR;
	   //dset_data_ext=new char*[dset_data_size];
	   //TODO memset(dset_data,0,dset_data_size*sizeof(int));
   }
...
int H5mdfile::H5_write_value_ext(char **argv)
{
	/* Write value to dataset array */
	int index;
	if(dataset_rank==1) index=atoi(argv[5]);
	...
	if(H5Tequal(dataset_type_id, H5T_NATIVE_FLOAT))
	{
		dset_data_float_ext = static_cast<float*>(dset_data_ext);
		dset_data_float_ext[index]=(float)atof(argv[3]);
	}
	if(H5Tequal(dataset_type_id, H5T_NATIVE_DOUBLE))
	{
		dset_data_double_ext = static_cast<double*>(dset_data_ext);
		dset_data_double_ext[index]=atof(argv[3]);
	}
	if(H5Tequal(dataset_type_id, H5T_NATIVE_INT))
	{
		dset_data_int_ext = static_cast<int*>(dset_data_ext);
		dset_data_int_ext[index]=atoi(argv[3]);
		printf("H5_write_value_ext %i\n",dset_data_int_ext[index]);
	}
	if(H5Tequal(dataset_type_id, H5T_NATIVE_SCHAR))
	{
//		dset_data_int = static_cast<int*>(dset_data_ext);
//		dset_data_int[index]=atoi(argv[3]);
	}
