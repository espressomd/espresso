...
int tclcommand_nemd_print_usage(Tcl_Interp *interp) {
  ...
  Tcl_AppendResult(interp,
                   ...
                   (char *)NULL);
  Tcl_AppendResult(interp,
                   "\"nemd profile\" for returning the velocity profile \n",
                   (char *)NULL);
...
int tclcommand_nemd_parse_exchange(Tcl_Interp *interp, int argc, char **argv) {
  ...
  if (n_slabs < 0 || n_slabs % 2 != 0) {
    Tcl_AppendResult(interp, "nemd <n_slabs> must be non negative and even!",
                     (char *)NULL);
    return (TCL_ERROR);
  }
  if (n_slabs > 0 && n_exchange < 0) {
    Tcl_AppendResult(interp, "nemd <n_exchange> must be positive!",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
int tclcommand_nemd_parse_and_print_viscosity(Tcl_Interp *interp) {
  ...
  char buffer[TCL_DOUBLE_SPACE];
  INTEG_TRACE(fprintf(
      stderr, "%d: tclcommand_nemd_parse_and_print_viscosity:\n", this_node));
