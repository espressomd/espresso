...
*/
/** \file constraint.cpp
    Implementation of \ref constraint.hpp "constraint.h", here it's just the
   parsing stuff.
*/
...
static int tclprint_to_result_Constraint(Tcl_Interp *interp, int i) {
  ...
  default:
    ...
    Tcl_AppendResult(interp, "unknown constraint type ", buffer, ".",
                     (char *)NULL);
    return (TCL_OK);
  }
...
static void tclprint_to_result_n_constraints(Tcl_Interp *interp) {
  ...
  sprintf(buffer, "%lu", constraints.size());
  Tcl_AppendResult(interp, buffer, (char *)NULL);
}
...
static int tclcommand_constraint_parse_wall(Constraint *con, Tcl_Interp *interp,
  ...
  con->c.wal.n[0] = con->c.wal.n[1] = con->c.wal.n[2] = 0;
  con->c.wal.d = 0;
    ...
    if (!strncmp(argv[0], "normal", strlen(argv[0]))) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint wall normal <nx> <ny> <nz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint wall dist <d> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint wall type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint penetrable <0/1> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint wall reflecting {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint wall only_positive {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint wall tunable_slip {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (norm < 1e-10 || con->part_rep.p.type < 0) {
    Tcl_AppendResult(interp, "usage: constraint wall normal <nx> <ny> <nz> "
                             "dist <d> type <t> penetrable <0/1> reflecting "
                             "<1/2>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
static int tclcommand_constraint_parse_sphere(Constraint *con,
                                              Tcl_Interp *interp, int argc,
                                              char **argv) {
  con->type = CONSTRAINT_SPH;

  /* invalid entries to start of */
  con->c.sph.pos[0] = con->c.sph.pos[1] = con->c.sph.pos[2] = 0;
  con->c.sph.rad = 0;
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "center", strlen(argv[0]))) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint sphere center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint sphere radius <r> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (!strncmp(argv[1], "inside", strlen(argv[1])))
        ...
        con->c.sph.direction = 1;
      else if (Tcl_GetDouble(interp, argv[1], &(con->c.sph.direction)) ==
               TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint sphere type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetInt(interp, argv[1], &(con->part_rep.p.type)) == TCL_ERROR)
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint sphere reflecting {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (con->c.sph.rad < 0. || con->part_rep.p.type < 0) {
    Tcl_AppendResult(interp, "usage: constraint sphere center <x> <y> <z> "
                             "radius <d> direction <direction> type <t> "
                             "penetrable <0/1> reflecting <1/2>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
static int tclcommand_constraint_parse_cylinder(Constraint *con,
                                                Tcl_Interp *interp, int argc,
                                                char **argv) {
  double axis_len;
  int i;
  ...
  /* invalid entries to start of */
  con->c.cyl.pos[0] = con->c.cyl.pos[1] = con->c.cyl.pos[2] = 0;
  con->c.cyl.axis[0] = con->c.cyl.axis[1] = con->c.cyl.axis[2] = 0;
  con->c.cyl.rad = 0;
  con->c.cyl.length = 0;
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "center", strlen(argv[0]))) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint cylinder center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint cylinder axis <rx> <ry> <rz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint cylinder radius <rad> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint cylinder length <len> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint cylinder direction <dir> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (!strncmp(argv[1], "inside", strlen(argv[1])))
        ...
        con->c.cyl.direction = 1;
      else if (Tcl_GetDouble(interp, argv[1], &(con->c.cyl.direction)) ==
               TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint cylinder type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetInt(interp, argv[1], &(con->part_rep.p.type)) == TCL_ERROR)
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "constraint cylinder penetrable <0/1> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "constraint cylinder reflecting {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (con->c.cyl.rad < 0. || con->part_rep.p.type < 0 || axis_len < 1e-30 ||
    ...
    Tcl_AppendResult(interp, "usage: constraint cylinder center <x> <y> <z> "
                             "axis <rx> <ry> <rz> radius <rad> length <length> "
                             "direction <direction> type <t> penetrable <0/1> "
                             "reflecting <1/2>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
static int tclcommand_constraint_parse_rhomboid(Constraint *con,
                                                Tcl_Interp *interp, int argc,
                                                char **argv) {
  ...
  con->c.rhomboid.pos[0] = con->c.rhomboid.pos[1] = con->c.rhomboid.pos[2] = 0;

  con->c.rhomboid.a[0] = con->c.rhomboid.a[1] = con->c.rhomboid.a[2] = 0;

  con->c.rhomboid.b[0] = con->c.rhomboid.b[1] = con->c.rhomboid.b[2] = 0;

  con->c.rhomboid.c[0] = con->c.rhomboid.c[1] = con->c.rhomboid.c[2] = 0;
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "a", strlen(argv[0]))) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint rhomboid a <ax> <ay> <az> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(con->c.rhomboid.a[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(con->c.rhomboid.a[1])) ==
              TCL_ERROR ||
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint rhomboid b <bx> <by> <bz> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(con->c.rhomboid.b[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(con->c.rhomboid.b[1])) ==
              TCL_ERROR ||
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint rhomboid c <cx> <cy> <cz> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(con->c.rhomboid.c[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(con->c.rhomboid.c[1])) ==
              TCL_ERROR ||
      ...
      argc -= 4;
      argv += 4;
    } else if (!strncmp(argv[0], "corner",
                        strlen(argv[0]))) { // this has to come after c
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint rhomboid corner <x> <y> <z> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetDouble(interp, argv[1], &(con->c.rhomboid.pos[0])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[2], &(con->c.rhomboid.pos[1])) ==
              TCL_ERROR ||
          Tcl_GetDouble(interp, argv[3], &(con->c.rhomboid.pos[2])) ==
              TCL_ERROR)
        return TCL_ERROR;
      ...
      if (argc < 2) {
        Tcl_AppendResult(
            interp, "constraint rhomboid direction {inside|outside} expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (!strncmp(argv[1], "inside", strlen(argv[1])))
        ...
        con->c.rhomboid.direction = 1;
      else if (Tcl_GetDouble(interp, argv[1], &(con->c.rhomboid.direction)) ==
               TCL_ERROR)
        return TCL_ERROR;
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint rhomboid type <t> expected",
                         (char *)NULL);
        return TCL_ERROR;
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint rhomboid penetrable {0|1} expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetInt(interp, argv[1], &(con->c.rhomboid.penetrable)) ==
          TCL_ERROR)
        return TCL_ERROR;
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint rhomboid reflecting {0|1} expected",
                         (char *)NULL);
        return TCL_ERROR;
      }

      if (Tcl_GetInt(interp, argv[1], &(con->c.rhomboid.reflecting)) ==
          TCL_ERROR)
        return TCL_ERROR;
      ...
      Tcl_AppendResult(interp, "Error: Unknown parameter ", argv[0],
                       " in constraint rhomboid", (char *)NULL);
      return TCL_ERROR;
    }
  }

  if ((con->c.rhomboid.a[0] == 0. && con->c.rhomboid.a[1] == 0. &&
       con->c.rhomboid.a[2] == 0.) ||
      (con->c.rhomboid.b[0] == 0. && con->c.rhomboid.b[1] == 0. &&
       con->c.rhomboid.b[2] == 0.) ||
      (con->c.rhomboid.c[0] == 0. && con->c.rhomboid.c[1] == 0. &&
       con->c.rhomboid.c[2] == 0.) ||
      con->part_rep.p.type < 0 || con->c.rhomboid.direction == 0) {
    Tcl_AppendResult(interp, "usage: constraint rhomboid corner <x> <y> <z> a "
  ...
  triple_product =
      con->c.rhomboid.a[0] * (con->c.rhomboid.b[1] * con->c.rhomboid.c[2] -
                              con->c.rhomboid.b[2] * con->c.rhomboid.c[1]) +
      con->c.rhomboid.a[1] * (con->c.rhomboid.b[2] * con->c.rhomboid.c[0] -
                              con->c.rhomboid.b[0] * con->c.rhomboid.c[2]) +
      con->c.rhomboid.a[2] * (con->c.rhomboid.b[0] * con->c.rhomboid.c[1] -
                              con->c.rhomboid.b[1] * con->c.rhomboid.c[0]);
...
static int tclcommand_constraint_parse_pore(Constraint *con, Tcl_Interp *interp,
  ...
  /* invalid entries to start of */
  con->c.pore.pos[0] = con->c.pore.pos[1] = con->c.pore.pos[2] = 0;
  con->c.pore.axis[0] = con->c.pore.axis[1] = con->c.pore.axis[2] = 0;
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "center", strlen(argv[0]))) {
      if (argc < 4) {
        Tcl_AppendResult(interp, "constraint pore center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(interp, "constraint pore axis <rx> <ry> <rz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint pore radius <rad> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint pore outer_radius <rad> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.pore.outer_rad_left)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp,
            "constraint pore smoothing_radius <smoothing_radius> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.pore.smoothing_radius)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp, "constraint pore radii <rad_left> <rad_right> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.pore.rad_left)) == TCL_ERROR)
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp,
            "constraint pore outer_radii <rad_left> <rad_right> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.pore.outer_rad_left)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      if (Tcl_GetDouble(interp, argv[2], &(con->c.pore.outer_rad_right)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      argc -= 3;
      argv += 3;
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint pore length <len/2> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint pore type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetInt(interp, argv[1], &(con->part_rep.p.type)) == TCL_ERROR)
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint pore reflecting {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (con->c.pore.rad_left < 0. || con->c.pore.rad_right < 0. ||
      con->part_rep.p.type < 0 || axis_len < 1e-30 || con->c.pore.length <= 0) {
    Tcl_AppendResult(interp, "usage: constraint pore center <x> <y> <z> axis "
...
static int tclcommand_constraint_parse_slitpore(Constraint *con,
                                                Tcl_Interp *interp, int argc,
                                                char **argv) {
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "pore_mouth", strlen(argv[0]))) {
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint slitpore mouth <mouth> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.slitpore.pore_mouth)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "constraint slitpore pore_width <pore_width> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.slitpore.pore_width)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp, "constraint slitpore pore_width <pore_length> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.slitpore.pore_length)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp,
            "constraint slitpore channel_width <channel_width> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1], &(con->c.slitpore.channel_width)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp, "constraint slitpore upper_smoothing_radius <r> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1],
                        &(con->c.slitpore.upper_smoothing_radius)) == TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
      ...
      if (argc < 1) {
        Tcl_AppendResult(
            interp, "constraint slitpore lower_smoothing_radius <r> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetDouble(interp, argv[1],
                        &(con->c.slitpore.lower_smoothing_radius)) == TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
    } else if (!strncmp(argv[0], "type", strlen(argv[0]))) {
      ...
      if (Tcl_GetInt(interp, argv[1], &(con->c.slitpore.reflecting)) ==
          TCL_ERROR)
        return (TCL_ERROR);
      argc -= 2;
      argv += 2;
  ...
  if (con->c.slitpore.channel_width <= 0.) {
    Tcl_AppendResult(interp,
                     "Error in contraint slitpore: Channel with must be > 0",
                     (char *)NULL);
    error = 1;
  }
  if (con->c.slitpore.pore_width <= 0.) {
    Tcl_AppendResult(interp,
                     "Error in contraint slitpore: Pore width must be > 0",
                     (char *)NULL);
    error = 1;
  }
  if (con->c.slitpore.pore_length < 0.) {
    Tcl_AppendResult(interp,
                     "Error in contraint slitpore: Pore length must be > 0",
                     (char *)NULL);
    error = 1;
  }
  if (con->part_rep.p.type < 0) {
    Tcl_AppendResult(interp, "Error in contraint slitpore: Type not set",
                     (char *)NULL);
    error = 1;
  }
...
static int tclcommand_constraint_parse_rod(Constraint *con, Tcl_Interp *interp,
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "center", strlen(argv[0]))) {
      if (argc < 3) {
        Tcl_AppendResult(interp, "constraint rod center <px> <py> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      Tcl_AppendResult(interp, "constraint rod radius <r> is deprecated, "
                               "please use a cylinder for LJ component",
                       (char *)NULL);
      return (TCL_ERROR);
    } else if (!strncmp(argv[0], "type", strlen(argv[0]))) {
      Tcl_AppendResult(interp, "constraint rod type <t> is deprecated, please "
                               "use a cylinder for LJ component",
                       (char *)NULL);
      return (TCL_ERROR);
    } else if (!strncmp(argv[0], "lambda", strlen(argv[0]))) {
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint rod lambda <l> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
...
static int tclcommand_constraint_parse_plate(Constraint *con,
                                             Tcl_Interp *interp, int argc,
                                             char **argv) {
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "height", strlen(argv[0]))) {
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint plate height <pz> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint rod sigma <s> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
...
static int tclcommand_constraint_parse_stomatocyte(Constraint *con,
                                                   Tcl_Interp *interp, int argc,
                                                   char **argv) {
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint stomatocyte center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(
            interp,
            "constraint stomatocyte orientation <ox> <oy> <oz> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint stomatocyte outer_radius <Ro> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint stomatocyte inner_radius <Ri> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint stomatocyte layer_width <w> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint stomatocyte direction {-1|1} or "
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint stomatocyte type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint stomatocyte penetrable {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "constraint stomatocyte reflecting {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
...
static int tclcommand_constraint_parse_hollow_cone(Constraint *con,
                                                   Tcl_Interp *interp, int argc,
                                                   char **argv) {
  ...
  while (argc > 0) {
    if (ARG_IS_S(0, "center")) {
      if (argc < 4) {
        Tcl_AppendResult(interp,
                         "constraint hollow_cone center <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 4) {
        Tcl_AppendResult(
            interp,
            "constraint hollow_cone orientation <ox> <oy> <oz> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint hollow_cone outer_radius <Ro> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint hollow_cone inner_radius <Ri> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint hollow_cone width <w> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(
            interp, "constraint hollow_cone opening_angle <alpha> expected",
            (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint hollow_cone direction {-1|1} or "
                                 "{inside|outside} is expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint hollow_cone type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 2) {
        Tcl_AppendResult(interp,
                         "constraint hollow_cone penetrable {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp,
                         "constraint hollow_cone reflecting {0|1} expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
  ...
  if (con->c.hollow_cone.outer_radius < 0.0 ||
      con->c.hollow_cone.inner_radius < 0.0 || con->c.hollow_cone.width < 0.0) {
    Tcl_AppendResult(interp,
...
static int tclcommand_constraint_parse_maze(Constraint *con, Tcl_Interp *interp,
  ...
  while (argc > 0) {
    if (!strncmp(argv[0], "nsphere", strlen(argv[0]))) {
      if (argc < 4) {
        Tcl_AppendResult(interp, "constraint maze nsphere <n> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint maze dim <d> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint maze sphrad <r> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint maze cylrad <r> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint maze type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetInt(interp, argv[1], &(con->part_rep.p.type)) == TCL_ERROR)
  ...
  if (con->c.maze.sphrad < 0. || con->c.maze.cylrad < 0. ||
      con->part_rep.p.type < 0 || con->c.maze.dim < 0) {
    Tcl_AppendResult(interp, "usage: constraint maze nsphere <n> dim <d> "
                             "sphrad <r> cylrad <r> type <t> penetrable <0/1>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
// ER
int tclcommand_constraint_parse_ext_magn_field(Constraint *con,
                                               Tcl_Interp *interp, int argc,
                                               char **argv) {
  int i;
  ...
  if (argc < 3) {
    Tcl_AppendResult(interp, "usage: constraint ext_magn_field <x> <y> <z>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
  for (i = 0; i < 3; i++) {
    if (Tcl_GetDouble(interp, argv[i], &(con->c.emfield.ext_magn_field[i])) ==
        TCL_ERROR)
      return (TCL_ERROR);
  }
  argc -= 3;
...
static int tclcommand_constraint_parse_plane_cell(Constraint *con,
                                                  Tcl_Interp *interp, int argc,
                                                  char **argv) {
  Tcl_AppendResult(
      interp, "constraint plane cell deprecated, use constraint wall instead!",
      (char *)NULL);
  return (TCL_ERROR);
  con->type = CONSTRAINT_PLANE;

  /* invalid entries to start of */
  con->c.plane.pos[0] = con->c.plane.pos[1] = con->c.plane.pos[2] = 0;
  con->part_rep.p.type = -1;

  while (argc > 0) {
    if (!strncmp(argv[0], "cell", strlen(argv[0]))) {
      if (argc < 4) {
        Tcl_AppendResult(interp, "constraint plane cell <x> <y> <z> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint plane cell type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetInt(interp, argv[1], &(con->part_rep.p.type)) == TCL_ERROR)
  ...
  if (con->part_rep.p.type < 0) {
    Tcl_AppendResult(interp,
                     "usage: constraint plane cell <x> <y> <z> type <t>",
                     (char *)NULL);
    return (TCL_ERROR);
  }
...
static int tclcommand_constraint_parse_voxel(Constraint *con,
                                             Tcl_Interp *interp, int argc,
                                             char **argv) {
  FILE *fp;
  ...
  while (argc > 0) {
    ...
    // if(argc < 4) {
    // Tcl_AppendResult(interp, "constraint voxel pos <x> <y> <z> expected",
    // (char *) NULL);
    // return (TCL_ERROR);
    //}
    // if (Tcl_GetDouble(interp, argv[1], &(con->c.sph.pos[0])) == TCL_ERROR ||
    ...
    if (!strncmp(argv[0], "file", strlen(argv[0]))) {
      if (argc < 2) {
        Tcl_AppendResult(interp, "constraint voxel file <filename> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      fp = fopen(argv[1], "r");
      ...
      if (argc < 1) {
        Tcl_AppendResult(interp, "constraint voxel type <t> expected",
                         (char *)NULL);
        return (TCL_ERROR);
      }
      if (Tcl_GetInt(interp, argv[1], &(con->part_rep.p.type)) == TCL_ERROR)
...
static int tclcommand_constraint_mindist_position(Tcl_Interp *interp, int argc,
                                                  char **argv) {
  double pos[3];
  double vec[3];
  double dist = 1e100;
  ...
  if (constraints.empty()) {
    Tcl_AppendResult(
        interp,
        "Error in constraint mindist_position: no constraints defined\n",
        (char *)NULL);
    return TCL_ERROR;
  }
    ...
    for (n = 0; n < constraints.size(); n++) {
      ...
      case CONSTRAINT_WAL:
        if (!constraints[n].c.wal.penetrable)
          calculate_wall_dist(p1, pos, &constraints[n].part_rep,
      ...
      case CONSTRAINT_SPH:
        if (!constraints[n].c.sph.penetrable)
          calculate_sphere_dist(p1, pos, &constraints[n].part_rep,
      ...
      case CONSTRAINT_CYL:
        if (!constraints[n].c.cyl.penetrable)
          calculate_cylinder_dist(p1, pos, &constraints[n].part_rep,
      ...
      case CONSTRAINT_RHOMBOID:
        if (!constraints[n].c.rhomboid.penetrable)
          calculate_rhomboid_dist(p1, pos, &constraints[n].part_rep,
      ...
      case CONSTRAINT_MAZE:
        if (!constraints[n].c.maze.penetrable)
          calculate_maze_dist(p1, pos, &constraints[n].part_rep,
      ...
      case CONSTRAINT_STOMATOCYTE:
        if (!constraints[n].c.stomatocyte.penetrable)
          calculate_stomatocyte_dist(p1, pos, &constraints[n].part_rep,
      ...
      case CONSTRAINT_HOLLOW_CONE:
        if (!constraints[n].c.hollow_cone.penetrable)
          calculate_hollow_cone_dist(p1, pos, &constraints[n].part_rep,
                                     &constraints[n].c.hollow_cone, &dist, vec);
        else
          dist = 1e100;
        break;
      case CONSTRAINT_PORE:
        calculate_pore_dist(p1, pos, &constraints[n].part_rep,
                            &constraints[n].c.pore, &dist, vec);
        break;
      case CONSTRAINT_SLITPORE:
        calculate_slitpore_dist(p1, pos, &constraints[n].part_rep,
                                &constraints[n].c.slitpore, &dist, vec);
        break;
      case CONSTRAINT_PLANE:
        calculate_plane_dist(p1, pos, &constraints[n].part_rep,
                             &constraints[n].c.plane, &dist, vec);
        break;
      case CONSTRAINT_VOXEL:
        calculate_voxel_dist(p1, pos, &constraints[n].part_rep,
...
int tclcommand_constraint_mindist_position_vec(Tcl_Interp *interp, int argc,
                                               char **argv) {
  double pos[3];
  double vec[3];
  double dist = 1e100;
  ...
  if (argc < 3) {
    Tcl_AppendResult(interp,
                     "Usage: constraint mindist_position_vec <px> <py> <py>\n",
                     (char *)NULL);
    return TCL_ERROR;
  }
...
int tclcommand_constraint(ClientData _data, Tcl_Interp *interp, int argc,
                          char **argv) {
#ifdef CONSTRAINTS
  int status, c_num;

  if (argc < 2)
    return tclcommand_constraint_print(interp);
  ...
  if (!strncmp(argv[1], "mindist_position_vec", strlen(argv[1]))) {
    return tclcommand_constraint_mindist_position_vec(interp, argc - 2,
                                                      argv + 2);
  } else if (!strncmp(argv[1], "wall", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_wall(generate_constraint(), interp,
                                              argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "sphere", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_sphere(generate_constraint(), interp,
                                                argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "cylinder", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_cylinder(generate_constraint(), interp,
                                                  argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "rhomboid", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_rhomboid(generate_constraint(), interp,
                                                  argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "rod", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_rod(generate_constraint(), interp,
                                             argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "plate", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_plate(generate_constraint(), interp,
                                               argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "maze", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_maze(generate_constraint(), interp,
                                              argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "pore", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_pore(generate_constraint(), interp,
                                              argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "slitpore", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_slitpore(generate_constraint(), interp,
                                                  argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "stomatocyte", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_stomatocyte(
        generate_constraint(), interp, argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "hollow_cone", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_hollow_cone(
        generate_constraint(), interp, argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  }
  // ER
  else if (!strncmp(argv[1], "ext_magn_field", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_ext_magn_field(
        generate_constraint(), interp, argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "plane cell", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_plane_cell(generate_constraint(),
                                                    interp, argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  } else if (!strncmp(argv[1], "voxel", strlen(argv[1]))) {
    status = tclcommand_constraint_parse_voxel(generate_constraint(), interp,
                                               argc - 2, argv + 2);
    mpi_bcast_constraint(-1);
  }
  // end ER
    ...
    if (Tcl_GetInt(interp, argv[2], &(c_num)) == TCL_ERROR)
      return (TCL_ERROR);
    if (c_num < 0 || c_num >= constraints.size()) {
    ...
    if (argc < 3) {
      ...
      if (Tcl_GetInt(interp, argv[2], &(c_num)) == TCL_ERROR)
        return (TCL_ERROR);
      if (c_num < 0 || c_num >= constraints.size()) {
        Tcl_AppendResult(interp, "Can not delete non existing constraint",
                         (char *)NULL);
        return (TCL_ERROR);
      }
    ...
    Tcl_AppendResult(interp, "possible constraints: wall sphere cylinder maze "
                             "pore stomatocyte hollow_cone ext_magn_field or "
                             "constraint delete {c} to delete constraint(s)",
                     (char *)NULL);
    return (TCL_ERROR);
  }
