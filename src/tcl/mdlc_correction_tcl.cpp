...
int tclcommand_inter_magnetic_parse_mdlc_params(Tcl_Interp *interp, int argc,
                                                char **argv) {
  double pwerror;
  double gap_size;
  double far_cut = -1;

  MDLC_TRACE(fprintf(stderr,
                     "%d: tclcommand_inter_magnetic_parse_mdlc_params().\n",
                     this_node));

  if (argc < 2) {
    Tcl_AppendResult(interp, "either nothing or mdlc <pwerror> <minimal layer "
                             "distance> {<cutoff>}  expected, not \"",
                     argv[0], "\"", (char *)NULL);
    return TCL_ERROR;
  }
  ...
  if (argc > 0) {
    ...
    if (argc > 0) {
      Tcl_AppendResult(interp, "either nothing or mdlc <pwerror> <minimal "
                               "layer distance=size of the gap without "
                               "particles> {<cutoff>}   expected, not \"",
                       argv[0], "\"", (char *)NULL);
      return TCL_ERROR;
    }
  }
