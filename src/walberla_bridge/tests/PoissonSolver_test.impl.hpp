...
#if defined(WALBERLA_BUILD_WITH_CUDA) and defined(ESPRESSO_TEST_BUILD_FOR_GPU)
    ...
    for (auto const &node : all_nodes_incl_ghosts(*params.lattice)) {
      if (params.lattice->node_in_local_halo(node)) {
        if (params.lattice->node_in_local_domain(node)) {
          ...
          BOOST_REQUIRE(not found);
          auto const found_in_halo = ek_solver.get_node_potential(node, true);
          if (not found_in_halo) {
            std::ignore = ek_solver.get_node_potential(node, true);
          }
          BOOST_REQUIRE(found_in_halo);
        }
      } else {
...
#if not defined(ESPRESSO_TEST_BUILD_FOR_GPU)
...
#if defined(HAS_HEFFTE) and not defined(ESPRESSO_TEST_BUILD_FOR_GPU)
...
#if defined(HAS_HEFFTE) and defined(WALBERLA_BUILD_WITH_CUDA) and              \
    defined(__CUDACC__) and defined(ESPRESSO_TEST_BUILD_FOR_GPU)
  Fixture<FT, lbmpy::Arch::GPU>().runTest();
#endif
}

BOOST_AUTO_TEST_CASE(ek_poisson_solver_none_cuda) {
#if defined(WALBERLA_BUILD_WITH_CUDA) and defined(__CUDACC__) and              \
    defined(ESPRESSO_TEST_BUILD_FOR_GPU)
