...
LbGeneratorVector unthermalized_lbs() {
  ...
  lbs.push_back(
      [](const Utils::Vector3i mpi_shape, const LBTestParameters &params) {
        return std::make_shared<walberla::LBWalberlaImpl<CollisionModelNameUnthermalized>>(
  ...
  lbs.push_back([](Utils::Vector3i mpi_shape, const LBTestParameters &params) {
    return std::make_shared<walberla::LBWalberlaImpl<CollisionModelNameThermalized>>(
