...
public:
    ...
    Dirichlet_single_precision( const shared_ptr<StructuredBlockForest> & blocks,
                   BlockDataID fieldID_, std::function<float(const Cell &, const shared_ptr<StructuredBlockForest>&, IBlock&)>& dirichletCallback )
        :elementInitaliser(dirichletCallback), fieldID(fieldID_)
    {
        auto createIdxVector = []( IBlock * const , StructuredBlockStorage * const ) { return new IndexVectors(); };
        indexVectorID = blocks->addStructuredBlockData< IndexVectors >( createIdxVector, "IndexField_Dirichlet_single_precision");
    };
    
    Dirichlet_single_precision(BlockDataID fieldID_, BlockDataID indexVectorID_, std::function<float(const Cell &, const shared_ptr<StructuredBlockForest>&, IBlock&)>& dirichletCallback )
        : elementInitaliser(dirichletCallback), fieldID(fieldID_), indexVectorID(indexVectorID_)
    {};
                            ...
                            FlagUID boundaryFlagUID, FlagUID domainFlagUID )
    {
        ...
        for( auto it = flagField->beginWithGhostLayerXYZ(); it != flagField->end(); ++it )
        {
            ...
            if ( flagWithGLayers.contains(it.x() + cell_idx_c(0), it.y() + cell_idx_c(0), it.z() + cell_idx_c(0)) && isFlagSet( it.neighbor(0, 0, 0 , 0 ), domainFlag ) )
            {
                ...
                float InitialisatonAdditionalData = elementInitaliser(Cell(it.x(), it.y(), it.z()), blocks, *block);
...
private:
    ...
    std::function<float(const Cell &, const shared_ptr<StructuredBlockForest>&, IBlock&)> elementInitaliser; 
public:
    BlockDataID fieldID;
};
