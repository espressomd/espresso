...
public:
    ...
    FixedFlux_single_precision( const shared_ptr<StructuredBlockForest> & blocks,
                   BlockDataID fluxID_, std::function<Vector3<float>(const Cell &, const shared_ptr<StructuredBlockForest>&, IBlock&)>& fluxCallback )
                            ...
                            FlagUID boundaryFlagUID, FlagUID domainFlagUID )
    {
        ...
        for( auto it = flagField->beginWithGhostLayerXYZ( cell_idx_c( flagField->nrOfGhostLayers() - 1 ) ); it != flagField->end(); ++it )
        {
           ...
           if ( isFlagSet( it.neighbor(1, 1, 1 , 0 ), boundaryFlag ) )
           {
              ...
              Vector3<float> InitialisatonAdditionalData = elementInitaliser(Cell(it.x() + 1, it.y() + 1, it.z() + 1), blocks, *block);
                element.flux_0 = InitialisatonAdditionalData[0];
                element.flux_1 = InitialisatonAdditionalData[1];
                element.flux_2 = InitialisatonAdditionalData[2];
              indexVectorAll.push_back( element );
