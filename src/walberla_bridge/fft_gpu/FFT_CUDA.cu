...
__global__ void create_greens_function(gpu::FieldAccessor<double> greens_function, int x, int y, int z) {
...
__global__ void multiply_by_greens_function(gpu::FieldAccessor<cufftDoubleComplex> potential, gpu::FieldAccessor<double> greens_function) {
  potential.set(blockIdx, threadIdx);
  if (potential.isValidPosition()) {
    potential.get(0u) = cufftDoubleComplex(potential.get(0u).x * greens_function.get(0u), potential.get(0u).y * greens_function.get(0u));
  }
...
FFT_CUDA<FloatType>::FFT_CUDA(std::shared_ptr<LatticeWalberla> lattice, double permittivity) 
  ...
  m_potential_field_id = gpu::addGPUFieldToStorage<PotentialField>(
      get_lattice().get_blocks(), "potential field", 1,
      field::fzyx, get_lattice().get_ghost_layers());
  m_greens_function_field_id = gpu::addGPUFieldToStorage<GreenFunctionField>(
      get_lattice().get_blocks(), "greens function", 1,
      field::fzyx, get_lattice().get_ghost_layers());
  m_potential_furier_id = gpu::addGPUFieldToStorage<PotentialFurier>(
      get_lattice().get_blocks(), "furier field", 1,
      field::fzyx, get_lattice().get_ghost_layers());
  reset_charge_field();

  auto dim = get_lattice().get_grid_dimensions();
  m_box_in = std::make_shared<heffte::box3d<>>(to_array(Utils::Vector3i({0,0,0})), to_array(dim));
  m_box_out = std::make_shared<heffte::box3d<>>(to_array(Utils::Vector3i({0,0,0})), to_array(dim));
  m_fft = std::make_shared<heffte::fft3d<heffte::backend::cufft>>(*m_box_in, *m_box_out, MPI_COMM_WORLD);
  m_buffer = std::make_shared<heffte::fft3d<heffte::backend::cufft>::buffer_container<ComplexType>>(m_fft->size_workspace());
  // m_fft_out = std::make_shared<heffte::gpu::vector<std::complex<FloatType>>>(m_fft->size_outbox());


  auto block = get_lattice().get_blocks()->getBlock(0,0,0);
  auto green_field = block->template getData<GreenFunctionField>(m_greens_function_field_id);
  auto kernel = gpu::make_kernel(create_greens_function);
  kernel.addFieldIndexingParam(gpu::FieldIndexing<FloatType>::xyz(*green_field));
...
template <typename FloatType>
void  FFT_CUDA<FloatType>::reset_charge_field() {
...
template <typename FloatType>
void  FFT_CUDA<FloatType>::solve() {
  for (auto &block : *get_lattice().get_blocks()) {
    auto potential = block.template getData<PotentialField>(m_potential_field_id);
    auto green = block.template getData<GreenFunctionField>(m_greens_function_field_id);
    auto furier = block.template getData<PotentialFurier>(m_potential_furier_id);
    ...
    kernel();
    // KERNELCALL(multiply_by_greens_function, dim_grid, threads_per_block, _data_potential);
