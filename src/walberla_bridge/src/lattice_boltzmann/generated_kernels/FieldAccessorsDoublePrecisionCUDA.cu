...
__global__ void kernel_get_interval(cuda::FieldAccessor<double> pdf,
...
__global__ void kernel_get(cuda::FieldAccessor<double> pdf,
                           double *RESTRICT const pop) {
  pdf.set(blockIdx, threadIdx);
  if (pdf.isValidPosition()) {
    const uint offset =
...
__global__ void kernel_set_interval(cuda::FieldAccessor<double> pdf,
...
__global__ void kernel_set(cuda::FieldAccessor<double> pdf,
                           const double *RESTRICT const pop) {
  pdf.set(blockIdx, threadIdx);
  if (pdf.isValidPosition()) {
    const uint offset =
...
std::array<double, 19u> get(cuda::GPUField<double> const *pdf_field,
  ...
  kernel.addFieldIndexingParam(
      cuda::FieldIndexing<double>::interval(*pdf_field, ci));
...
namespace Vector {
__global__ void kernel_get_interval(cuda::FieldAccessor<double> vec,
                                    double *const out) {
...
__global__ void kernel_get(cuda::FieldAccessor<double> vec, double *const out) {
  vec.set(blockIdx, threadIdx);
  if (vec.isValidPosition()) {
    const uint offset =
        getLinearIndexFZYX(blockIdx, threadIdx, gridDim, blockDim, 3u);
...
__global__ void kernel_set_interval(cuda::FieldAccessor<double> vec,
...
__global__ void kernel_set(cuda::FieldAccessor<double> vec,
                           const double *RESTRICT const u) {
  vec.set(blockIdx, threadIdx);
  if (vec.isValidPosition()) {
    const uint offset =
...
__global__ void kernel_add_interval(cuda::FieldAccessor<double> vec,
                                    const double *RESTRICT const u) {
  vec.set(blockIdx, threadIdx);
  if (vec.isValidPosition()) {
    const uint offset =
...
__global__ void kernel_add(cuda::FieldAccessor<double> vec,
                           const double *RESTRICT const u) {
  vec.set(blockIdx, threadIdx);
  if (vec.isValidPosition()) {
    const uint offset =
...
Vector3<double> get(cuda::GPUField<double> const *vec_field, Cell const &cell) {
  CellInterval ci(cell, cell);
...
void set(cuda::GPUField<double> *vec_field, Vector3<double> const &vec,
         Cell const &cell) {
  CellInterval ci(cell, cell);
...
void add(cuda::GPUField<double> *vec_field, Vector3<double> const &vec,
         Cell const &cell) {
  CellInterval ci(cell, cell);
  thrust::device_vector<double> dev_data(vec.data(), vec.data() + 3u);
  auto const dev_data_ptr = thrust::raw_pointer_cast(dev_data.data());
...
void broadcast(cuda::GPUField<double> *vec_field, Vector3<double> const &vec) {
  CellInterval ci = vec_field->xyzSizeWithGhostLayer();
  thrust::device_vector<double> dev_data(vec.data(), vec.data() + 3u);
  auto const dev_data_ptr = thrust::raw_pointer_cast(dev_data.data());
  auto kernel = cuda::make_kernel(kernel_set);
...
void add_to_all(cuda::GPUField<double> *vec_field, Vector3<double> const &vec) {
  CellInterval ci = vec_field->xyzSizeWithGhostLayer();
  thrust::device_vector<double> dev_data(vec.data(), vec.data() + 3u);
  auto const dev_data_ptr = thrust::raw_pointer_cast(dev_data.data());
  auto kernel = cuda::make_kernel(kernel_add);
...
__device__ void kernel_set_device(cuda::FieldAccessor<double> pdf,
...
void set(cuda::GPUField<double> *pdf_field, const double rho,
         Cell const &cell) {
  CellInterval ci(cell, cell);
...
void set(cuda::GPUField<double> *pdf_field, cuda::GPUField<double> *force_field,
  ...
  kernel.addFieldIndexingParam(
      cuda::FieldIndexing<double>::interval(*pdf_field, ci));
  kernel.addFieldIndexingParam(
      cuda::FieldIndexing<double>::interval(*force_field, ci));
  kernel.addParam(const_cast<const double *>(dev_data_ptr));
  kernel();
}
} // namespace Velocity
...
__global__ void kernel_sum(cuda::FieldAccessor<double> pdf,
                           cuda::FieldAccessor<double> force,
                           double *RESTRICT const out) {
  pdf.set(blockIdx, threadIdx);
  force.set(blockIdx, threadIdx);
