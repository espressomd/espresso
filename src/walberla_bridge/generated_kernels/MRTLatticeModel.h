...
/**
MRTLatticeModel was generated with lbmpy. Do not edit this file directly.
Instead modify MRTLatticeModel.py. For details see documentation of lbmpy.

Usage:
    - Create an instance of this lattice model class: the constructor parameters
vary depending on the configure lattice model. A model with constant force needs
a single force vector, while a model with variable forces needs a force field.
All constructor parameters are ordered alphabetically.
    - Create a PDFField with the lattice model as template argument to store the
particle distribution functions. Use the PDFField to get and modify macroscopic
values.
    - The internal class MRTLatticeModel::Sweep is a functor to execute one LB
time step. Stream, collide steps can be executed separately, or together in an
optimized stream-pull-collide scheme
...
public:
  ...
  public:
    ...
    // void stream       ( IBlock * const block, const uint_t
    // numberOfGhostLayersToInclude = uint_t(0) );
    void collide(IBlock *const block,
                 const uint_t numberOfGhostLayersToInclude = uint_t(0));
    void streamCollide(IBlock *const block,
                       const uint_t numberOfGhostLayersToInclude = uint_t(0));
    void stream(IBlock *const block,
                const uint_t numberOfGhostLayersToInclude = uint_t(0));
  ...
  MRTLatticeModel(BlockDataID forceID_, double omega_bulk, double omega_even,
                  double omega_odd, double omega_shear)
      : forceID(forceID_), omega_bulk_(omega_bulk), omega_even_(omega_even),
        omega_odd_(omega_odd), omega_shear_(omega_shear), currentLevel(0){};

  void configure(IBlock &block, StructuredBlockStorage &storage) {
    configureBlock(&block, &storage);
  }
...
private:
  ...
  friend struct internal::AdaptVelocityToForce;
  ...
  friend struct DensityAndVelocityRange;

  friend mpi::SendBuffer & ::walberla::mpi::operator<<(mpi::SendBuffer &,
                                                       const MRTLatticeModel &);
  friend mpi::RecvBuffer & ::walberla::mpi::operator>>(mpi::RecvBuffer &,
                                                       MRTLatticeModel &);
};
...
template <> class EquilibriumDistribution<MRTLatticeModel, void> {
public:
...
template <> struct AdaptVelocityToForce<MRTLatticeModel, void> {
  ...
  static Vector3<real_t> get(const cell_idx_t x, const cell_idx_t y,
                             const cell_idx_t z, const MRTLatticeModel &lm,
                             const Vector3<real_t> &velocity,
                             const real_t rho) {
...
template <> struct Equilibrium<MRTLatticeModel, void> {
...
template <> struct Density<MRTLatticeModel, void> {
  template <typename FieldPtrOrIterator>
  ...
  template <typename PdfField_T>
  static inline real_t get(const MRTLatticeModel &, const PdfField_T &pdf,
...
template <> struct DensityAndVelocity<MRTLatticeModel> {
  template <typename FieldPtrOrIterator>
  ...
  static void set(PdfField_T &pdf, const cell_idx_t x, const cell_idx_t y,
    ...
    Equilibrium<MRTLatticeModel>::set(pdf, x, y, z,
...
struct DensityAndVelocityRange<MRTLatticeModel, FieldIteratorXYZ> {
  ...
  static void set(FieldIteratorXYZ &begin, const FieldIteratorXYZ &end,
    ...
    for (auto cellIt = begin; cellIt != end; ++cellIt) {
      ...
      Equilibrium<MRTLatticeModel>::set(cellIt, Vector3<real_t>(u_0, u_1, u_2),
                                        rho);
    }
  }
};

template <> struct DensityAndMomentumDensity<MRTLatticeModel> {
  template <typename FieldPtrOrIterator>
...
template <> struct MomentumDensity<MRTLatticeModel> {
  template <typename FieldPtrOrIterator>
  static void get(Vector3<real_t> &momentumDensity, const MRTLatticeModel &lm,
                  const FieldPtrOrIterator &it) {
    const auto x = it.x();
    const auto y = it.y();
    const auto z = it.z();
...
template <> struct PressureTensor<MRTLatticeModel> {
  template <typename FieldPtrOrIterator>
  static void get(Matrix3<real_t> & /* pressureTensor */,
                  const MRTLatticeModel & /* latticeModel */,
                  const FieldPtrOrIterator & /* it */) {
...
template <> struct ShearRate<MRTLatticeModel> {
  template <typename FieldPtrOrIterator>
  ...
  get(const MRTLatticeModel &latticeModel, const PdfField_T & /* pdf */,
      const cell_idx_t /* x */, const cell_idx_t /* y */,
