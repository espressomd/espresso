...
/**
MRTLatticeModelAvx was generated with lbmpy. Do not edit this file directly.
Instead modify MRTLatticeModelAvx.py. For details see documentation of lbmpy.
...
values.
    - The internal class MRTLatticeModelAvx::Sweep is a functor to execute one
LB time step. Stream, collide steps can be executed separately, or together in
an optimized stream-pull-collide scheme
...
public:
  ...
  MRTLatticeModelAvx(BlockDataID forceID_, double omega_bulk, double omega_even,
...
private:
  ...
  friend class MRTLatticeModelAvx::Sweep;
  ...
  friend mpi::SendBuffer & ::walberla::mpi::operator<<(
      mpi::SendBuffer &, const MRTLatticeModelAvx &);
...
template <> class EquilibriumDistribution<MRTLatticeModelAvx, void> {
public:
  typedef typename MRTLatticeModelAvx::Stencil Stencil;
...
template <> struct AdaptVelocityToForce<MRTLatticeModelAvx, void> {
  template <typename FieldPtrOrIterator>
  static Vector3<real_t>
  get(FieldPtrOrIterator &it, const MRTLatticeModelAvx &lm,
  ...
  static Vector3<real_t> get(const cell_idx_t x, const cell_idx_t y,
                             const cell_idx_t z, const MRTLatticeModelAvx &lm,
...
template <> struct Equilibrium<MRTLatticeModelAvx, void> {
...
template <> struct Density<MRTLatticeModelAvx, void> {
  template <typename FieldPtrOrIterator>
  static inline real_t get(const MRTLatticeModelAvx &,
  ...
  template <typename PdfField_T>
  static inline real_t get(const MRTLatticeModelAvx &, const PdfField_T &pdf,
...
template <> struct DensityAndVelocity<MRTLatticeModelAvx> {
  template <typename FieldPtrOrIterator>
  static void set(FieldPtrOrIterator &it, const MRTLatticeModelAvx &lm,
                  const Vector3<real_t> &u = Vector3<real_t>(real_t(0.0)),
    ...
    Equilibrium<MRTLatticeModelAvx>::set(it, Vector3<real_t>(u_0, u_1, u_2),
                                         rho);
  }
  ...
  static void set(PdfField_T &pdf, const cell_idx_t x, const cell_idx_t y,
    ...
    Equilibrium<MRTLatticeModelAvx>::set(pdf, x, y, z,
                                         Vector3<real_t>(u_0, u_1, u_2), rho);
  }
};

template <typename FieldIteratorXYZ>
struct DensityAndVelocityRange<MRTLatticeModelAvx, FieldIteratorXYZ> {

  static void set(FieldIteratorXYZ &begin, const FieldIteratorXYZ &end,
                  const MRTLatticeModelAvx &lm,
                  const Vector3<real_t> &u = Vector3<real_t>(real_t(0.0)),
                  const real_t rho_in = real_t(1.0)) {
    for (auto cellIt = begin; cellIt != end; ++cellIt) {
      ...
      Equilibrium<MRTLatticeModelAvx>::set(cellIt,
                                           Vector3<real_t>(u_0, u_1, u_2), rho);
    }
  }
};

template <> struct DensityAndMomentumDensity<MRTLatticeModelAvx> {
  template <typename FieldPtrOrIterator>
  ...
  template <typename PdfField_T>
  static real_t get(Vector3<real_t> &momentumDensity,
                    const MRTLatticeModelAvx &lm, const PdfField_T &pdf,
                    const cell_idx_t x, const cell_idx_t y,
                    const cell_idx_t z) {
    const real_t &xyz0 = pdf(x, y, z, 0);
    const real_t f_0 = pdf.getF(&xyz0, 0);
...
template <> struct MomentumDensity<MRTLatticeModelAvx> {
  template <typename FieldPtrOrIterator>
  static void get(Vector3<real_t> &momentumDensity,
                  const MRTLatticeModelAvx &lm, const FieldPtrOrIterator &it) {
    const auto x = it.x();
    const auto y = it.y();
    const auto z = it.z();
...
template <> struct PressureTensor<MRTLatticeModelAvx> {
  template <typename FieldPtrOrIterator>
  static void get(Matrix3<real_t> & /* pressureTensor */,
                  const MRTLatticeModelAvx & /* latticeModel */,
                  const FieldPtrOrIterator & /* it */) {
...
template <> struct ShearRate<MRTLatticeModelAvx> {
  template <typename FieldPtrOrIterator>
  static inline real_t get(const MRTLatticeModelAvx & /* latticeModel */,
                           const FieldPtrOrIterator & /* it */,
                           const Vector3<real_t> & /* velocity */,
                           const real_t /* rho */) {
    WALBERLA_ABORT("Not implemented");
  ...
  get(const MRTLatticeModelAvx &latticeModel, const PdfField_T & /* pdf */,
      const cell_idx_t /* x */, const cell_idx_t /* y */,
