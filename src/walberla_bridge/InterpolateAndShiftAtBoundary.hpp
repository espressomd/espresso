...
private:
  void kernel(IBlock *block, stencil::Direction slab_dir) {
    ...
    auto const weight = std::abs(std::fmod(get_pos_offset() + length, FloatType{1}));
    ...
    for (auto cell = ci.begin(); cell != ci.end(); ++cell) {
      Cell source1 = *cell;
      Cell source2 = *cell;
      source1[dir] = int(std::floor(source1[dir] + offset)) % dim;
      source1[dir] = int(source1[dir] + length);
      source1[dir] = cell_idx_c(int(source1[dir] % dim));

      source2[dir] = int(std::ceil(source2[dir] + offset)) % dim;
      source2[dir] = int(source2[dir] + length);
      source2[dir] = cell_idx_c(int(source2[dir] % dim));
