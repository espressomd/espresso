...
void mpi_send_v_slave(int pnode, int part) {
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(p->e->m.v.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_mu_E_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(p->e->p.mu_E.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_solvation(int pnode, int part, double *solvation) {
  ...
  if (pnode == this_node) {
    ...
    for (int ii = 0; ii < 2 * LB_COMPONENTS; ii++)
      p->e->p.solvation[ii] = solvation[ii];
  } else {
...
void mpi_send_solvation_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(&p->e->p.solvation, 2 * LB_COMPONENTS, MPI_DOUBLE, 0, SOME_TAG,
...
void mpi_send_mass_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(&p->e->p.mass, 1, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_rotational_inertia(int pnode, int part, double rinertia[3]) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    p->e->p.rinertia[0] = rinertia[0];
    p->e->p.rinertia[1] = rinertia[1];
    p->e->p.rinertia[2] = rinertia[2];
  } else {
...
void mpi_send_rotational_inertia_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(p->e->p.rinertia.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_affinity_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(p->e->p.bond_site.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_affinity(int pnode, int part, double bond_site[3]) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    p->e->p.bond_site[0] = bond_site[0];
    p->e->p.bond_site[1] = bond_site[1];
    p->e->p.bond_site[2] = bond_site[2];
  } else {
...
void mpi_send_out_direction(int pnode, int part, double out_direction[3]) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    p->e->p.out_direction[0] = out_direction[0];
    p->e->p.out_direction[1] = out_direction[1];
    p->e->p.out_direction[2] = out_direction[2];
  } else {
...
void mpi_send_out_direction_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(p->e->p.out_direction.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_mol_id_slave(int pnode, int part) {
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(&p->e->p.mol_id, 1, MPI_INT, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_omega(int pnode, int part, double omega[3]) {
  ...
  if (pnode == this_node) {
    ...
    p->e->m.omega[0] = omega[0];
    p->e->m.omega[1] = omega[1];
    p->e->m.omega[2] = omega[2];
  } else {
...
void mpi_send_omega_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(p->e->m.omega.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_dip(int pnode, int part, double dip[3]) {
  ...
  if (pnode == this_node) {
    ...
    p->r.dip[2] = dip[2];
#ifdef ROTATION
    convert_dip_to_quat(p->r.dip, p->r.quat, &p->e->p.dipm);
    convert_quat_to_quatu(p->r.quat, p->r.quatu);
#else
    p->e->p.dipm = sqrt(p->r.dip[0] * p->r.dip[0] + p->r.dip[1] * p->r.dip[1] +
                     p->r.dip[2] * p->r.dip[2]);
#endif
  } else {
...
void mpi_send_dipm_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(&p->e->p.dipm, 1, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
#ifdef ROTATION
    convert_quatu_to_dip(p->r.quatu, p->e->p.dipm, p->r.dip);
#endif
...
void mpi_send_virtual_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(&(p->e->p.is_virtual), 1, MPI_INT, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_vs_quat_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(p->e->p.vs_quat, 4, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
  }
...
void mpi_send_vs_relative(int pnode, int part, int vs_relative_to,
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    p->e->p.vs_relative_to_particle_id = vs_relative_to;
    p->e->p.vs_relative_distance = vs_distance;
    for (int i = 0; i < 4; i++) {
      p->e->p.vs_relative_rel_orientation[i] = rel_ori[i];
    }
  } else {
...
void mpi_send_vs_relative_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Recv(&p->e->p.vs_relative_to_particle_id, 1, MPI_INT, 0, SOME_TAG,
             comm_cart, MPI_STATUS_IGNORE);
    MPI_Recv(&p->e->p.vs_relative_distance, 1, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
             MPI_STATUS_IGNORE);
    MPI_Recv(p->e->p.vs_relative_rel_orientation, 4, MPI_DOUBLE, 0, SOME_TAG,
             comm_cart, MPI_STATUS_IGNORE);
  }
...
void mpi_send_rotation_slave(int pnode, int part) {
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    MPI_Status status;
    MPI_Recv(&p->e->p.rotation, 1, MPI_SHORT, 0, SOME_TAG, MPI_COMM_WORLD,
             &status);
  }
...
void mpi_send_ext_torque(int pnode, int part, int flag, int mask,
  ...
  if (pnode == this_node) {
    ...
    if (mask & PARTICLE_EXT_TORQUE)
      p->e->p.ext_torque = {torque[0], torque[1], torque[2]};
  } else {
    int s_buf[2];
    s_buf[0] = flag;
    s_buf[1] = mask;
...
void mpi_send_ext_torque_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    ...
    /* mask out old flags */
    p->e->p.ext_flag &= ~s_buf[1];
    /* set new values */
    p->e->p.ext_flag |= s_buf[0];

    if (s_buf[1] & PARTICLE_EXT_TORQUE)
      MPI_Recv(p->e->p.ext_torque.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
               MPI_STATUS_IGNORE);
  }
...
void mpi_send_ext_force(int pnode, int part, int flag, int mask,
  ...
  if (pnode == this_node) {
    ...
    if (mask & PARTICLE_EXT_FORCE)
      p->e->p.ext_force = {force[0], force[1], force[2]};
  } else {
    int s_buf[2];
    s_buf[0] = flag;
    s_buf[1] = mask;
...
void mpi_send_ext_force_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    ...
    if (s_buf[1] & PARTICLE_EXT_FORCE)
      MPI_Recv(p->e->p.ext_force.data(), 3, MPI_DOUBLE, 0, SOME_TAG, comm_cart,
               MPI_STATUS_IGNORE);
  }
...
void mpi_set_particle_gamma_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    comm_cart.recv(0, SOME_TAG, p->e->p.gamma);
  }
...
void mpi_set_particle_gamma_rot_slave(int pnode, int part) {
  ...
  if (pnode == this_node) {
    Particle *p = local_particles[part];
    comm_cart.recv(0, SOME_TAG, p->e->p.gamma_rot);
  }
