    ...
    for (int j = 0; j < 3; j++) {
      ...
      if (!(p.e->p.ext_flag & COORD_FIXED(j))) {
        ...
        if (integ_switch == INTEG_METHOD_NPT_ISO &&
          ...
          nptiso.p_vel[j] += Utils::sqr(p.e->m.v[j] * time_step) * p.e->p.mass;
          p.e->m.v[j] += 0.5 * time_step / p.e->p.mass * p.f.f[j] +
                      friction_therm0_nptiso(p.e->m.v[j]) / p.e->p.mass;
        } else
#endif
          /* Propagate velocity: v(t+dt) = v(t+0.5*dt) + 0.5*dt * a(t+dt) */
          p.e->m.v[j] += 0.5 * time_step * p.f.f[j] / p.e->p.mass;
#ifdef EXTERNAL_FORCES
      }
#endif
    }
      ...
      for (int j = 0; j < 3; j++) {
        ...
        if (!(p.e->p.ext_flag & COORD_FIXED(j))) {
          ...
          if (nptiso.geometry & nptiso.nptgeom_dir[j]) {
            {
              p.r.p[j] = scal[1] * (p.r.p[j] + scal[2] * p.e->m.v[j] * time_step);
              p.e->l.p_old[j] *= scal[1];
              p.e->m.v[j] *= scal[0];
            }
          } else {
            p.r.p[j] += p.e->m.v[j] * time_step;
          }
#ifdef EXTERNAL_FORCES
        }
#endif
...
// Don't propagate translational degrees of freedom of vs
    ...
    for (int j = 0; j < 3; j++) {
...
#endif
      {
        ...
        if (integ_switch == INTEG_METHOD_NPT_ISO &&
            (nptiso.geometry & nptiso.nptgeom_dir[j])) {
          p.e->m.v[j] += p.f.f[j] * 0.5 * time_step / p.e->p.mass +
                      friction_therm0_nptiso(p.e->m.v[j]) / p.e->p.mass;
          nptiso.p_vel[j] += Utils::sqr(p.e->m.v[j] * time_step) * p.e->p.mass;
        } else
#endif
    ...
    for (auto &p : local_cells.particles()) {
      ...
      for (int j = 0; j < 3; j++) {
...
#endif
        {
           ...
           * v(t+0.5*dt) */
          p.r.p[j] += time_step * p.e->m.v[j];
        }
      }
      /* Verlet criterion check */
      if (distance2(p.r.p, p.e->l.p_old) > skin2)
        set_resort_particles(Cells::RESORT_LOCAL);
    }
  }
...
// Don't propagate translational degrees of freedom of vs
    ...
    /* Verlet criterion check*/
    if (Utils::sqr(p.r.p[0] - p.e->l.p_old[0]) +
            Utils::sqr(p.r.p[1] - p.e->l.p_old[1]) +
            Utils::sqr(p.r.p[2] - p.e->l.p_old[2]) >
        skin2)
      set_resort_particles(Cells::RESORT_LOCAL);
  }
...
void force_and_velocity_display() {
  ...
  if (db_max_vel > skin2)
    fprintf(stderr, "%d: max_vel=%e, part=%d v=(%e,%e,%e)\n", this_node,
            sqrt(db_max_vel), db_maxv_id, local_particles[db_maxv_id]->e->m.v[0],
            local_particles[db_maxv_id]->e->m.v[1],
            local_particles[db_maxv_id]->e->m.v[2]);
#endif
}
