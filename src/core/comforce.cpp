...
int comforce_set_params(int part_type_a, int part_type_b, int flag, int dir,
                        double force, double fratio) {
    ...
    for (int t1 = t0 + 1; t1 < n_particle_types; t1++) {
      ...
      if (ia_params->COMFORCE_flag == 1) {
        ...
        if (ia_params->COMFORCE_dir == 1) {
          ...
          /*By doing two vector products find radial axis along the target
           * system */
