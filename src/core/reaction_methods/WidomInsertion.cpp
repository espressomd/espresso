...
std::pair<double, double> WidomInsertion::measure_excess_chemical_potential(
  ...
  std::pair<double, double> result = std::make_pair(
      -kT * log(current_reaction.accumulator_potential_energy_difference_exponential.mean()[0]),
      std::abs(-kT /
               current_reaction.accumulator_potential_energy_difference_exponential.mean()[0] *
               current_reaction.accumulator_potential_energy_difference_exponential.std_error()[0]));
  return result;
}
