  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 3; ++k) {
        ...
        auto const f_expr = calculate_factorial_expression_cpH(reaction, p_numbers);
	// acceptance = f_expr * exp(- E / T + nu_bar * log(10) * (pH - nu_bar * pKa))
        auto const acceptance_ref = f_expr *
