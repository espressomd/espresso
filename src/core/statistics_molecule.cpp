...
int analyze_fold_molecules(PartCfg&partCfg, float *coord, double shift[3]) {
  ...
  if (lees_edwards_offset != 0.0) {
    fprintf(stderr,
            "Error: Folding molecules not supported under Lees-Edwards.\n");
    errexit();
  }
#endif
      ...
      for (i = 0; i < 3; i++) {
        if (PERIODIC(i)) {
          ...
          if (cm_tmp < -10e-6 || cm_tmp > box_l[i] + 10e-6) {
            runtimeErrorMsg()
                << "analyze_fold_molecules: chain center of mass is out of "
                   "range (coord "
                << i << ": " << cm_tmp << " not in box_l " << box_l[i] << ")";
            return ES_ERROR;
          }
        }
      }
...
void calc_mol_center_of_mass(PartCfg &partCfg, Molecule mol, double com[3]) {
  ...
  for (i = 0; i < mol.part.n; i++) {
    id = mol.part.e[i];
    for (j = 0; j < 3; j++)
      com[j] += partCfg[id].r.p[j] * (partCfg[id]).p.mass;
    M += (partCfg[id]).p.mass;
  }
  for (j = 0; j < 3; j++)
    com[j] /= M;
