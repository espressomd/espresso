...
void check_particle_consistency() {
  ...
  for (c = 0; c < local_cells.n; c++) {
    ...
    for (int n = 0; n < cell->n; n++) {
      if (part[n].e->p.identity < 0 || part[n].e->p.identity > max_seen_particle) {
        fprintf(stderr,
                "%d: check_particle_consistency: ERROR: Cell %d Part "
                "%d has corrupted id=%d\n",
                this_node, c, n, cell->part[n].e->p.identity);
        errexit();
      }
      for (dir = 0; dir < 3; dir++) {
        if (PERIODIC(dir) &&
          ...
          fprintf(stderr,
                  ...
                  this_node, dir, part[n].r.p[dir], n, part[n].e->p.identity, c);
          errexit();
        }
      }
      if (local_particles[part[n].e->p.identity] != &part[n]) {
        fprintf(stderr,
                "%d: check_particle_consistency: ERROR: address "
                "mismatch for part id %d: local: %p cell: %p in cell "
                "%d\n",
                this_node, part[n].e->p.identity,
                static_cast<void *>(local_particles[part[n].e->p.identity]),
  ...
  for (n = 0; n < max_seen_particle + 1; n++) {
    if (local_particles[n] != nullptr) {
      local_part_cnt++;
      if (local_particles[n]->e->p.identity != n) {
        fprintf(stderr,
                "%d: check_particle_consistency: ERROR: "
                "local_particles part %d has corrupted id %d\n",
                this_node, n, local_particles[n]->e->p.identity);
        errexit();
      }
    }
  }
    ...
    for (c = 0; c < local_cells.n; c++) {
      for (int p = 0; p < local_cells.cell[c]->n; p++)
        fprintf(stderr, "%d: got particle %d in cell %d\n", this_node,
                local_cells.cell[c]->part[p].e->p.identity, c);
    }
