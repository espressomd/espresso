...
typedef struct collective_variable{
    ...
    double CV_maximum;
    double delta_CV;
    double (*determine_current_state_in_collective_variable_with_index) (int, void* m_wang_landau_system);	//declare a function pointer with name determine_current_state_in_this_collective_variable that has to point to a function that has 1 int input parameter (for the collective_variable_index) and another input for a void pointer and returns a double
...
double calculate_degree_of_association(int index_of_current_collective_variable, void* m_wang_landau_system);
...
public:
    ...
    void add_reaction(double equilibrium_constant, std::vector<int> _reactant_types, std::vector<int> _reactant_coefficients, std::vector<int> _product_types, std::vector<int> _product_coefficients);
    ///////////////////////////////////////////// Wang-Landau algorithm

    wang_landau_system m_current_wang_landau_system={.histogram=NULL,.len_histogram=0 , \
    ...
    void add_new_CV_degree_of_association(int associated_type, double CV_minimum, double CV_maximum, std::vector<int> _corresponding_acid_types);
...
private:
    ...
    int m_invalid_charge=-10000;//this is the default charge which is assigned to a type which occurs in a reaction. this charge has to be overwritten. if it is not overwritten the reaction ensemble will complain.
    enum m_reaction_mode { reaction_ensemble_mode, reaction_ensemble_wang_landau_mode, constant_pH_mode };
