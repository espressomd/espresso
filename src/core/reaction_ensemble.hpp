...
struct energy_collective_variable : public collective_variable {
  ...
  void load_CV_boundaries(wang_landau_system &m_current_wang_landau_system);
};
...
public:
  ...
  wang_landau_system m_current_wang_landau_system = {
      ...
      std::vector<
          std::shared_ptr<collective_variable>>(), // collective_variables
      std::vector<int>(), // nr_subindices_of_collective_variable
...
private:
  ...
  /**
  *Calculates the average of an integer array (used for the histogram of the
