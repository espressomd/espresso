...
struct reaction_system {
  ...
  double exclusion_radius=0.0; // this is used as a kind of hard sphere radius, if
...
struct energy_collective_variable : public collective_variable {
  ...
  void load_CV_boundaries(WangLandauSystem &m_current_wang_landau_system);
};

struct degree_of_association_collective_variable : public collective_variable {
...
struct WangLandauSystem {
  ...
  std::vector<double>
      wang_landau_potential=std::vector<double>(); // equals the logarithm to basis e of the
                             // degeneracy of the states
  std::vector<std::shared_ptr<collective_variable>> collective_variables=std::vector<
          std::shared_ptr<collective_variable>>();
  ...
  std::vector<double>
      minimum_energies_at_flat_index=std::vector<double>(); // only present in energy preparation run
  std::vector<double>
      maximum_energies_at_flat_index=std::vector<double>(); // only present in energy preparation run
  ...
  private:
      ...
      double calculate_delta_degree_of_association(
          degree_of_association_collective_variable &current_collective_variable);
...
public:
  ...
  virtual void on_reaction_entry(WangLandauSystem& m_current_wang_landau_system, int& old_state_index){}
  virtual void on_reaction_rejection_directly_after_entry(WangLandauSystem& m_current_wang_landau_system, int& old_state_index){}
  virtual void on_attempted_reaction(WangLandauSystem& m_current_wang_landau_system, int& new_state_index){}
  virtual void on_end_reaction(WangLandauSystem& m_current_wang_landau_system, int& accepted_state){}
  ...
  virtual double calculate_boltzmann_factor(
      ...
      int new_state_index, bool only_make_configuration_changing_move){return -10;};
  ...
  virtual void on_mc_rejection_directly_after_entry(WangLandauSystem& m_current_wang_landau_system, int& old_state_index){};
  virtual void on_mc_accept(WangLandauSystem& m_current_wang_landau_system, int& new_state_index){};
  virtual void on_mc_reject(WangLandauSystem& m_current_wang_landau_system, int& old_state_index){};
  virtual int on_mc_use_WL_get_new_state(WangLandauSystem& m_current_wang_landau_system){return -10;};
  
  ///////////////////////////////////////////// Wang-Landau algorithm
  WangLandauSystem m_current_wang_landau_system;
...
class ReactionEnsemble: public ReactionAlgorithm{
    public:
        double calculate_boltzmann_factor(single_reaction& current_reaction, double E_pot_old, double E_pot_new, std::vector<int>& old_particle_numbers, int dummy_old_state_index, int dummy_new_state_index, bool dummy_only_make_configuration_changing_move);
};

class WangLandauReactionEnsemble: public ReactionAlgorithm {
  public:
    void on_reaction_entry(WangLandauSystem& m_current_wang_landau_system, int& old_state_index);
    void on_reaction_rejection_directly_after_entry(WangLandauSystem& m_current_wang_landau_system, int& old_state_index);
    void on_attempted_reaction(WangLandauSystem& m_current_wang_landau_system, int& new_state_index);
    void on_end_reaction(WangLandauSystem& m_current_wang_landau_system, int& accepted_state);
    double calculate_boltzmann_factor(single_reaction& current_reaction, double E_pot_old, double E_pot_new, std::vector<int>& old_particle_numbers, int old_state_index, int new_state_index, bool only_make_configuration_changing_move);
    int do_reaction(int reaction_steps);
    void on_mc_rejection_directly_after_entry(WangLandauSystem& m_current_wang_landau_system, int& old_state_index);
    void on_mc_accept(WangLandauSystem& m_current_wang_landau_system, int& new_state_index);
    void on_mc_reject(WangLandauSystem& m_current_wang_landau_system, int& old_state_index);
    int on_mc_use_WL_get_new_state(WangLandauSystem& m_current_wang_landau_system);
...
class ConstantpHEnsemble: public ReactionAlgorithm{
    public:
        ...
        double calculate_boltzmann_factor(single_reaction& current_reaction, double E_pot_old, double E_pot_new, std::vector<int>& dummy_old_particle_numbers, int dummy_old_state_index, int dummy_new_state_index, bool dummy_only_make_configuration_changing_move);
    private:
        int get_random_p_id();
        int do_reaction(int reaction_steps);
};
...
double calculate_factorial_expression(single_reaction &current_reaction,
                                        int *old_particle_numbers, reaction_system& m_current_reaction_system);
