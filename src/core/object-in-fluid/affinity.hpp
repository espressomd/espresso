...
inline void add_affinity_pair_force(Particle *p1, Particle *p2,
  ...
  if (aff_type_extracted == 1) {
    ...
    * I have the position of the particle - p1, and under p1->p.bond_site I have
    *the coordinate of the bond_site.
    * Also, under d[3] I have the vector towards the constraint meaning that
    *force on p1 should be in the direction of d[3].
    *
    * Algorithm:
    ...
    * 4. If no, I maintain the creation of a bond. First I check whether I am in
    *the area of possible bond creation: ?dist < affinity_r0?
    ...
    *length and relaxed bond length
    * 	bond is always created, no probability is involved
    * 	if bondlength reaches maxBond, the bond imediately ruptures. No
    ...
    if ((dist < ia_params->affinity_cut)) { // Checking whether I am inside the
      ...
      if (dist > 0.0) {
        ...
        {                              // Bond exists
          ...
          for (j = 0; j < 3; j++)
            vec[j] = p1->p.bond_site[j] -
                     unfolded_pos[j]; // Shouldn't be the vec vector normalized?
                                      // Yes, but with affinity_r0 and not by
                                      // len!!!
          len2 = sqrlen(vec);
          len = sqrt(len2);
        ...
        } else if (dist < ia_params->affinity_r0) { // Bond does not exist, we
                                                    // are inside of possible
                                                    // bond creation area, lets
  ...
  if (aff_type_extracted == 2) { // second implementation of affinity
    ...
    *the bond
    * 	Koff depends on the bondlenth via Koff = K0*exp(F/Fd) =
    *K0*exp(kappa(r-r0)/Fd)
    * 	here, ia_params->Koff gives us K_0, off rate when bond is relaxed.
    * 	here, maxBond is used as detachment force F_d. The original check for
    *ensuring, that partice flows out of the cut-off radius and the bond remains
    *active is replaced with fixed check, that bond length must not be greater
    *that 0.8 cut_off
    *********************/
    int j;
    double fac = 0.0;
    if ((dist < ia_params->affinity_cut)) { // Checking whether I am inside the
      ...
      if (dist > 0.0) {
        ...
        {                              // Bond exists
          ...
          // printf("%f ", Poff);
          if (len < 0.8 * ia_params->affinity_cut) { // in other implementation,
                                                     // maxBond is used here.
                                                     // However, in this
                                                     // implementation, we need
                                                     // maxBond for setting
                                                     // detachment force F_d
          ...
          if (period_for_output > 0)
            if (((int)floor(sim_time / time_step) % period_for_output == 0) &&
              ...
              fprintf(fp, "Pon %f, Kon %f, particle %d, Poff = %f, F = %f, "
                          "Koff = %f, K0 = %f, len = %f \n",
  ...
  if (aff_type_extracted == 3) {
    ...
    *length and relaxed bond length
    * 	bond is created with probability 1-exp(-Kon*timestep)
    * 	bond is ruptured with probability 1-exp(-Koff*timestep) to brake the
    *bond
    * 	Koff is given as parameter, is not dependent on the force nor the bond
    *length
    * 	here, maxBond stands for ensuring, that partice flows out of the
    *cut-off radius and the bond remains active. maxBond should be always less
    *than cut_off radius
    *********************/
    int j;
    double fac = 0.0;
  ...
  if (aff_type_extracted == 4) {
    ...
    * 	maxBond is not used, we use probability 1-exp(-Koff*timestep) to brake
    *the bond
    * 	Koff depends on the bondlength via Koff = K0*exp(F/Fd) =
    *K0*exp(kappa*r/Fd)
    * 	here, ia_params->Koff gives us K_0, off rate when bond is relaxed.
    * 	here, maxBond is used as detachment force F_d. The original check for
    *ensuring, that partice flows out of the cut-off radius and the bond remains
  ...
  if (aff_type_extracted == 5) { // second implementation of affinity
    ...
    * comments:
    * 	strength of the force is proportiona to the difference of actual bond
    *length and 75% of the relaxed bond length
    * 	bond is created with probability 1-exp(-Kon*timestep)
    * 	maxBond is not used, we use probability 1-exp(-Koff*timestep) to brake
    *the bond
    * 	Koff depends on the bondlenth via Koff = K0*exp(F/Fd) =
  ...
  if (aff_type_extracted == 6) { // second implementation of affinity
    ...
    * comments:
    * 	strength of the force is proportiona to the difference of actual bond
    *length and the relaxed bond length
    * 	bond is created with probability 1-exp(-Kon*timestep)
    * 	maxBond is not used, we use probability 1-exp(-Koff*timestep) to brake
    *the bond
    * 	Koff depends on the bondlenth via Koff = K0*exp(F/Fd) =
