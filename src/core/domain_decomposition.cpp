...
void dd_mark_cells() {
  ...
  int dd_fill_comm_cell_lists(Cell * *part_lists, int const lc[3],
    ...
    void dd_prepare_comm(GhostCommunicator * comm, int data_parts,
      ...
      CELL_TRACE(fprintf(
          stderr, "%d Create Communicator: prep_comm data_parts %d num %d\n",
        ...
        for (lr = 0; lr < 2; lr++) {
          if (grid[dir] == 1) {
            ...
            if (PERIODIC(dir) || (boundary[2 * dir + lr] == 0)) {
              ...
              CELL_TRACE(fprintf(stderr,
                                 "%d: prep_comm %d copy to          grid "
                                 "(%d,%d,%d)-(%d,%d,%d)\n",
                                 this_node, cnt, lc[0], lc[1], lc[2], hc[0],
                                 hc[1], hc[2]));
              ...
              if (PERIODIC(dir) || (boundary[2 * dir + lr] == 0)) {
                if ((node_pos[dir] + i) % 2 == 0) {
                  ...
                  CELL_TRACE(fprintf(stderr,
                                     ...
                                     "grid (%d,%d,%d)-(%d,%d,%d)\n",
                                     this_node, cnt, comm->comm[cnt].node,
    ...
    /** Of every two communication rounds, set the first receivers to prefetch
     * and poststore
     */
      ...
      for (o = 1; o < dd.cell_grid[2] + 1; o++) {
        ...
        Cell *dd_save_position_to_cell(const Vector3d &pos) {
          ...
          for (int i = 0; i < 3; i++) {
            ...
            cpos[i] =
                static_cast<int>(std::floor(lpos * dd.inv_cell_size[i])) + 1;

            /* particles outside our box. Still take them if
               nonperiodic boundary. We also accept the particle if we are at
               the box boundary, and the particle is within the box. In this
               case the particle belongs here and could otherwise potentially be
               dismissed due to rouding errors. */
            if (cpos[i] < 1) {
              ...
              if ((!PERIODIC(i) or (pos[i] < box_l[i])) &&
                  boundary[2 * i + 1]) {
          ...
          auto const ind =
              get_linear_index(cpos[0], cpos[1], cpos[2],
                               {dd.ghost_cell_grid[0], dd.ghost_cell_grid[1],
                                dd.ghost_cell_grid[2]});
          return &(cells[ind]);
        }
        ...
        void dd_on_geometry_change(int flags, const Vector3i &grid) {
          ...
          for (int i = 0; i < 3; i++) {
            if (local_box_l[i] < max_range) {
              runtimeErrorMsg()
                  << "box_l in direction " << i << " is too small";
            }
          }
          ...
          double min_cell_size = std::min(
              std::min(dd.cell_size[0], dd.cell_size[1]), dd.cell_size[2]);
        ...
        void dd_topology_init(CellPList * old, const Vector3i &grid) {
          ...
          CELL_TRACE(fprintf(
              stderr, "%d: dd_topology_init: Number of recieved cells=%d\n",
              this_node, old->n));

          /* Min num cells can not be smaller than calc_processor_min_num_cells,
             but may be set to a larger value by the user for performance
             reasons. */
          min_num_cells =
          ...
          dd_prepare_comm(&cell_structure.update_ghost_pos_comm, update_data,
                          grid);
          dd_prepare_comm(&cell_structure.collect_ghost_force_comm,
                          GHOSTTRANS_FORCE, grid);
        ...
        namespace {
        /**
         * @brief Move particles into the cell system if it belongs to this
         * node.
         *
        ...
        void exchange_neighbors(ParticleList *pl, const Vector3i &grid) {
          for (int dir = 0; dir < 3; dir++) {
            ...
            if (grid[dir] == 1) {
              ...
              Utils::Mpi::sendrecv(comm_cart, node_neighbors[2 * dir], 0,
                                   send_buf, node_neighbors[2 * dir], 0,
                                   recv_buf);
              ...
              auto req_r = isendrecv(comm_cart, node_neighbors[2 * dir + 1], 0,
                                     send_buf_r, node_neighbors[2 * dir + 1], 0,
                                     recv_buf_r);

              std::array<request, 4> reqs{
                  {req_l[0], req_l[1], req_r[0], req_r[1]}};
        ...
        int calc_processor_min_num_cells(const Vector3i &grid) {
          int i, min = 1;
          /* the minimal number of cells can be lower if there are at least two
             nodes serving a direction, since this also ensures that the cell
             size is at most half the box length. However, if there is only one
             processor for a direction, there have to be at least two cells for
             this direction. */
          for (i = 0; i < 3; i++) {
            if (grid[i] == 1) {
              min *= 2;
