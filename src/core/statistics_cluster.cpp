...
int test_mesh_elements(PartCfg &partCfg, double pos[3], int probe_part_type) {
  ...
  for (auto &p : partCfg) {
    IA_parameters *ia_params = get_ia_param(p.type, probe_part_type);
...
void create_free_volume_grid(PartCfg &partCfg, IntList mesh, int dim[3],
                             int probe_part_type) {
...
int cluster_free_volume_grid(IntList mesh, int dim[3], int ***holes) {
  ...
  while (i < (dim[0] * dim[1] * dim[2])) {
    ...
    if (mesh.e[i] == -2 || mesh.e[i] >= 0) {
      ...
      n++;
      mesh.e[i] = n;
      // start new cluster, put mesh point as first element, and set list
      ...
      while (li < sizes[n]) {
        ...
        for (k = 0; k < 6; k++) {
          ...
          if (mesh.e[neighbors[k]] > 0 && mesh.e[neighbors[k]] < n) {
            fprintf(stderr,
                    "cfvg:error: i=%d, li=%d, n=%d, mesh.e[x]=%d, x=%d\n", i,
                    li, n, mesh.e[neighbors[k]], neighbors[k]);
            fflush(stderr);
          }
        }
        li++;
