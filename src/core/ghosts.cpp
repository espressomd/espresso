...
static void ghost_communicator_async(GhostCommunicator *gc) {
  ...
  for (int i = 0; i < gc->num; i++) {
    ...
    if (comm_type == GHOST_RECV) {
      ...
      MPI_Irecv(commbufs[i], commbufs[i].size(), MPI_BYTE, gcn->node, gcn->tag,
                comm_cart, &reqs[i]);
  ...
  for (int i = 0; i < gc->num; i++) {
    ...
    if (comm_type == GHOST_SEND) {
      ...
      MPI_Isend(commbufs[i], commbufs[i].size(), MPI_BYTE, gcn->node, gcn->tag,
                comm_cart, &reqs[i]);

      // MPI guarantees ordered communication for the same pair of (receiver,
      // tag)
      std::vector<int> &bbuf = commbufs[i].bondbuf();
      if (bbuf.size() > 0)
        MPI_Isend(bbuf.data(), bbuf.size(), MPI_INT, gcn->node, gcn->tag,
                  comm_cart, &reqs[gc->num + i]);
    }
  }
  ...
  while (true) {
    ...
    // requests
    MPI_Waitsome(gc->num, reqs.data(), &outcount, idxs.data(),
                 MPI_STATUS_IGNORE);
      ...
      if (comm_type == GHOST_RECV) {
        if (data_parts == GHOSTTRANS_FORCE /*&& comm_type != GHOST_RDCE*/) {
          ...
          if (buf.bondbuf().size() != n_bonds && n_bonds > 0) {
            ...
            // Post the Irecv for the bonds replacing(!) the MPI_Request of the
            // particles
            MPI_Irecv(buf.bondbuf().data(), n_bonds, MPI_INT, gcn->node,
                      gcn->tag, comm_cart, &reqs[gcnr]);
          } else {
  ...
  for (n = 0; n < gc->num; n++) {
    ...
    if (comm_type == GHOST_LOCL)
      ...
      case GHOST_RECV: {
        ...
        if (data_parts & GHOSTTRANS_PROPRTS) {
          int n_bonds =
              *(int *)((char *)r_buffer + r_buffer.size() - sizeof(int));
          GHOST_TRACE(fprintf(stderr,
                              "%d: ghost_comm receive from %d (%d bonds)\n",
                              this_node, node, n_bonds));
          if (n_bonds) {
            r_buffer.bondbuf().resize(n_bonds);
            MPI_Recv(&r_buffer.bondbuf()[0], n_bonds, MPI_INT, node,
                     REQ_GHOST_SEND, comm_cart, &status);
          }
        }
        break;
      }
      case GHOST_SEND: {
        ...
        GHOST_TRACE(fprintf(stderr, "%d: ghost_comm send to %d (%d ints)\n",
                            this_node, node, n_bonds));
        if (n_bonds) {
          MPI_Send(&s_buffer.bondbuf()[0], n_bonds, MPI_INT, node,
                   REQ_GHOST_SEND, comm_cart);
        }
        break;
      }
      case GHOST_BCST:
        GHOST_TRACE(fprintf(
            stderr, "%d: ghost_comm bcast from %d (%d bytes)\n", this_node,
            node, (node == this_node) ? sbuffer.size() : r_buffer.size()));
        if (node == this_node) {
          ...
          if (n_bonds) {
            MPI_Bcast(&s_buffer.bondbuf()[0], n_bonds, MPI_INT, node,
                      comm_cart);
          }
        } else {
          ...
          if (data_parts & GHOSTTRANS_PROPRTS) {
            ...
            if (n_bonds) {
              r_buffer.bondbuf().resize(n_bonds);
              MPI_Bcast(&r_buffer.bondbuf()[0], n_bonds, MPI_INT, node,
                        comm_cart);
            }
          }
        }
      ...
      case GHOST_RDCE:
        GHOST_TRACE(fprintf(stderr, "%d: ghost_comm reduce to %d (%d bytes)\n",
                            this_node, node, s_buffer.size()));
        if (node == this_node)
          MPI_Reduce(s_buffer, r_buffer, s_buffer.size(), MPI_BYTE,
                     MPI_FORCES_SUM, node, comm_cart);
        else
          MPI_Reduce(s_buffer, NULL, s_buffer.size(), MPI_BYTE, MPI_FORCES_SUM,
                     node, comm_cart);
        break;
      }
