...
int local_vs_relate_to(int part_num, int relate_to) {
  ...
  if ((p_current == NULL) || (p_relate_to == NULL)) {
    runtimeErrorMsg() << "Could not retrieve particle data for the given ids "
                         "from local_particles[p[]";
    return ES_ERROR;
  }
  ...
  // Set the particle id of the particle we want to relate to, the distance
  // and the relative orientation
  p_current->e->p.vs_relative_to_particle_id = relate_to;
  p_current->e->p.vs_relative_distance = l;
  for (int i = 0; i < 4; i++)
    p_current->e->p.vs_relative_rel_orientation[i] = quat[i];
  return ES_OK;
}
