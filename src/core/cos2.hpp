...
inline void add_cos2_pair_force(Particle *p1, Particle *p2,
                                IA_parameters *ia_params, double d[3],
                                double dist, double force[3]) {
  if ((dist < ia_params->COS2_cut)) {
    double fac;
    if (dist > ia_params->COS2_offset) {
      fac = -ia_params->COS2_eps * M_PI / 2 / ia_params->COS2_w / dist *
            sin(M_PI * (dist - ia_params->COS2_offset) / ia_params->COS2_w);
    }
    for (int j = 0; j < 3; j++)
      force[j] += fac * d[j];
  }
...
inline double cos2_pair_energy(Particle *p1, Particle *p2,
                               IA_parameters *ia_params, double d[3],
                               double dist) {
  ...
  if ((dist < ia_params->COS2_cut)) {
    if (dist < ia_params->COS2_offset)
      ...
      return -ia_params->COS2_eps / 2 *
             (cos(M_PI * (dist - ia_params->COS2_offset) / ia_params->COS2_w) +
              1);
  }
  return 0.0;
}
