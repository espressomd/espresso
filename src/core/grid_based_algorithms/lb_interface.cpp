...
int lb_lbfluid_set_density(double *p_dens) {
  for (int ii = 0; ii < LB_COMPONENTS; ii++) {
    ...
    if (lattice_switch & LATTICE_LB_GPU) {
      ...
      on_lb_params_change(LBPAR_DENSITY);
#endif // LB_GPU
    } else {
#ifdef LB
      lbpar.rho = p_dens[ii];
...
int lb_lbfluid_set_visc(double *p_visc) {
  for (int ii = 0; ii < LB_COMPONENTS; ii++) {
    ...
    if (lattice_switch & LATTICE_LB_GPU) {
      ...
      on_lb_params_change(LBPAR_VISCOSITY);
#endif // LB_GPU
    } else {
#ifdef LB
...
int lb_lbfluid_set_bulk_visc(double *p_bulk_visc) {
  for (int ii = 0; ii < LB_COMPONENTS; ii++) {
    ...
    if (lattice_switch & LATTICE_LB_GPU) {
      ...
      on_lb_params_change(LBPAR_BULKVISC);
#endif // LB_GPU
    } else {
#ifdef LB
...
int lb_lbfluid_set_friction(double *p_friction) {
  for (int ii = 0; ii < LB_COMPONENTS; ii++) {
    ...
    if (lattice_switch & LATTICE_LB_GPU) {
      ...
      on_lb_params_change(LBPAR_FRICTION);
#endif // LB_GPU
    } else {
#ifdef LB
...
int lb_lbfluid_set_agrid(double p_agrid) {
  ...
  if (lattice_switch & LATTICE_LB_GPU) {
    ...
    on_lb_params_change(LBPAR_AGRID);
#endif // LB_GPU
  } else {
#ifdef LB
    lbpar.agrid = p_agrid;
...
int lb_lbfluid_set_shanchen_coupling(double *p_coupling) {
  ...
  on_lb_params_change(LBPAR_COUPLING);
#endif // LB_GPU
#ifdef LB
...
int lb_lbfluid_set_mobility(double *p_mobility) {
  ...
  for (ii = 0; ii < LB_COMPONENTS - 1; ii++) {
    ...
    if (lattice_switch & LATTICE_LB_GPU) {
      ...
      on_lb_params_change(LBPAR_MOBILITY);
#endif // LB_GPU
    } else {
#ifdef LB
#error not implemented
