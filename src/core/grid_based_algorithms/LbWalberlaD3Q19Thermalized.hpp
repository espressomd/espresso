...
namespace walberla {
class LbWalberlaD3Q19Thermalized
...
public:
  void construct_lattice_model(double viscosity) {
    ...
    m_lattice_model = std::make_shared<lbm::FluctuatingMRT_LatticeModel>(
        lbm::FluctuatingMRT_LatticeModel(m_last_applied_force_field_id, 
                                         omega_2,omega_2,omega_2,omega, 0,1E-12,0));
  ...
  LbWalberlaD3Q19Thermalized(double viscosity, double density, double agrid, double tau,
