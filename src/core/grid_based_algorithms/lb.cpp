...
inline void lb_viscous_coupling(Particle *p, double force[3]) {
  ...
  double velocity[3];
  velocity[0] = p->e->m.v[0];
  velocity[1] = p->e->m.v[1];
  velocity[2] = p->e->m.v[2];

  Vector3d v_drift = {interpolated_u[0], interpolated_u[1], interpolated_u[2]};
#ifdef ENGINE
  if (p->e->swim.swimming) {
    v_drift += p->e->swim.v_swim * p->r.quatu;
    p->e->swim.v_center[0] = interpolated_u[0];
    p->e->swim.v_center[1] = interpolated_u[1];
    p->e->swim.v_center[2] = interpolated_u[2];
  }
#endif
  ...
  force[0] = force[0] + p->e->lc.f_random[0];
  force[1] = force[1] + p->e->lc.f_random[1];
  force[2] = force[2] + p->e->lc.f_random[2];
  ...
  if (p->e->swim.swimming) {
    ...
    double direction = double(p->e->swim.push_pull) * p->e->swim.dipole_length;
    ...
    lb_lbfluid_get_interpolated_velocity(Vector3d(source_position),
                                         p->e->swim.v_source.data());

    // calculate and set force at source position
    delta_j[0] =
        -p->e->swim.f_swim * p->r.quatu[0] * time_step * lbpar.tau / lbpar.agrid;
    delta_j[1] =
        -p->e->swim.f_swim * p->r.quatu[1] * time_step * lbpar.tau / lbpar.agrid;
    delta_j[2] =
        -p->e->swim.f_swim * p->r.quatu[2] * time_step * lbpar.tau / lbpar.agrid;
...
#ifdef GAUSSRANDOM
        p.e->lc.f_random[0] = lb_coupl_pref2 * gaussian_random();
        p.e->lc.f_random[1] = lb_coupl_pref2 * gaussian_random();
        p.e->lc.f_random[2] = lb_coupl_pref2 * gaussian_random();
#elif defined(GAUSSRANDOMCUT)
        p.e->lc.f_random[0] = lb_coupl_pref2 * gaussian_random_cut();
        p.e->lc.f_random[1] = lb_coupl_pref2 * gaussian_random_cut();
        p.e->lc.f_random[2] = lb_coupl_pref2 * gaussian_random_cut();
#elif defined(FLATNOISE)
        p.e->lc.f_random[0] = lb_coupl_pref * (d_random() - 0.5);
        p.e->lc.f_random[1] = lb_coupl_pref * (d_random() - 0.5);
        p.e->lc.f_random[2] = lb_coupl_pref * (d_random() - 0.5);
      } else {
        p.e->lc.f_random = {0.0, 0.0, 0.0};
      }
#else // GAUSSRANDOM
#error No noise type defined for the CPU LB
#endif // GAUSSRANDOM
