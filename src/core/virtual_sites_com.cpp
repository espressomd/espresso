...
int calc_mol_pos_cfg(Particle *p_com, double r_com[3]) {
  ...
  if (count != topology[mol_id].part.n - 1) {
    fprintf(stderr,
            "There is more than one COM in calc_mol_pos_cfg! mol_id=%i\n",
            mol_id);
    return 0;
  }
#endif
...
double get_mol_dist(Particle *p1, Particle *p2) {
  ...
  if (p1_com == NULL) {
    runtimeErrorMsg()
        << "COM Particle not found for particle in get_mol_dist id= "
        << p1->p.identity << "\n";
...
void get_mol_dist_vector_from_molid_cfg(int mol_id1, int mol_id2,
                                        double dist[3]);
...
#endif
        calc_non_bonded_pair_force_from_partcfg_simple(p1, p2, vec12, dist,
                                                       dist2, force);
    }
  }
}
...
#ifdef VIRTUAL_SITES_DEBUG
  fprintf(stderr,
          "No com found in get_mol_com_particle_from_molid_cfg ! mol_id=%i\n",
          mol_id);
#endif
  return NULL;
}
...
void get_mol_dist_vector_from_molid_cfg(int mol_id1, int mol_id2,
  ...
  if (p1_com == NULL) {
    fprintf(
        stderr,
        "No com found in get_mol_dist_vector_from_molid_cfg for mol id=%i\n",
        mol_id1);
    dist[0] = dist[1] = dist[2] = 0.0;
    return;
  }
