...
public:
  ...
  void set_correlation_args(Utils::Vector3d const &args) { m_correlation_args = args; }
...
private:
  ...
  Utils::Vector3d m_correlation_args; // additional arguments, which the correlation
  ...
  using correlation_operation_type = std::vector<double> (*)(
      std::vector<double> const &, std::vector<double> const &, Utils::Vector3d);
