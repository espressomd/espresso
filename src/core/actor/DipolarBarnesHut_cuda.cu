...
void setBHPrecision(float *epssq, float *itolsq) {
  cuda_safe_mem(cudaMemcpyToSymbol(SYMBOL(epssqd), epssq, sizeof(float), 0,
                                   cudaMemcpyHostToDevice));
  cuda_safe_mem(cudaMemcpyToSymbol(SYMBOL(itolsqd), itolsq, sizeof(float),
...
void fillConstantPointers(float *r, float *dip, BHData bh_data) {
  cuda_safe_mem(cudaMemcpyToSymbol(SYMBOL(bhpara), &bh_data, sizeof(BHData),
                                   0, cudaMemcpyHostToDevice));
