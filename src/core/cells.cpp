...
void check_cells_consistency() {
  ...
  for (c = 0; c < local_cells.n; c++) {
    ...
    if ((index % sizeof(Cell)) != 0) {
      fprintf(stderr, "%d: local cell pointer not even aligned, certainly "
                      "wrong (local_cell[%d], index=%d).\n",
              this_node, c, index);
      errexit();
    }
    index /= sizeof(Cell);
    if (index < 0 || index >= n_cells) {
      fprintf(stderr, "%d: local cell pointer out of range, maybe old leftover "
                      "(local_cell[%d]).\n",
              this_node, c);
      errexit();
    }
    if (used.e[index]) {
      fprintf(stderr,
              "%d: local cell is already pointed to (local_cell[%d]).\n",
              this_node, c);
      errexit();
    }
    used.e[index] = 1;
  ...
  for (c = 0; c < ghost_cells.n; c++) {
    ...
    if ((index % sizeof(Cell)) != 0) {
      fprintf(stderr, "%d: ghost cell pointer not even aligned, certainly "
                      "wrong (ghost_cell[%d], index=%d).\n",
              this_node, c, index);
      errexit();
    }
    index /= sizeof(Cell);
    if (index < 0 || index >= n_cells) {
      fprintf(stderr, "%d: ghost cell pointer out of range, maybe old leftover "
                      "(ghost_cell[%d]).\n",
              this_node, c);
      errexit();
    }
    if (used.e[index]) {
      fprintf(stderr,
              "%d: ghost cell is already pointed to (ghost_cell[%d]).\n",
              this_node, c);
      errexit();
    }
    used.e[index] = 1;
...
void local_sort_particles() {
  ...
  for (int c = 0; c < local_cells.n; c++) {
    ...
    for (int id = 0; id < np; ++id) {
      ...
      if (tgt_cell != cell) {
        fprintf(stderr, "%d: particle %d at position %lf %lf %lf is not in its "
                        "expected cell. Have %ld, expected %ld\n",
                this_node, p[id].p.identity, p[id].r.p[0], p[id].r.p[1],
                p[id].r.p[2], (cell - cells) / sizeof(Cell *),
                (tgt_cell - cells) / sizeof(Cell *));
      }
    }
#endif
