...
inline Utils::Vector3d bd_random_walk(BrownianThermostat const &brownian,
  ...
  for (int j = 0; j < 3; j++) {
...
#endif
    {
      ...
      if (sigma_pos > 0.0) {
        delta_pos_body[j] = sigma_pos * std::sqrt(dt) * noise[j];
      } else {
        delta_pos_body[j] = 0.0;
      }
#else
      if (sigma_pos[j] > 0.0) {
        delta_pos_body[j] = sigma_pos[j] * std::sqrt(dt) * noise[j];
      } else {
        delta_pos_body[j] = 0.0;
      }
#endif // PARTICLE_ANISOTROPY
...
inline Utils::Vector3d bd_random_walk_vel(BrownianThermostat const &brownian,
  ...
  for (int j = 0; j < 3; j++) {
...
#endif
    {
      ...
      velocity[j] += sigma_vel * noise[j] / std::sqrt(p.p.mass);
    }
  }
  return velocity;
}
...
#ifndef PARTICLE_ANISOTROPY
      if (sigma_pos > 0.0) {
        dphi[j] = noise[j] * sigma_pos * std::sqrt(dt);
      }
#else
      if (sigma_pos[j] > 0.0) {
        dphi[j] = noise[j] * sigma_pos[j] * std::sqrt(dt);
      }
#endif // ROTATIONAL_INERTIA
    }
  }
...
#endif
    {
      domega[j] = sigma_vel * noise[j] / std::sqrt(p.p.rinertia[j]);
    }
  }
