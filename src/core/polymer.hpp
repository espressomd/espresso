...
/** C implementation of 'polymer \<N_P\> \<MPC\> \<bond_length\> [options]',
   ...
   polymer?  (0=no, 1=yes, default: 0)
    @return Returns how often the attempt to place a monomer failed in the worst
   case. <br>
...
/** C implementation of 'salt \<N_pS\> \<N_nS\> [options]',
    ...
    @param  part_id     = particle number of the first salt ion (defaults to
   'n_total_particles')
    @param  mode        = selects setup mode: Self avoiding walk (SAW) or plain
   random walk (RW) (defaults to 'SAW')
    @param  shield      = shield around each particle another particle's
   position may not enter if using SAW (defaults to '0')
    @param  max_try     = how often a monomer should be reset if current
   position collides with a previous particle (defaults to '30000')
    ...
    @param  type_pS     = type numbers to be used for positive salt ions
   (defaults to '3')
    @param  type_nS     = type numbers to be used for negative salt ions
   (defaults to '4')
    @param  rad         = radius of the sphere for the cell model. If value is
   >0 the ions are distributed in a sphere centered
...
/** C implementation of 'velocities \<v_max\> [options]',
    ...
    @param  part_id     = particle number of the first of the \<N_T\> particles
   (defaults to '0')
    @param  N_T         = number of particles of which the velocities should be
   set (defaults to 'n_total_particles - part_id')
...
/** Collects the bonds leading to and from the ending monomers of the chains
   (mode == 1) or
...
/** C implementation of 'crosslink \<N_P\> \<MPC\> [options]',
    ...
    @param  link_dist   = minimum distance between the indices of two
   crosslinked monomers with different binding partners (defaults to '2')
    @param  chain_dist  = same as \<link_dist\>, but for monomers of the same
   bond (defaults to \<MPC\> =\> no bonds to the same chain allowed)
    @param  type_FENE   = type number of the FENE-typed bonded interaction bonds
   to be set between the monomers (defaults to '0')
