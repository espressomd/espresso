  ...
  if (nemd_method == NEMD_METHOD_EXCHANGE) {
    if (slab_num == nemddata.top_slab) {
      ...
      if (slab->n_fastest < nemddata.n_exchange) {
        slab->fastest[slab->n_fastest] = part.e->p.identity;
        if (part.e->m.v[0] > slab->v_min) {
          slab->v_min = part.e->m.v[0];
          slab->ind_min = slab->n_fastest;
        }
        slab->n_fastest++;
      }
      /* then replace the slowest with a new one if necessary... */
      else if (part.e->m.v[0] < slab->v_min) {
        slab->fastest[slab->ind_min] = part.e->p.identity;
        /* ... and find again the slowest one now */
        slab->v_min = local_particles[slab->fastest[0]]->e->m.v[0];
        slab->ind_min = 0;
        for (i = 1; i < slab->n_fastest; i++)
          if (local_particles[slab->fastest[i]]->e->m.v[0] > slab->v_min) {
            slab->v_min = local_particles[slab->fastest[i]]->e->m.v[0];
            slab->ind_min = i;
          }
      }
