...
/** Computes the three body angle interaction force and adds this
    ...
    @return 0
*/
int Bond::AngleCosine::add_bonded_three_particle_force(Particle *p1, Particle *p2, Particle *p3, double force[3], double force2[3]) const {
  ...
  if ( cosine < -TINY_COS_VALUE)  cosine = -TINY_COS_VALUE;
  fac *= m_sin_phi0 * (cosine/sqrt(1-SQR(cosine))) + m_cos_phi0;
...
/** Computes the three body angle interaction energy (see \ref tclcommand_inter, \ref tclcommand_analyze). 
    ...
    @return 0.
*/
int Bond::AngleCosine::add_bonded_three_particle_energy(Particle *p1, Particle *p2, Particle *p3, 
  ...
  *_energy = m_bend*(cosine*m_cos_phi0 - sqrt(1-SQR(cosine))*m_sin_phi0+1);
