    ...
    for (k = 0; k < 3; k++) {
      ...
      if (p1molid == p2molid) {
        *obsstat_nonbonded_intra(&virials_non_bonded, p1->p.type, p2->p.type) +=
        ...
        if (p1molid != p2molid) {
          *obsstat_nonbonded_inter(&virials_non_bonded, p1->p.type,
                                   p2->p.type) +=
              d[0] * force[0] + d[1] * force[1] + d[2] * force[2];
        ...
        if (coulomb.method != COULOMB_NONE) {
          ...
          case COULOMB_P3M:
            /**
            Here we calculate the short ranged contribution of the
            electrostatics. These terms are called Pi_{dir, alpha, beta} in the
            paper by Essmann et al "A smooth particle mesh Ewald method", The
            Journal of Chemical Physics 103, 8577 (1995); doi: 10.1063/1.470117.
            The part Pi_{corr, alpha, beta} in the Essmann paper is not present
            here since M is the empty set in our simulations.
            */
            force[0] = 0.0;
            force[1] = 0.0;
            force[2] = 0.0;
            ...
            case COULOMB_DH: {
              ...
              for (k = 0; k < 3; k++) {
                ...
                virials.coulomb[0] +=
                    force[0] * d[0] + force[1] * d[1] + force[2] * d[2];
                break;
              }
            case COULOMB_RF: {
              double force[3] = {0, 0, 0};
            ...
            default:
              fprintf(stderr,
                      "calculating pressure for electrostatics method that "
            ...
            /* calc_three_body_bonded_forces is called by
               add_three_body_bonded_stress. This routine is only entered for
               angular potentials. */
            inline void calc_three_body_bonded_forces(
                Particle * p1, Particle * p2, Particle * p3,
                Bonded_ia_parameters * iaparams, Vector3d & force1,
...
// char* errtxt;
              ...
              case BONDED_IA_ANGLE_HARMONIC:
                ...
                calc_angle_harmonic_3body_forces(p1, p2, p3, iaparams, force1,
                                                 force2, force3);
                break;
              case BONDED_IA_ANGLE_COSINE:
                // p1 is *p_mid, p2 is *p_left, p3 is *p_right
                calc_angle_cosine_3body_forces(p1, p2, p3, iaparams, force1,
                                               force2, force3);
                break;
              case BONDED_IA_ANGLE_COSSQUARE:
                // p1 is *p_mid, p2 is *p_left, p3 is *p_right
                calc_angle_cossquare_3body_forces(p1, p2, p3, iaparams, force1,
                                                  force2, force3);
                break;
#ifdef TABULATED
              case BONDED_IA_TABULATED:
                ...
                case TAB_BOND_ANGLE:
                  // p1 is *p_mid, p2 is *p_left, p3 is *p_right
                  calc_angle_3body_tabulated_forces(p1, p2, p3, iaparams,
                                                    force1, force2, force3);
                  break;
                default:
                  runtimeErrorMsg()
                      << "calc_bonded_force: tabulated bond type of atom "
             ...
             *  For performance reasons the force routines add their values
             * directly to the particles. So here we do some tricks to get the
             * value out without changing the forces.
            ...
            inline void add_bonded_virials(Particle * p1) {
              ...
              while (i < p1->bl.n) {
                ...
                if (!p2) {
                  // for harmonic spring:
                  // if cutoff was defined and p2 is not there it is anyway
                  // outside the cutoff, see calc_maximal_cutoff()
                  if ((type_num == BONDED_IA_HARMONIC) &&
                      (iaparams->p.harmonic.r_cut > 0)) {
                    return;
                  }
                  runtimeErrorMsg()
                ...
                *obsstat_bonded(&virials, type_num) +=
                ...
                for (k = 0; k < 3; k++) {
                  for (l = 0; l < 3; l++) {
                    obsstat_bonded(&p_tensor, type_num)[k * 3 + l] +=
                        force[k] * dx[l];
                  }
                }
              }

              /** Calculate the contribution to the stress tensor from angular
               * potentials. The central particle of the three-particle
               * interaction is responsible for the contribution of the entire
               * interaction - this is the coding not the physics.
               */
              inline void add_three_body_bonded_stress(Particle * p1) {
                ...
                while (i < p1->bl.n) {
                  ...
                  calc_three_body_bonded_forces(p1, p2, p3, iaparams, force1,
                                                force2, force3);
                  /* three-body bonded interactions contribute to the stress but
                   * not the scalar pressure */
                  for (int k = 0; k < 3; k++) {
                    for (int l = 0; l < 3; l++) {
               ...
               *  @param v_comp flag which enables (1) compensation of the
               * velocities required for deriving a pressure reflecting \ref
               * nptiso_struct::p_inst (hence it only works with domain
               * decomposition); naturally it therefore doesn't make sense to
               * use it without NpT.
               */
              inline void add_kinetic_virials(Particle * p1, int v_comp) {
                  ...
                  if (v_comp) {
                    virials.data.e[0] +=
                        (Utils::sqr(p1->m.v[0] * time_step -
                                    p1->f.f[0] * 0.5 * time_step * time_step /
                                        p1->p.mass) +
                         Utils::sqr(p1->m.v[1] * time_step -
                                    p1->f.f[1] * 0.5 * time_step * time_step /
                                        p1->p.mass) +
                         Utils::sqr(p1->m.v[2] * time_step -
                                    p1->f.f[2] * 0.5 * time_step * time_step /
                                        p1->p.mass)) *
                        (*p1).p.mass;
                  } else {
                ...
                /* ideal gas contribution (the rescaling of the velocities by
                 * '/=time_step' each will be done later) */
                for (k = 0; k < 3; k++) {
                  for (l = 0; l < 3; l++) {
                    p_tensor.data.e[k * 3 + l] += (p1->m.v[k] * time_step) *
                                                  (p1->m.v[l] * time_step) *
                                                  (*p1).p.mass;
                  }
                }
