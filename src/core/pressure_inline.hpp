...
inline void add_non_bonded_pair_virials(Particle const &p1, Particle const &p2,
...
#endif
  {
    ...
    if (p1molid == p2molid) {
      obs_scalar_pressure.non_bonded_intra_contribution(
          p1.p.type, p2.p.type)[0] += d * force;

      for (int k = 0; k < 3; k++)
        for (int l = 0; l < 3; l++)
          obs_pressure_tensor.non_bonded_intra_contribution(
              p1.p.type, p2.p.type)[k * 3 + l] += force[k] * d[l];
    } else {
      obs_scalar_pressure.non_bonded_inter_contribution(
          p1.p.type, p2.p.type)[0] += d * force;

      for (int k = 0; k < 3; k++)
        for (int l = 0; l < 3; l++)
          obs_pressure_tensor.non_bonded_inter_contribution(
              p1.p.type, p2.p.type)[k * 3 + l] += force[k] * d[l];
    }
  }
