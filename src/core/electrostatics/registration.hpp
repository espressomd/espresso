...
void remove_actor(std::shared_ptr<T> const &actor) {
  auto &system = System::get_system();
  if (not is_already_stored(actor, system.coulomb.solver)) {
    throw std::runtime_error(
        "The given electrostatics solver is not currently active");
  }
  remove_actor(system.coulomb.solver, actor, ::on_coulomb_change);
}
...
void add_actor(std::shared_ptr<T> const &actor) {
  auto &system = System::get_system();
  if (system.coulomb.extension) {
    auto const name = get_actor_name(*system.coulomb.extension);
    throw std::runtime_error("An electrostatics extension is already active (" +
                             name + ")");
  }
  add_actor(system.coulomb.extension, actor, ::on_coulomb_change,
            detail::flag_all_reduce);
}
...
void remove_actor(std::shared_ptr<T> const &actor) {
  auto &system = System::get_system();
  if (not is_already_stored(actor, system.coulomb.extension)) {
    throw std::runtime_error(
        "The given electrostatics extension is not currently active");
  }
  remove_actor(system.coulomb.extension, actor, ::on_coulomb_change);
}
