...
std::shared_ptr<Observable_stat> System::calculate_energy() {
  ...
  VerletCriterion<> const verlet_criterion{
      *this, cell_structure->get_verlet_skin(), get_interaction_range(),
      coulomb.cutoff(), dipoles.cutoff(), -1.0};
  ...
  auto mol_id_host =
      Kokkos::create_mirror_view(mol_id_view);
  ...
  // Non Bonded energies
  EnergyKernel pair_e_kernel{
      *bonded_ias, *nonbonded_ias, coulomb, get_ptr(coulomb_kernel), get_ptr(dipoles_kernel),
      *box_geo, cell_structure->get_unique_particles(),
      local_energy, layout, cell_structure->get_aosoa(), mol_id_view,
      maximal_cutoff()};
																		   
  // Bonded energies: write a BondsEnergyKernelData + *BondsEnergyKernel
  auto &bs = cell_structure->bond_state();
  BondsEnergyKernelData bonds_e_data{
      *bonded_ias, *box_geo, local_energy, layout, cell_structure->get_aosoa()};
  PairBondsEnergyKernel  pair_be_kernel{bonds_e_data, bs.pair_list, bs.pair_ids, get_ptr(coulomb_kernel)};
  AngleBondsEnergyKernel angle_be_kernel{bonds_e_data, bs.angle_list, bs.angle_ids};
  DihedralBondsEnergyKernel dih_be_kernel{bonds_e_data, bs.dihedral_list, bs.dihedral_ids};
  ...
  reduce_cabana_energy(local_energy, layout, obs_energy,
		       *bonded_ias, nonbonded_ias->get_max_seen_particle_type() + 1);
#else
  short_range_loop(
