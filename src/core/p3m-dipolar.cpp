...
/** Calculates for magnetic dipoles the properties of the send/recv sub-meshes
...
/** Initializes for magnetic dipoles the (inverse) mesh constant \ref
    p3m_parameter_struct::a (\ref p3m_parameter_struct::ai) and the cutoff for
   charge
...
/** Calculate for magnetic dipoles the spacial position of the left down mesh
...
/** Calculates the influence function optimized for the dipolar energy and
 * torques. */
static void dp3m_calc_influence_function_energy();

/** Calculates the constants necessary to correct the dipolar energy to minimize
 * the error. */
...
void dp3m_dipole_assign(void) {
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < dp3m.local_mesh.size; j++)
          ...
          for (i1 = 0; i1 < dp3m.params.cao; i1++) {
            for (i2 = 0; i2 < dp3m.params.cao; i2++) {
              ...
              */
              switch (d_rs) {
              case 0: // E_x
                p[i].f.torque[1] -= p[i].r.dip[2] * prefac *
                                    dp3m.ca_frac[cf_cnt] * dp3m.rs_mesh[q_ind];
                p[i].f.torque[2] += p[i].r.dip[1] * prefac *
                                    dp3m.ca_frac[cf_cnt] * dp3m.rs_mesh[q_ind];
                break;
              case 1: // E_y
                p[i].f.torque[0] += p[i].r.dip[2] * prefac *
                                    dp3m.ca_frac[cf_cnt] * dp3m.rs_mesh[q_ind];
                p[i].f.torque[2] -= p[i].r.dip[0] * prefac *
                                    dp3m.ca_frac[cf_cnt] * dp3m.rs_mesh[q_ind];
                break;
              case 2: // E_z
                p[i].f.torque[0] -= p[i].r.dip[1] * prefac *
                                    dp3m.ca_frac[cf_cnt] * dp3m.rs_mesh[q_ind];
                p[i].f.torque[1] += p[i].r.dip[0] * prefac *
                                    dp3m.ca_frac[cf_cnt] * dp3m.rs_mesh[q_ind];
              }
              q_ind++;
              cf_cnt++;
...
double calc_surface_term(int force_flag, int energy_flag) {
  ...
  Particle *part;
  double pref = coulomb.Dprefactor * 4 * M_PI * box_l_i[0] * box_l_i[1] *
...
/** get the minimal error for this combination of parameters. In fact, the real
   space error is tuned such that it
    contributes half of the total error, and then the Fourier space error is
   calculated. Returns the error and the
  ...
  for (c = 0; c < local_cells.n; c++) {
    ...
    for (i = 0; i < np; i++) {
      if (part[i].p.dipm != 0.0) {
        node_sums[0] += SQR(part[i].r.dip[0]) + SQR(part[i].r.dip[1]) +
                        SQR(part[i].r.dip[2]);
        node_sums[1] += 1.0;
      }
    }
...
void dp3m_compute_constants_energy_dipolar() {
  ...
  double volume = box_l[0] * box_l[1] * box_l[2];
  Ukp3m =
      dp3m_average_dipolar_self_energy(box_l[0], dp3m.params.mesh[0]) * volume;
