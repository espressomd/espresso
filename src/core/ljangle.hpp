...
int ljangle_set_params(int part_type_a, int part_type_b, double eps, double sig,
                       ...
                       double z0, double dz, double kappa, double epsprime);

/** Calculate lj-angle force between particle p1 and p2
    Involves 6 particles total */
inline void add_ljangle_force(Particle *p1, Particle *p2,
  ...
  double effective_eps = ia_params->LJANGLE_eps, z1, z2, z_middle, z_ref,
         z_ref5, localz0 = ia_params->LJANGLE_z0,
         localdz2 = ia_params->LJANGLE_dz / 2.,
  ...
  if (part1p >= 0 && part1p < n_part && part1n >= 0 && part1n < n_part &&
      part2p >= 0 && part2p < n_part && part2n >= 0 && part2n < n_part) {
    ...
    if (cos_jik > 0. && cos_ikn > 0.) {
      ...
      /* Optional 2nd environment */
      if (localdz2 > 0.) {
        /* calculate center position of the interaction (calculate minimal
         * distance) */
        z1 = p1->r.p[2];
        z2 = p2->r.p[2];
        z_middle = z1 + z2;
        ...
        /* If we're in the environment #2 region, calculate the new interaction
         * strength */
        if (z_middle > localz0 - localdz2 && z_middle <= localz0) {
          z_ref = z_middle - localz0 + localdz2;
          z_ref5 = pow(z_ref, 5);
          effective_eps +=
              (ia_params->LJANGLE_epsprime - ia_params->LJANGLE_eps) *
      ...
      if (radprime > 1000)
        fprintf(stderr, "%d: LJANGLE-Warning: Pair (%d-%d) force=%f dist=%f\n",
                this_node, p1->p.identity, p2->p.identity, radprime, dist);
#endif

      /* Regroup the last two cases in one */
      /* Propagate all forces in this function rather than in the forces.hpp
       * file */
      for (j = 0; j < 3; ++j) {
        p1->f.f[j] +=
            angular_jik * angular_ikn * rik[j] / l_rik * radprime +
      ...
      LJ_TRACE(fprintf(
          stderr,
          "%d: LJANGLE: Pair (%d-%d) dist=%.3f: force+-: (%.3e,%.3e,%.3e)\n",
          this_node, p1->p.identity, p2->p.identity, dist, rad * d[0],
          rad * d[1], rad * d[2]));
    }
  }
}
