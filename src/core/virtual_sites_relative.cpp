...
void update_mol_pos_particle(Particle *p) {
  ...
  if (!p_real) {
    ostringstream msg;
    msg << "virtual_sites_relative.cpp - update_mol_pos_particle(): No real "
  ...
  for (c = 0; c < local_cells.n; c++) {
    ...
    for (i = 0; i < np; i++) {
      ...
      if (ifParticleIsVirtual(&p[i])) {
        ...
        // Get distance vector pointing from real to virtual particle,
        // respecting periodic boundary i
        // conditions
        double d[3];
        ...
        //       %d\n",p_real->p.identity, p[i].f.f[0], p[i].f.f[1],p[i].f.f[2],
        //       p[i].p.identity);
        for (j = 0; j < 3; j++) {
...
void vs_relative_pressure_and_stress_tensor(double *pressure,
  ...
  for (c = 0; c < local_cells.n; c++) {
    ...
    for (i = 0; i < np; i++) {
      ...
      *pressure +=
          (p[i].f.f[0] * d[0] + p[i].f.f[1] * d[1] + p[i].f.f[2] * d[2]);
    }
  }
