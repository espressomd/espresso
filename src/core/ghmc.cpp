...
void save_last_state() {
  for (auto &p : local_cells.particles()) {
    memmove(&p.e->l.r_ls, &p.r, sizeof(ParticlePosition));
    memmove(&p.e->l.m_ls, &p.e->m, sizeof(ParticleMomentum));
  }
}

void load_last_state() {
  for (auto &p : local_cells.particles()) {
    memmove(&p.r, &p.e->l.r_ls, sizeof(ParticlePosition));
    memmove(&p.e->m, &p.e->l.m_ls, sizeof(ParticleMomentum));
  }
}
...
double calc_local_temp() {
  ...
  for (auto &p : local_cells.particles()) {
    for (int j = 0; j < 3; j++) {
      temp += p.e->p.mass * Utils::sqr(p.e->m.v[j] / time_step);
#ifdef ROTATION
      temp += Utils::sqr(p.e->m.omega[j]);
#endif
    }
  }
#ifdef ROTATION
...
void calc_kinetic(double *ek_trans, double *ek_rot) {
  ...
  for (auto &p : local_cells.particles()) {
    ...
    /* kinetic energy */
    et += (Utils::sqr(p.e->m.v[0]) + Utils::sqr(p.e->m.v[1]) + Utils::sqr(p.e->m.v[2])) *
          (p).e->p.mass;

/* rotational energy */
#ifdef ROTATION
    er += Utils::sqr(p.e->m.omega[0]) * p.e->p.rinertia[0] +
          Utils::sqr(p.e->m.omega[1]) * p.e->p.rinertia[1] +
          Utils::sqr(p.e->m.omega[2]) * p.e->p.rinertia[2];
#endif
  }
...
void tscale_momentum_update() {
  ...
  for (auto &p : local_cells.particles()) {
    ...
    for (int j = 0; j < 3; j++) {
      p.e->m.v[j] = cosp * (p.e->l.m_ls.v[j]) + sinp * (p.e->m.v[j]);
#ifdef ROTATION
      p.e->m.omega[j] = cosp * (p.e->l.m_ls.omega[j]) + sinp * (p.e->m.omega[j]);
#endif
    }
  }
}
...
#ifdef MASS
    sigmat = sqrt(temperature / (p).e->p.mass);
#endif
    for (int j = 0; j < 3; j++) {
      if (sigmat > 0.0) {
        p.e->m.v[j] = sigmat * gaussian_random() * time_step;
      }
#ifdef ROTATION
#ifdef ROTATIONAL_INERTIA
      sigmar = sqrt(temperature / p.e->p.rinertia[j]);
#endif
      if (sigmar > 0.0) {
        p.e->m.omega[j] = sigmar * gaussian_random();
      }
#endif
    }
  }
...
void partial_momentum_update() {
  ...
  for (auto &p : local_cells.particles()) {
    ...
    for (int j = 0; j < 3; j++) {
      if (sigmat > 0.0) {
        p.e->m.v[j] =
            cosp * (p.e->m.v[j]) + sinp * (sigmat * gaussian_random() * time_step);
      } else {
        p.e->m.v[j] = cosp * p.e->m.v[j];
      }
#ifdef ROTATION
#ifdef ROTATIONAL_INERTIA
      sigmar = sqrt(temperature / p.e->p.rinertia[j]);
      ...
      if (sigmar > 0.0) {
        p.e->m.omega[j] =
            cosp * (p.e->m.omega[j]) + sinp * (sigmar * gaussian_random());
      } else {
        p.e->m.omega[j] = cosp * p.e->m.omega[j];
      }
#endif
    }
  }
