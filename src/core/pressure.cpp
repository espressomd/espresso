...
int getintersection(double pos1[3], double pos2[3], int given, int get,
                    double value, double *answer, double box_size[3]) {
  /*pos1 and pos2 are two particle positions. */
  /*given and get are integers from 0 to 2. 0 = x direction. 1 = y direction. 2
   * = z direction  */
  /*there is a point on the line between the two particles p1 and p2 such that
   * r[given]=value  */
  ...
  for (i = 0; i < 3; i++) {
    p2r[i] = drem_down((pos2[i] - pos1[i]) + box_size[i] / 2.0, box_size[i]) -
             box_size[i] / 2.0;
  }
  value = drem_down((value - pos1[given]) + box_size[given] / 2.0,
                    box_size[given]) -
          box_size[given] / 2.0;
  // PTENSOR_TRACE(fprintf(stderr,"%d: getintersection: p1 is %f %f %f p2 is %f
  // %f %f p2r is %f %f %f newvalue is
  ...
  if ((value) * (p2r[given]) < -0.0001) {
    ...
    *answer = drem_down(value + pos1[given], box_size[given]);
    ...
    runtimeErrorMsg() << "analyze stress_profile: getintersection: "
                         "intersection is a line, not a point - value is "
                      << value << " same as " << 0.0 << " and " << p2r[given]
                      << "\n";
    return 0;
  } else {
    *answer =
        drem_down(pos1[get] + p2r[get] / p2r[given] * value, box_size[get]);
  }
  return 1;
}

int getlength(double pos1[3], double pos2[3], int d1, double val1, int d2,
              double val2, int l, double *answer) {
  ...
  *answer = 0;
  ...
  if (!getintersection(pos1, pos2, d2, l, val2, &intersect1, box_l) ||
      !getintersection(pos1, pos2, d1, l, val1, &intersect2, box_l)) {
    return 0;
  } else {
    *answer = drem_down(intersect2 - intersect1 + box_l[l] / 2.0, box_l[l]) -
              box_l[l] / 2.0;
    return 1;
  }
}

int does_line_go_through_cube(double pos1[3], double pos2[3],
                              double range_start[3], double range[3],
                              int sign[3], double entry[3], double exit[3],
...
*/
{
  ...
  *faceout = -1;
  ...
  inside2 = 1;
  for (i = 0; i < 3; i++) {
    if (fabs(vect2centre1[i]) > range[i] / 2.0)
      inside1 = 0;
    ...
    for (i = 0; i < 3; i++) {
      ...
      if ((!doesntenter) && (!found_entry) &&
        ...
        if ((vect2centre1[i] * sign[i] > 0) ||
          ...
          getintersection(vect2centre1, vect2centre2, i, i1,
                          -range[i] / 2.0 * sign[i], &intersection1, box_l);
          if (intersection1 > box_l[i1] / 2)
            intersection1 -= box_l[i1];
          if (fabs(intersection1) < range[i1] / 2.0) {
            getintersection(vect2centre1, vect2centre2, i, i2,
                            -range[i] / 2.0 * sign[i], &intersection2, box_l);
            if (intersection2 > box_l[i2] / 2)
              intersection2 -= box_l[i2];
  ...
  if (!(found_entry) && !(inside1)) {
    PTENSOR_TRACE(
        fprintf(
            stderr,
            "%d: does_line_go_through_cube: Line does not pass through cube \n",
            this_node););
    return 0;
  } else {
    PTENSOR_TRACE(fprintf(stderr,
    ...
    if (!inside2) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        if ((found_exit == 0) &&
            (fabs(vect2centre2[i]) >
             range[i] / 2.0)) { /*starts outside cube in i direction */
          getintersection(vect2centre1, vect2centre2, i, i1,
                          range[i] / 2.0 * sign[i], &intersection1, box_l);
          if (intersection1 > box_l[i1] / 2)
            intersection1 -= box_l[i1];
    ...
    PTENSOR_TRACE(fprintf(stderr,
                          "%d: does_line_go_through_cube: Exit is %f %f %f\n",
                          this_node, exit[0], exit[1], exit[2]););
  }
  PTENSOR_TRACE(
      fprintf(stderr,
              "%d: does_line_go_through_cube: facein is %d faceout is %d\n",
              this_node, *facein, *faceout););
  return 1;
}

int distribute_tensors(DoubleList *TensorInBin, double *force, int bins[3],
  ...
  */
  ...
  /* check if the line passes through the cube and if so where it enters and
   * exits it */
  if (does_line_go_through_cube(pos1, pos2, range_start, range, sign, entry,
                                exit, &facein, &faceout)) {

    /* calculate reduced coordinates */
    /* range_start becomes (0,0,0) and range_start + range becomes
    ...
    if ((endx < 0) && (range[0] == box_l[0]))
      startx += bins[0];
    occupiedxbins = (sign[0] * (endx - startx) + bins[0]) % bins[0] + 1;

    PTENSOR_TRACE(fprintf(stderr,
                          "%d: distribute_tensors: x goes from %d to %d\n",
    ...
    /* in xbin the line starts in y-bin number starty[xbin-startx] and ends in
     * starty[xbin-startx+1] */
    totoccupiedybins = 0;
    if (facein == 1) {
    ...
    for (xa = 0; xa < occupiedxbins; xa++) {
      ...
      if (xbin == endx) {
        ...
        if (faceout == 1)
          intersect -= sign10[1];
        if ((intersect < 0) && (range[1] == box_l[1]))
          intersect += bins[1];
      } else {
        if (getintersection(redentry, redexit, 0, 1, xbin + sign10[0],
                            &intersect, redbox_l) != 1)
          return 0;
      }
      starty[xa + 1] = floor(intersect);
      occupiedybins[xa] =
          ((starty[xa + 1] - starty[xa]) * sign[1] + bins[1]) % bins[1] + 1;
      totoccupiedybins += occupiedybins[xa];
      PTENSOR_TRACE(
          fprintf(stderr,
                  "%d: distribute_tensors: in xbin %d y goes from %d to %d\n",
        ...
        for (za = 0; za < occupiedzbins[counter]; za++) {
          ...
          PTENSOR_TRACE(
              fprintf(stderr,
                      ...
                      range_start[d2] + val2 * range[d2] / (double)bins[d2]););
          for (l = 0; l < 3; l++) {
            if (getlength(entry, exit, d1,
                          range_start[d1] + val1 * range[d1] / (double)bins[d1],
                          d2,
                          range_start[d2] + val2 * range[d2] / (double)bins[d2],
                          l, &segment) != 1)
              return 0;
            ...
            for (k = 0; k < 3; k++) {
              TensorInBin[xbin * bins[1] * bins[2] + ybin * bins[2] + zbin]
                  .e[3 * k + l] += force[k] * segment;
            }
          }
    ...
    PTENSOR_TRACE(
        fprintf(stderr,
                "%d: distribute_tensors: calclength is %e and length is %e\n}",
                this_node, calclength, length););

    if (calclength - length > 0.0000000001) {
      runtimeErrorMsg() << this_node
                        << ": analyze stress_profile: bug in distribute tensor "
                           "code - calclength is "
                        << calclength << " and length is " << length;
      return 0;
    }
  }
  return 1;
...
int reducepos(double pos[3], int bins[3], double centre[3], double range[3],
              int reducedpos[3]) {
  int i;
  double working[3];
  ...
  for (i = 0; i < 3; i++) {
    reducedpos[i] =
        floor((working[i] + range[i] / 2.0) * (double)bins[i] / range[i]);
  }
  return 1;
}
...
int incubewithskin(double pos[3], double centre[3], double range[3]) {
  ...
  get_mi_vector(working, pos, centre);
  for (i = 0; i < 3; i++) {
    if (fabs(working[i]) > range[i] / 2.0 + skin + max_cut)
      return 0;
  }
  return 1;
}

int whichbin(double pos[3], int bins[3], double centre[3], double range[3],
             int *bin)
/*calculates which bin a particle is in for local_stress_tensor */
{
  ...
  *bin = reducedpos[0] * bins[1] * bins[2] + reducedpos[1] * bins[2] +
         reducedpos[2];
  return 1;
}
...
int get_nonbonded_interaction(Particle *p1, Particle *p2, double *force,
  ...
  if ((p1->e->p.identity != p2->e->p.identity) &&
    ...
    if (coulomb.method != COULOMB_NONE) {
      ...
      case COULOMB_P3M_GPU:
        fprintf(stderr, "WARNING: Local stress tensor calculation cannot "
                        "handle GPU P3M electrostatics so it is left out\n");
        break;
      case COULOMB_P3M:
        fprintf(stderr, "WARNING: Local stress tensor calculation cannot "
                        "handle P3M electrostatics so it is left out\n");
        break;
#endif
      case COULOMB_DH:
        for (i = 0; i < 3; i++)
    ...
    if (coulomb.Dmethod != DIPOLAR_NONE) {
      ...
      case DIPOLAR_P3M:
        fprintf(stderr, "WARNING: Local stress tensor calculation cannot "
                        "handle P3M magnetostatics so it is left out\n");
        break;
#endif
      case DIPOLAR_ALL_WITH_ALL_AND_NO_REPLICA:
        fprintf(stderr, "WARNING: Local stress tensor calculation cannot "
      ...
      default:
        fprintf(stderr, "WARNING: Local stress tensor calculation does not "
                        "recognise this magnetostatic interaction\n");
      }
    }
#endif /*ifdef DIPOLES */
...
int local_stress_tensor_calc(DoubleList *TensorInBin, int bins[3],
  ...
  */
      ...
      for (int k = 0; k < 3; k++) {
        for (int l = 0; l < 3; l++) {
          TensorInBin[bin].e[k * 3 + l] +=
              (p.e->m.v[k]) * (p.e->m.v[l]) * p.e->p.mass / time_step / time_step;
        }
      }
    }
  };

  auto add_bonded = [&](Particle &p) {
    int j = 0;
    while (j < p.bl.n) {
      ...
      PTENSOR_TRACE(
          fprintf(stderr, "%d: Bonded to particle %d with force %f %f %f\n",
                  this_node, p2->e->p.identity, force[0], force[1], force[2]));
  ...
  auto add_single_particle_contribution = [&add_ideal,
                                           &add_bonded](Particle &p) {
    add_ideal(p);
    add_bonded(p);
  };
...
int analyze_local_stress_tensor(int *periodic, double *range_start,
  ...
  PTENSOR_TRACE(
      fprintf(stderr, "%d: Running analyze_local_stress_tensor\n", this_node));

  mpi_local_stress_tensor(TensorInBin, bins, periodic, range_start, range);
  PTENSOR_TRACE(fprintf(
      stderr,
      "%d: analyze_local_stress_tensor: finished mpi_local_stress_tensor \n",
      this_node));
