...
void update_mol_ids_setchains() {
  for (auto &p : local_cells.particles()) {
    p.e->p.mol_id = floor((p.e->p.identity - chain_start) / (double)chain_length);
  }
}
  ...
  *_rg = rg = Utils::realloc(rg, 4 * sizeof(double));
  ...
  for (i = 0; i < chain_n_chains; i++) {
    ...
    for (j = 0; j < chain_length; j++) {
      ...
      r_CM_x += partCfg[p].r.p[0] * (partCfg[p]).e->p.mass;
      r_CM_y += partCfg[p].r.p[1] * (partCfg[p]).e->p.mass;
      r_CM_z += partCfg[p].r.p[2] * (partCfg[p]).e->p.mass;
      M += (partCfg[p]).e->p.mass;
    }
    r_CM_x /= M;
    r_CM_y /= M;
    r_CM_z /= M;
  ...
  *_rg = rg = Utils::realloc(rg, 4 * sizeof(double));
  ...
  for (k = 0; k < n_configs; k++) {
    for (i = 0; i < chain_n_chains; i++) {
      ...
      for (j = 0; j < chain_length; j++) {
        p = chain_start + i * chain_length + j;
        r_CM_x += configs[k][3 * p] * (partCfg[p]).e->p.mass;
        r_CM_y += configs[k][3 * p + 1] * (partCfg[p]).e->p.mass;
        r_CM_z += configs[k][3 * p + 2] * (partCfg[p]).e->p.mass;
        M += (partCfg[p]).e->p.mass;
      }
      r_CM_x /= M;
      r_CM_y /= M;
...
void init_g123(PartCfg &partCfg) {
  ...
  partCoord_g = Utils::realloc(partCoord_g, 3 * n_part * sizeof(float));
  partCM_g = Utils::realloc(partCM_g, 3 * chain_n_chains * sizeof(float));
  n_part_g = n_part;
  ...
  for (j = 0; j < chain_n_chains; j++) {
    ...
    for (i = 0; i < chain_length; i++) {
      ...
      cm_tmp[0] += partCfg[p].r.p[0] * (partCfg[p]).e->p.mass;
      partCoord_g[3 * p + 1] = partCfg[p].r.p[1];
      cm_tmp[1] += partCfg[p].r.p[1] * (partCfg[p]).e->p.mass;
      partCoord_g[3 * p + 2] = partCfg[p].r.p[2];
      cm_tmp[2] += partCfg[p].r.p[2] * (partCfg[p]).e->p.mass;
      M += (partCfg[p]).e->p.mass;
    }
    ...
    for (t = window ? 0 : n_configs - k - 1; t < n_configs - k; t++) {
      for (j = 0; j < chain_n_chains; j++) {
        for (i = 0; i < chain_length; i++) {
          p = chain_start + j * chain_length + i;
          g1[k] += weights[0] *
                       Utils::sqr(configs[t + k][3 * p] - configs[t][3 * p]) +
                   weights[1] * Utils::sqr(configs[t + k][3 * p + 1] -
                                           configs[t][3 * p + 1]) +
                   weights[2] * Utils::sqr(configs[t + k][3 * p + 2] -
                                           configs[t][3 * p + 2]);
        }
      }
    }
    ...
    for (t = window ? 0 : n_configs - k - 1; t < n_configs - k; t++) {
      for (j = 0; j < chain_n_chains; j++) {
        ...
        for (i = 0; i < chain_length; i++) {
          p = chain_start + j * chain_length + i;
          cm_tmp[0] +=
              (configs[t + k][3 * p] - configs[t][3 * p]) * (partCfg[p]).e->p.mass;
...
void analyze_formfactor(PartCfg &partCfg, double qmin, double qmax, int qbins,
                        double **_ff) {
  int i, j, k, qi, cnt, cnt_max;
  double q, qfak, qr, dx, dy, dz, *r_ij = nullptr, *ff = nullptr;
  *_ff = ff = Utils::realloc(ff, (qbins + 1) * sizeof(double));
  r_ij = Utils::realloc(r_ij,
                        chain_length * (chain_length - 1) / 2 * sizeof(double));
