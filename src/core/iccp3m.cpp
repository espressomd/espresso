...
int iccp3m_iteration() {
    ...
    for (j = 0; j < iccp3m_cfg.num_iteration; j++) {
        ...
        ghost_communicator(&cell_structure.exchange_ghosts_comm); //Exchange particle properties from ghosts. IMPROVE: only charges

        force_calc_iccp3m(); /* Calculate electrostatic forces (SR+LR) excluding source source interaction*/
        ghost_communicator(&cell_structure.collect_ghost_force_comm); //Exchange forces from ghosts
        ...
        for (auto &p : local_cells.particles()) {
            if (p.p.identity < iccp3m_cfg.n_ic + iccp3m_cfg.first_id &&
                ...
                if (iccp3m_cfg.sigma != 0)
                    f2 = (2 * iccp3m_cfg.eout) / (iccp3m_cfg.eout + iccp3m_cfg.ein[id]) *
                        (iccp3m_cfg.sigma[id]);
...
#ifdef ICC_DEBUG
        if (this_node == 0) 
            fprintf(stderr, "Iteration %d, max global relative charge variation %e\n", j, globalmax);
#endif
...
void force_calc_iccp3m() {
    ...
    /*  I don´t see the point of this part until there are electrical dipoles in
        Espresso, BTW, it generates a warning .. JJCP
#ifdef DIPOLES
convert_quat_to_dip_all();
    ...
    if (coulomb.method == COULOMB_P3M_GPU) {
        copy_part_data_to_gpu(local_cells.particles()); //IMPROVE: Only copy charges to GPU, copy positions only once
    }
#endif

    calc_long_range_forces_iccp3m(); //IMPROVE: Calc Field for non-ICC particles once only calc ICC LR here
        ...
        case CELL_STRUCTURE_DOMDEC:
            if (dd.use_vList) {
                if (rebuild_verletlist) {
                    ...
                    calculate_verlet_ia_iccp3m(); //IMPROVE: Only calc SR for pairs which involve ICC particles via verlet_criterion 
                }
            } else
...
#ifdef CUDA
    if (coulomb.method == COULOMB_P3M_GPU) {
        copy_forces_from_GPU(local_cells.particles()); //IMPROVE: only copy forces from ICC particles
    }
#endif
}
...
void build_verlet_lists_and_calc_verlet_ia_iccp3m() {
    ...
    estimate = 0.5 * n_part * (4.0 / 3.0 * PI * pow(max_cut_nonbonded, 3.0)) *
        (n_part / (box_l[0] * box_l[1] * box_l[2])) / n_nodes;

    if (!dd.use_vList) {
        fprintf(stderr, "%d: build_verlet_lists, but use_vList == 0\n", this_node);
        errexit();
    }
#endif
    ...
    for (c = 0; c < local_cells.n; c++) {
        ...
        for (c = 0; c < local_cells.n; c++) {
            ...
            for (n = 0; n < dd.cell_inter[c].n_neighbors; n++) {
                ...
                for (i = 0; i < np1; i++) {
                    j_start = 0;
                    /* Tasks within cell: (no bonded forces) store old position, avoid
                     * double counting */
                    if (n == 0) {
                    ...
                    for (j = j_start; j < np2; j++) {
...
#endif
                        {
                            ...
                            VERLET_TRACE(fprintf(stderr, "%d: pair %d %d has distance %f\n",
                                        this_node, p1[i].p.identity,
                                        p2[j].p.identity, sqrt(dist2)));
                            if (verlet_list_criterion(p1 + i, p2 + j, dist2)) {
                                ONEPART_TRACE(if (p1[i].p.identity == check_id) fprintf(
                                            stderr,
                                            "%d: OPT: Verlet Pair %d %d (Cells %d,%d %d,%d dist %f)\n",
                                            this_node, p1[i].p.identity, p2[j].p.identity, c, i, n, j,
                                            sqrt(dist2)));
                                ONEPART_TRACE(if (p2[j].p.identity == check_id) fprintf(
                                            stderr,
                                            "%d: OPT: Verlet Pair %d %d (Cells %d %d dist %f)\n",
                                            this_node, p1[i].p.identity, p2[j].p.identity, c, n,
                                            sqrt(dist2)));

                                add_pair_iccp3m(pl, &p1[i], &p2[j]);
                                /* calc non bonded interactions */
                                add_non_bonded_pair_force_iccp3m(&(p1[i]), &(p2[j]), vec21,
                                        sqrt(dist2), dist2);
                            }
                        }
                    }
    ...
    VERLET_TRACE(fprintf(
                stderr,
                "%d: total number of interaction pairs: %d (should be around %d)\n",
        ...
        for (n = 0; n < dd.cell_inter[c].n_neighbors; n++) {
            ...
            for (i = 0; i < np1; i++) {
                ...
                for (j = j_start; j < np2; j++) {
                    dist2 = distance2vec(p1[i].r.p, p2[j].r.p, vec21);
                    if (dist2 <=
                            SQR(get_ia_param(p1[i].p.type, p2[j].p.type)->max_cut + skin)) {
                        /* calc non bonded interactions */
                        add_non_bonded_pair_force_iccp3m(&(p1[i]), &(p2[j]), vec21,
                                sqrt(dist2), dist2);
                    }
