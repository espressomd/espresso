...
#ifdef LENNARD_JONES
  params->LJ_eps = params->LJ_sig = params->LJ_shift = params->LJ_offset =
      params->LJ_min = 0.0;
  params->LJ_cut = INACTIVE_CUTOFF;
#endif

#ifdef LENNARD_JONES_GENERIC
  params->LJGEN_eps = params->LJGEN_sig = params->LJGEN_shift =
      params->LJGEN_offset = params->LJGEN_a1 = params->LJGEN_a2 =
          params->LJGEN_b1 = params->LJGEN_b2 = 0.0;
...
#ifdef LJ_ANGLE
  params->LJANGLE_eps = params->LJANGLE_sig = params->LJANGLE_bonded1type =
      params->LJANGLE_bonded1pos = params->LJANGLE_bonded1neg =
          params->LJANGLE_bonded2pos = params->LJANGLE_bonded2neg =
              params->LJANGLE_z0 = params->LJANGLE_kappa =
...
#ifdef SMOOTH_STEP
  params->SmSt_eps = params->SmSt_sig = params->SmSt_d = params->SmSt_n =
      params->SmSt_k0 = 0.0;
...
#ifdef BMHTF_NACL
  params->BMHTF_A = params->BMHTF_B = params->BMHTF_C = params->BMHTF_D =
      params->BMHTF_sig = params->BMHTF_computed_shift = 0.0;
  params->BMHTF_cut = INACTIVE_CUTOFF;
#endif

#ifdef MORSE
  params->MORSE_eps = params->MORSE_alpha = params->MORSE_rmin =
      params->MORSE_rest = params->MORSE_cut = INACTIVE_CUTOFF;
#endif

#ifdef BUCKINGHAM
  params->BUCK_A = params->BUCK_B = params->BUCK_C = params->BUCK_D =
      params->BUCK_discont = params->BUCK_shift = params->BUCK_F1 =
          params->BUCK_F2 = 0.0;
  params->BUCK_cut = INACTIVE_CUTOFF;
#endif

#ifdef SOFT_SPHERE
  params->soft_a = params->soft_n = params->soft_offset = 0.0;
  params->soft_cut = INACTIVE_CUTOFF;
#endif

#ifdef AFFINITY
  params->affinity_type = params->affinity_kappa = params->affinity_r0 =
      params->affinity_Kon = params->affinity_Koff = params->affinity_maxBond =
          params->affinity_cut = INACTIVE_CUTOFF;
#endif

#ifdef MEMBRANE_COLLISION
  params->membrane_a = params->membrane_n = params->membrane_offset = 0.0;
  params->membrane_cut = INACTIVE_CUTOFF;
#endif

#ifdef HAT
  params->HAT_Fmax = params->HAT_r = INACTIVE_CUTOFF;
#endif

#ifdef LJCOS
  params->LJCOS_eps = params->LJCOS_sig = params->LJCOS_offset =
      params->LJCOS_alfa = params->LJCOS_beta = params->LJCOS_rmin = 0.0;
  params->LJCOS_cut = INACTIVE_CUTOFF;
#endif

#ifdef LJCOS2
  params->LJCOS2_eps = params->LJCOS2_sig = params->LJCOS2_offset =
      params->LJCOS2_w = params->LJCOS2_rchange = params->LJCOS2_cut =
          INACTIVE_CUTOFF;
#endif

#ifdef COS2
  params->COS2_eps = params->COS2_offset = params->COS2_w = params->COS2_cut =
      INACTIVE_CUTOFF;
#endif

#ifdef GAY_BERNE
  params->GB_eps = params->GB_sig = params->GB_k1 = params->GB_k2 =
      params->GB_mu = params->GB_nu = params->GB_chi1 = params->GB_chi2 = 0.0;
  params->GB_cut = INACTIVE_CUTOFF;
#endif

#ifdef TABULATED
  params->TAB_npoints = params->TAB_startindex = 0;
  params->TAB_minval = params->TAB_stepsize = 0.0;
...
const char *get_name_of_bonded_ia(BondedInteraction type) {
  ...
  default:
    fprintf(stderr,
            "%d: INTERNAL ERROR: name of unknown interaction %d requested\n",
            this_node, type);
    errexit();
  }
