...
void nemd_init(int n_slabs, int n_exchange, double shear_rate) {
  ...
  nemddata.velocity_profile =
      (double *)Utils::malloc(nemddata.n_slabs * sizeof(double));
  ...
  if (nemddata.n_exchange > 0) {
    nemddata.slab[nemddata.top_slab].fastest =
        (int *)Utils::malloc(nemddata.n_exchange * sizeof(int));
    nemddata.slab[nemddata.mid_slab].fastest =
        (int *)Utils::malloc(nemddata.n_exchange * sizeof(int));
  }
...
void nemd_change_momentum() {
  ...
  if (nemd_method == NEMD_METHOD_EXCHANGE) {
    ...
    for (i = 0; i < nemddata.n_exchange; i++) {
      ...
      nemddata.momentum += local_particles[top_slab->fastest[i]]->e->m.v[0];
      nemddata.momentum -= local_particles[mid_slab->fastest[i]]->e->m.v[0];
      tmp_v0 = local_particles[mid_slab->fastest[i]]->e->m.v[0];
      local_particles[mid_slab->fastest[i]]->e->m.v[0] =
          local_particles[top_slab->fastest[i]]->e->m.v[0];
      local_particles[top_slab->fastest[i]]->e->m.v[0] = tmp_v0;
    }
...
void nemd_store_velocity_profile() {
  ...
  for (i = 0; i < nemddata.n_slabs; i++) {
    if (nemddata.slab[i].n_parts_in_slab == 0)
      fprintf(stderr, "Zero particles in slab %d!!!\n", i);
    nemddata.velocity_profile[i] +=
        nemddata.slab[i].v_mean / (double)nemddata.slab[i].n_parts_in_slab;
    nemddata.slab[i].v_mean = 0.0;
    nemddata.slab[i].n_parts_in_slab = 0;
  }
