...
inline int calc_thermalized_bond_forces(Particle *p1, Particle *p2,
  ...
  double mass_tot = p1->e->p.mass + p2->e->p.mass;
  double mass_tot_inv = 1.0 / mass_tot;
  double sqrt_mass_tot = sqrt(mass_tot);
  double sqrt_mass_red = sqrt(p1->e->p.mass * p2->e->p.mass / mass_tot);

  for (int i = 0; i < 3; i++) {

    // Langevin thermostat for center of mass
    com_vel =
        mass_tot_inv * (p1->e->p.mass * p1->e->m.v[i] + p2->e->p.mass * p2->e->m.v[i]);
    ...
    dist_vel = p2->e->m.v[i] - p1->e->m.v[i];
    ...
    // Add forces
    force1[i] = p1->e->p.mass * mass_tot_inv * force_lv_com - force_lv_dist;
    force2[i] = p2->e->p.mass * mass_tot_inv * force_lv_com + force_lv_dist;
  }
