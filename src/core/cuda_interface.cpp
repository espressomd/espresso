...
static void pack_particles(ParticleRange particles,
  ...
  for (auto const &part : particles) {
...
#ifdef LB_GPU
    buffer[i].v[0] = static_cast<float>(part.e->m.v[0]);
    buffer[i].v[1] = static_cast<float>(part.e->m.v[1]);
    buffer[i].v[2] = static_cast<float>(part.e->m.v[2]);
#ifdef VIRTUAL_SITES
    buffer[i].is_virtual = part.e->p.is_virtual;
    ...
    for (int ii = 0; ii < 2 * LB_COMPONENTS; ii++) {
      buffer[i].solvation[ii] = static_cast<float>(part.e->p.solvation[ii]);
    }
#endif

#ifdef LB_ELECTROHYDRODYNAMICS
    buffer[i].mu_E[0] = static_cast<float>(part.e->p.mu_E[0]);
    buffer[i].mu_E[1] = static_cast<float>(part.e->p.mu_E[1]);
    buffer[i].mu_E[2] = static_cast<float>(part.e->p.mu_E[2]);
#endif
...
#ifdef MASS
    buffer[i].mass = static_cast<float>(part.e->p.mass);
#endif
...
#ifdef ENGINE
    buffer[i].swim.v_swim = static_cast<float>(part.e->swim.v_swim);
    buffer[i].swim.f_swim = static_cast<float>(part.e->swim.f_swim);
    ...
    buffer[i].swim.push_pull = part.e->swim.push_pull;
    buffer[i].swim.dipole_length = static_cast<float>(part.e->swim.dipole_length);
#endif
    buffer[i].swim.swimming = part.e->swim.swimming;
#endif
    i++;
  }
}
...
void set_v_cs(ParticleRange particles, CUDA_v_cs *v_cs) {
  for (auto &p : particles) {
    for (int i = 0; i < 3; i++) {
      p.e->swim.v_center[i] = v_cs->v_cs[0 + i];
      p.e->swim.v_source[i] = v_cs->v_cs[3 + i];
    }
    v_cs++;
  }
}
