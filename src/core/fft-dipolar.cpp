...
void dfft_pre_init() { fft_common_pre_init(&dfft); }
...
int dfft_init(double **data, int *local_mesh_dim, int *local_mesh_margin,
  ...
  dfft.max_comm_size = 0;
  dfft.max_mesh_size = 0;
  for (i = 0; i < 4; i++) {
  ...
  for (i = 1; i < 4; i++)
    dfft.plan[i].n_permute = (dfft.plan[1].row_dir + i) % 3;
  for (i = 0; i < 3; i++) {
  ...
  for (i = 1; i < 4; i++) {
    ...
    dfft.plan[i].new_size = fft_calc_local_mesh(
        ...
        dfft.plan[i].new_mesh, dfft.plan[i].start);
    ...
    for (j = 0; j < dfft.plan[i].g_size; j++) {
      ...
      permute_ifield(&(dfft.plan[i].send_block[6 * j]), 3,
                     -(dfft.plan[i - 1].n_permute));
      permute_ifield(&(dfft.plan[i].send_block[6 * j + 3]), 3,
                     -(dfft.plan[i - 1].n_permute));
      ...
      dfft.plan[i].recv_size[j] = fft_calc_send_block(
          my_pos[i], n_grid[i], &(n_pos[i - 1][3 * node]), n_grid[i - 1],
          global_mesh_dim, global_mesh_off, &(dfft.plan[i].recv_block[6 * j]));
      permute_ifield(&(dfft.plan[i].recv_block[6 * j]), 3,
                     -(dfft.plan[i].n_permute));
      permute_ifield(&(dfft.plan[i].recv_block[6 * j + 3]), 3,
                     -(dfft.plan[i].n_permute));
    ...
    for (j = 0; j < 3; j++)
      dfft.plan[i].old_mesh[j] = dfft.plan[i - 1].new_mesh[j];
    if (i == 1)
  ...
  for (i = 1; i < 4; i++)
    if (2 * dfft.plan[i].new_size > dfft.max_mesh_size)
      dfft.max_mesh_size = 2 * dfft.plan[i].new_size;
...
void dfft_forw_grid_comm(fft_forw_plan plan, double *in, double *out) {
  ...
  for (i = 0; i < plan.g_size; i++) {
    ...
    if (plan.group[i] < this_node) { /* send first, receive second */
      MPI_Send(dfft.send_buf, plan.send_size[i], MPI_DOUBLE, plan.group[i],
               REQ_FFT_FORW, comm_cart);
      MPI_Recv(dfft.recv_buf, plan.recv_size[i], MPI_DOUBLE, plan.group[i],
...
void dfft_back_grid_comm(fft_forw_plan plan_f, fft_back_plan plan_b, double *in,
  ...
  for (i = 0; i < plan_f.g_size; i++) {
    ...
    if (plan_f.group[i] < this_node) { /* send first, receive second */
      MPI_Send(dfft.send_buf, plan_f.recv_size[i], MPI_DOUBLE, plan_f.group[i],
               REQ_FFT_BACK, comm_cart);
      MPI_Recv(dfft.recv_buf, plan_f.send_size[i], MPI_DOUBLE, plan_f.group[i],
