...
/** \file RuntimeErrorCollector_test.cpp Unit tests for the ErrorHandling::RuntimeErrorCollector class.
 *
*/
...
BOOST_AUTO_TEST_CASE(gather) {
  ...
  if(world.rank() == 0) {
    ...
    BOOST_CHECK(std::all_of(present.begin(), present.end(), [](int i) { return i == 2; }));
    /** Count warnings, should be world.rank() many */
    BOOST_CHECK(std::count_if(results.begin(), results.end(), [](const RuntimeError &e) {
