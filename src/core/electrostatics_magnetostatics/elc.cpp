...
static void add_dipole_force() {
  ...
  for (auto const &p : local_cells.particles()) {
    gblcblk[0] += p.q * (p.r.p[2] - shift);
    gblcblk[1] += p.q * p.r.p[2];
    gblcblk[2] += p.q;

    if (elc_params.dielectric_contrast_on) {
      if (p.r.p[2] < elc_params.space_layer) {
        gblcblk[0] += elc_params.delta_mid_bot * p.q * (-p.r.p[2] - shift);
        gblcblk[2] += elc_params.delta_mid_bot * p.q;
      }
      if (p.r.p[2] > (elc_params.h - elc_params.space_layer)) {
        gblcblk[0] += elc_params.delta_mid_top * p.q *
                      (2 * elc_params.h - p.r.p[2] - shift);
        gblcblk[2] += elc_params.delta_mid_top * p.q;
      }
    }
  }
...
static double dipole_energy() {
  ...
  for (auto &p : local_cells.particles()) {
    gblcblk[0] += p.q;
    gblcblk[2] += p.q * (p.r.p[2] - shift);
    gblcblk[4] += p.q * (Utils::sqr(p.r.p[2] - shift));
    gblcblk[6] += p.q * p.r.p[2];

    if (elc_params.dielectric_contrast_on) {
      if (p.r.p[2] < elc_params.space_layer) {
        gblcblk[1] += elc_params.delta_mid_bot * p.q;
        gblcblk[3] += elc_params.delta_mid_bot * p.q * (-p.r.p[2] - shift);
        gblcblk[5] +=
            elc_params.delta_mid_bot * p.q * (Utils::sqr(-p.r.p[2] - shift));
      }
      if (p.r.p[2] > (elc_params.h - elc_params.space_layer)) {
        gblcblk[1] += elc_params.delta_mid_top * p.q;
        gblcblk[3] += elc_params.delta_mid_top * p.q *
                      (2 * elc_params.h - p.r.p[2] - shift);
        gblcblk[5] += elc_params.delta_mid_top * p.q *
  ...
  if (elc_params.dielectric_contrast_on) {
    if (elc_params.const_pot) {
      ...
      for (auto &p : local_cells.particles()) {
        ...
        if (elc_params.dielectric_contrast_on) {
          if (p.r.p[2] < elc_params.space_layer) {
            gblcblk[2] += fac_delta * (elc_params.delta_mid_bot + 1) * p.q;
            gblcblk[3] +=
                p.q * (image_sum_b(elc_params.delta_mid_bot * delta,
                                     -(2 * elc_params.h + p.r.p[2])) +
                         image_sum_b(delta, -(2 * elc_params.h - p.r.p[2])));
          } else {
            gblcblk[2] +=
                fac_delta_mid_bot * (1 + elc_params.delta_mid_top) * p.q;
            gblcblk[3] +=
                p.q * (image_sum_b(elc_params.delta_mid_bot, -p.r.p[2]) +
                         image_sum_b(delta, -(2 * elc_params.h - p.r.p[2])));
          }
          if (p.r.p[2] > (elc_params.h - elc_params.space_layer)) {
            // note the minus sign here which is required due to |z_i-z_j|
            gblcblk[2] -= fac_delta * (elc_params.delta_mid_top + 1) * p.q;
            gblcblk[3] -=
                p.q * (image_sum_t(elc_params.delta_mid_top * delta,
                                     4 * elc_params.h - p.r.p[2]) +
                         image_sum_t(delta, 2 * elc_params.h + p.r.p[2]));
          } else {
            // note the minus sign here which is required due to |z_i-z_j|
            gblcblk[2] -=
                fac_delta_mid_top * (1 + elc_params.delta_mid_bot) * p.q;
            gblcblk[3] -=
                p.q * (image_sum_t(elc_params.delta_mid_top,
                                     2 * elc_params.h - p.r.p[2]) +
                         image_sum_t(delta, 2 * elc_params.h + p.r.p[2]));
          }
        }
...
static void add_z_force() {
  ...
  if (elc_params.dielectric_contrast_on) {
    ...
    if (elc_params.const_pot) {
      ...
      for (auto &p : local_cells.particles()) {
        if (p.r.p[2] < elc_params.space_layer)
          gblcblk[0] -= elc_params.delta_mid_bot * p.q;
        if (p.r.p[2] > (elc_params.h - elc_params.space_layer))
          gblcblk[0] += elc_params.delta_mid_top * p.q;
      }
    } else {
      ...
      for (auto &p : local_cells.particles()) {
        if (p.r.p[2] < elc_params.space_layer) {
          gblcblk[0] += fac_delta * (elc_params.delta_mid_bot + 1) * p.q;
        } else {
          gblcblk[0] +=
              fac_delta_mid_bot * (1 + elc_params.delta_mid_top) * p.q;
        }
        ...
        if (p.r.p[2] > (elc_params.h - elc_params.space_layer)) {
          // note the minus sign here which is required due to |z_i-z_j|
          gblcblk[0] -= fac_delta * (elc_params.delta_mid_top + 1) * p.q;
        } else {
          // note the minus sign here which is required due to |z_i-z_j|
          gblcblk[0] -=
              fac_delta_mid_top * (1 + elc_params.delta_mid_bot) * p.q;
...
static void setup_P(int p, double omega) {
  ...
  for (auto &p : local_cells.particles()) {
    ...
    partblk[size * ic + POQESM] = p.q * scxcache[o + ic].s / e;
    partblk[size * ic + POQESP] = p.q * scxcache[o + ic].s * e;
    partblk[size * ic + POQECM] = p.q * scxcache[o + ic].c / e;
    partblk[size * ic + POQECP] = p.q * scxcache[o + ic].c * e;
    ...
    if (elc_params.dielectric_contrast_on) {
      if (p.r.p[2] < elc_params.space_layer) { // handle the lower case first
        ...
        scale = p.q * elc_params.delta_mid_bot;
      ...
      lclimge[POQESP] += p.q * scxcache[o + ic].s * e;
      lclimge[POQECP] += p.q * scxcache[o + ic].c * e;
      ...
      if (p.r.p[2] > (elc_params.h -
        ...
        scale = p.q * elc_params.delta_mid_top;
      ...
      lclimge[POQESM] += p.q * scxcache[o + ic].s * e;
      lclimge[POQECM] += p.q * scxcache[o + ic].c * e;
    }
...
static void setup_Q(int q, double omega) {
  ...
  for (auto &p : local_cells.particles()) {
    ...
    partblk[size * ic + POQESM] = p.q * scycache[o + ic].s / e;
    partblk[size * ic + POQESP] = p.q * scycache[o + ic].s * e;
    partblk[size * ic + POQECM] = p.q * scycache[o + ic].c / e;
    partblk[size * ic + POQECP] = p.q * scycache[o + ic].c * e;
    ...
    if (elc_params.dielectric_contrast_on) {
      ...
      lclimge[POQESP] += p.q * scycache[o + ic].s * e;
      lclimge[POQECP] += p.q * scycache[o + ic].c * e;
      ...
      lclimge[POQESM] += p.q * scycache[o + ic].s * e;
      lclimge[POQECM] += p.q * scycache[o + ic].c * e;
    }
...
static void setup_PQ(int p, int q, double omega) {
  ...
  for (auto &p : local_cells.particles()) {
    ...
    partblk[size * ic + PQESSM] =
        scxcache[ox + ic].s * scycache[oy + ic].s * p.q / e;
    partblk[size * ic + PQESCM] =
        scxcache[ox + ic].s * scycache[oy + ic].c * p.q / e;
    partblk[size * ic + PQECSM] =
        scxcache[ox + ic].c * scycache[oy + ic].s * p.q / e;
    partblk[size * ic + PQECCM] =
        scxcache[ox + ic].c * scycache[oy + ic].c * p.q / e;
...
void ELC_P3M_self_forces() {
  ...
  for (auto &p : local_cells.particles()) {
    if (p.r.p[2] < elc_params.space_layer) {
      q = elc_params.delta_mid_bot * p.q * p.q;
      pos[0] = p.r.p[0];
      pos[1] = p.r.p[1];
      pos[2] = -p.r.p[2];
    ...
    if (p.r.p[2] > (elc_params.h - elc_params.space_layer)) {
      q = elc_params.delta_mid_top * p.q * p.q;
      pos[0] = p.r.p[0];
      pos[1] = p.r.p[1];
...
void ELC_p3m_charge_assign_both() {
  ...
  for (auto &p : local_cells.particles()) {
    if (p.q != 0.0) {
      p3m_assign_charge(p.q, p.r.p, cp_cnt);

      if (p.r.p[2] < elc_params.space_layer) {
        double q = elc_params.delta_mid_bot * p.q;
        pos[0] = p.r.p[0];
        pos[1] = p.r.p[1];
        pos[2] = -p.r.p[2];
      ...
      if (p.r.p[2] > (elc_params.h - elc_params.space_layer)) {
        double q = elc_params.delta_mid_top * p.q;
        pos[0] = p.r.p[0];
        pos[1] = p.r.p[1];
        pos[2] = 2 * elc_params.h - p.r.p[2];
        p3m_assign_charge(q, pos, -1);
...
void ELC_P3M_dielectric_layers_force_contribution(Particle *p1, Particle *p2,
  ...
  if (p1->r.p[2] < elc_params.space_layer) {
    q = elc_params.delta_mid_bot * p1->q * p2->q;
    pos[0] = p1->r.p[0];
    pos[1] = p1->r.p[1];
    pos[2] = -p1->r.p[2];
  ...
  if (p1->r.p[2] > (elc_params.h - elc_params.space_layer)) {
    q = elc_params.delta_mid_top * p1->q * p2->q;
    pos[0] = p1->r.p[0];
    pos[1] = p1->r.p[1];
  ...
  for (auto &p : local_cells.particles()) {
    if (p.q != 0.0) {
      ...
      if (p.r.p[2] < elc_params.space_layer) {
        ...
        node_sums[0] += 1.0;
        node_sums[1] += Utils::sqr(elc_params.delta_mid_bot * p.q);
        node_sums[2] += elc_params.delta_mid_bot * p.q;
      }
      if (p.r.p[2] > (elc_params.h - elc_params.space_layer)) {

        node_sums[0] += 1.0;
        node_sums[1] += Utils::sqr(elc_params.delta_mid_top * p.q);
        node_sums[2] += elc_params.delta_mid_top * p.q;
      }
    }
  }
