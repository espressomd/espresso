...
void dp3m_dipole_assign(void) {
  ...
  void dp3m_assign_dipole(double const real_pos[3], double mu,
    ...
    /* we do not really want to export these, but this function should be
     * inlined
     */
    ...
    if (dp3m.params.inter == 0) {
      for (d = 0; d < 3; d++) {
        ...
        if ((nmp + dp3m.params.cao) > dp3m.local_mesh.dim[d]) {
          fprintf(stderr,
                  "%d: dipolar dp3m.rs_mesh overflow! (pos %f, nmp=%d)\n",
      ...
      for (d = 0; d < 3; d++) {
        ...
        /* for the first dimension, q_ind is always zero, so this shifts
         * correctly
         */
  ...
  double dp3m_calc_kspace_forces(int force_flag, int energy_flag) {
    ...
    dipole_prefac = coulomb.Dprefactor /
                    (double)(dp3m.params.mesh[0] * dp3m.params.mesh[1] *
                             dp3m.params.mesh[2]);
      ...
      **********************/
      if (dp3m.sum_mu2 > 0) {
        ...
        if (this_node == 0) {
          ...
          k_space_energy_dip +=
              coulomb.Dprefactor * dp3m.energy_correction /
    ...
    if (force_flag) {
      ...
      if (dp3m.sum_mu2 > 0) {
#ifdef ROTATION
        P3M_TRACE(fprintf(
            stderr, "%d: dipolar p3m start torques calculation: k-space\n",
            this_node));
        ...
        for (d = 0; d < 3; d++) {
          ...
          for (j[0] = 0; j[0] < dp3m.fft.plan[3].new_mesh[0]; j[0]++) {
            for (j[1] = 0; j[1] < dp3m.fft.plan[3].new_mesh[1]; j[1]++) {
              for (j[2] = 0; j[2] < dp3m.fft.plan[3].new_mesh[2]; j[2]++) {
                dp3m.rs_mesh[ind] =
                    dp3m.d_op[j[d] + dp3m.fft.plan[3].start[d]] *
                    dp3m.ks_mesh[ind];
                ind++;
                dp3m.rs_mesh[ind] =
                    dp3m.d_op[j[d] + dp3m.fft.plan[3].start[d]] *
        ...
        ****************************/
        ...
        // Compute forces after torques because the algorithm below overwrites
        // the grids dp3m.rs_mesh_dip ! Note: I'll do here 9 inverse FFTs. By
        // symmetry, we can reduce this number to 6 !
        ...
        for (d = 0; d < 3; d++) { /* direction in k-space: */
          ...
          /* Assign force component from mesh to particle */
          dp3m_assign_forces_dip(dipole_prefac * pow(2 * PI / box_l[0], 2),
                                 d_rs);
        }
  ...
  double calc_surface_term(int force_flag, int energy_flag) {
    const double pref = coulomb.Dprefactor * 4 * M_PI * box_l_i[0] *
                        box_l_i[1] * box_l_i[2] / (2 * dp3m.params.epsilon + 1);
    double suma, a[3];
    double en;
  ...
  void dp3m_spread_force_grid(double *themesh) {
    ...
    double *tmp_ptr;
    P3M_TRACE(
        fprintf(stderr, "%d: dipolar p3m_spread_force_grid:\n", this_node));
  ...
  void dp3m_calc_influence_function_force() {
    ...
    fak1 = dp3m.params.mesh[0] * dp3m.params.mesh[0] * dp3m.params.mesh[0] *
           2.0 / (box_l[0] * box_l[0]);
    ...
    double dp3m_perform_aliasing_sums_force(const int n[3],
                                            double nominator[1]) {
    ...
    void dp3m_calc_influence_function_energy() {
        ...
        for (n[1] = dp3m.fft.plan[3].start[1]; n[1] < end[1]; n[1]++) {
          for (n[2] = dp3m.fft.plan[3].start[2]; n[2] < end[2]; n[2]++) {
            ...
            if ((n[0] == 0) && (n[1] == 0) && (n[2] == 0)) {
              ...
              fak2 = nominator[0];
              fak2 /= pow(Utils::sqr(dp3m.d_op[n[0]]) +
      ...
      double dp3m_perform_aliasing_sums_energy(const int n[3],
                                               double nominator[1]) {
        double denominator = 0.0;
        /* lots of temporary variables... */
        double sx, sy, sz, f1, f2, f3, mx, my, mz, nmx, nmy, nmz, nm2, expo;
...
#define P3M_TUNE_MAX_CUTS 50
       ...
       */
      double dp3m_get_accuracy(int mesh, int cao, double r_cut_iL,
                               double *_alpha_L, double *_rs_err,
                               double *_ks_err) {
        double rs_err, ks_err;
        double alpha_L;
        P3M_TRACE(fprintf(stderr,
                          "dp3m_get_accuracy: mesh %d, cao %d, r_cut %f ", mesh,
                          cao, r_cut_iL));

        /* calc maximal real space error for setting */

        // Alpha cannot be zero in the dipolar case because real_space formula
        // breaks down
        rs_err = P3M_DIPOLAR_real_space_error(box_l[0], coulomb.Dprefactor,
                                              r_cut_iL, dp3m.sum_dip_part,
                                              dp3m.sum_mu2, 0.001);

        if (M_SQRT2 * rs_err > dp3m.params.accuracy) {
          /* assume rs_err = ks_err -> rs_err = accuracy/sqrt(2.0) -> alpha_L */
          alpha_L = dp3m_rtbisection(box_l[0], coulomb.Dprefactor, r_cut_iL,
                                     dp3m.sum_dip_part, dp3m.sum_mu2,
                                     0.0001 * box_l[0], 5.0 * box_l[0], 0.0001,
      ...
      static double dp3m_mc_time(char **log, int mesh, int cao,
        ...
        P3M_TRACE(fprintf(stderr,
                          "dp3m_mc_time: mesh=%d, cao=%d, rmin=%f, rmax=%f\n",
        ...
        /* Either low and high boundary are equal (for fixed cut), or the low
           border is initially 0 and therefore has infinite error estimate, as
           required. Therefore if the high boundary
        ...
        for (;;) {
          ...
          /* bisection */
          if (dp3m_get_accuracy(mesh, cao, r_cut_iL, _alpha_L, &rs_err,
                                &ks_err) > dp3m.params.accuracy) {
        ...
        *_r_cut_iL = r_cut_iL = r_cut_iL_max;
        ...
        /* check whether we are running P3M+DLC, and whether we leave a
         * reasonable gap space */
        if (coulomb.Dmethod == DIPOLAR_MDLC_P3M) {
          runtimeErrorMsg() << "dipolar P3M: tuning when dlc needs to be fixed";
        }

        /* check whether this radius is too large, so that we would use less
         * cells than allowed */
        n_cells = 1;
        for (i = 0; i < 3; i++) {
          n_cells *=
              (int)(floor(local_box_l[i] / (r_cut_iL * box_l[0] + skin)));
        }
        if (n_cells < min_num_cells) {
          P3M_TRACE(fprintf(
              stderr,
              "dp3m_mc_time: mesh %d cao %d r_cut %f reject n_cells %d\n", mesh,
              cao, r_cut_iL, n_cells));
          /* print result */
          sprintf(
              b,
              "%-4d %-3d %.5e %.5e %.5e %.3e %.3e radius dangerously high\n\n",
              mesh, cao, r_cut_iL_max, *_alpha_L, *_accuracy, rs_err, ks_err);
          *log = strcat_alloc(*log, b);
        ...
        if (int_time == -P3M_TUNE_FAIL) {
          *log = strcat_alloc(*log,
                              "tuning failed, test integration not possible\n");
          return int_time;
        }
        ...
        *_accuracy =
        ...
        P3M_TRACE(fprintf(stderr,
                          "dp3m_mc_time: mesh %d cao %d r_cut %f time %f\n",
                          mesh, cao, r_cut_iL, int_time));
        /* print result */
        sprintf(b, "%-4d %-3d %.5e %.5e %.5e %.3e %.3e %-8d\n", mesh, cao,
                r_cut_iL, *_alpha_L, *_accuracy, rs_err, ks_err, (int)int_time);
        *log = strcat_alloc(*log, b);
        return int_time;
      }
       ...
       *
       *  @p _cao should contain an initial guess, which is then adapted by
      ...
      static double dp3m_m_time(char **log, int mesh, int cao_min, int cao_max,
        ...
        P3M_TRACE(fprintf(stderr,
                          "dp3m_m_time: Dmesh=%d, Dcao_min=%d, Dcao_max=%d, "
                          "Drmin=%f, Drmax=%f\n",
                          mesh, cao_min, cao_max, r_cut_iL_min, r_cut_iL_max));
        /* the initial step sets a timing mark. If there is no valid r_cut, we
           can only try
           to increase cao to increase the obtainable precision of the far
           formula. */
        double tmp_time;
        do {
          ...
          /* the required accuracy could not be obtained, try higher caos.
             Therefore optimisation can only be
        ...
        if (final_dir == 0) {
          /* check in which direction we can optimise. Both directions are
           * possible */
          double dir_times[3];
          for (final_dir = -1; final_dir <= 1; final_dir += 2) {
            dir_times[final_dir + 1] = tmp_time = dp3m_mc_time(
                log, mesh, cao + final_dir, r_cut_iL_min, r_cut_iL_max,
                &tmp_r_cut_iL, &tmp_alpha_L, &tmp_accuracy);
            /* bail out on errors, as usual */
            if (tmp_time == -P3M_TUNE_FAIL) {
              return tmp_time;
            }
            /* in this direction, we cannot optimise, since we get into
             * precision trouble */
            if (tmp_time < 0) {
              continue;
            }
          ...
          if (dir_times[0] == best_time) {
            ...
            /* no improvement in either direction, however if one is only
             * marginally worse, we can still try*/
            /* down is possible and not much worse, while up is either illegal
             * or even worse */
            if ((dir_times[0] >= 0 &&
                 dir_times[0] < best_time + P3M_TIME_GRAN) &&
        ...
        /* move cao into the optimisation direction until we do not gain
         * anymore. */
      ...
      int dp3m_adaptive_tune(char **logger) {
        /** Tuning of dipolar P3M. The algorithm basically determines the mesh,
         * cao and then the real space cutoff, in this nested order.
         *
         *  For each mesh, the cao optimal for the mesh tested previously is
         * used as an initial guess, and the algorithm tries whether increasing
         * or decreasing it leads to a better solution. This is efficient, since
         * the optimal cao only changes little with the meshes in general.
         *
         ...
         *  estimate equals the required accuracy. Therefore the smallest
         * possible, i.e. fastest real space cutoff is determined.
         *
         *  Both the search over mesh and cao stop to search in a specific
         * direction once the computation time is significantly higher than the
         * currently known optimum.
         */
        ...
        *logger = strcat_alloc(*logger, b);
        ...
        if (dp3m.sum_dip_part == 0) {
          *logger = strcat_alloc(
              *logger,
              "no dipolar particles in the system, cannot tune dipolar P3M");
          return ES_ERROR;
        }
        ...
        if (dp3m.params.mesh[0] == 0) {
          ...
          /* avoid using more than 1 GB of FFT arrays (per default, see
           * config.hpp) */
        ...
        *logger =
            strcat_alloc(*logger, "Dmesh cao Dr_cut_iL   Dalpha_L     Derr     "
                                  "    Drs_err    Dks_err    time [ms]\n");

        /* mesh loop */
        for (; tmp_mesh <= mesh_max; tmp_mesh += 2) {
          tmp_cao = cao;
          tmp_time = dp3m_m_time(logger, tmp_mesh, cao_min, cao_max, &tmp_cao,
          ...
          /* the optimum r_cut for this mesh is the upper limit for higher
             meshes, everything else is slower */
      ...
      void p3m_print_dp3m_struct(p3m_parameter_struct ps) {
        ...
        fprintf(stderr, "   mesh=(%d,%d,%d), mesh_off=(%.4f,%.4f,%.4f)\n",
                ps.mesh[0], ps.mesh[1], ps.mesh[2], ps.mesh_off[0],
                ps.mesh_off[1], ps.mesh_off[2]);
        fprintf(stderr, "   Dcao=%d, Dinter=%d, Depsilon=%f\n", ps.cao,
                ps.inter, ps.epsilon);
        fprintf(stderr, "   Dcao_cut=(%f,%f,%f)\n", ps.cao_cut[0],
                ps.cao_cut[1], ps.cao_cut[2]);
      ...
      /* Following are the two functions for computing the error in dipolar P3M
         and tune the parameters to minimize the time with the desired accuracy.
      ...
      */
      ...
      static double dp3m_k_space_error(double box_size, double prefac, int mesh,
        ...
        for (nx = -mesh / 2; nx < mesh / 2; nx++) {
          for (ny = -mesh / 2; ny < mesh / 2; ny++) {
            for (nz = -mesh / 2; nz < mesh / 2; nz++) {
              if ((nx != 0) || (ny != 0) || (nz != 0)) {
                ...
                dp3m_tune_aliasing_sums(nx, ny, nz, mesh, mesh_i, cao,
                                        alpha_L_i, &alias1, &alias2);
                double d = alias1 - Utils::sqr(alias2 / cs) / (n2 * n2 * n2);
                /* at high precisions, d can become negative due to extinction;
                   also, don't take values that have no significant digits
                   left*/
          ...
          void dp3m_tune_aliasing_sums(int nx, int ny, int nz, int mesh,
                                       double mesh_i, int cao, double alpha_L_i,
                                       double *alias1, double *alias2) {
            int mx, my, mz;
            double nmx, nmy, nmz;
            ...
            for (mx = -P3M_BRILLOUIN; mx <= P3M_BRILLOUIN; mx++) {
              ...
              for (my = -P3M_BRILLOUIN; my <= P3M_BRILLOUIN; my++) {
                ...
                for (mz = -P3M_BRILLOUIN; mz <= P3M_BRILLOUIN; mz++) {
                  ...
                  *alias1 += ex2 * nm2;
                  *alias2 +=
                      U2 * ex * pow((nx * nmx + ny * nmy + nz * nmz), 3.) / nm2;
                }
              }
            }
          }
          ...
          //  for the REAL part of the force in terms of the splitting parameter
          //  alpha of Ewald based on the formulas 33, the paper of
          //  Zuowei-HolmJCP, 115,6351,(2001).
          ...
          double P3M_DIPOLAR_real_space_error(double box_size, double prefac,
            ...
            d_cc = 4.0 * d_a2 * d_a2 * d_rcut2 * d_rcut2 +
                   6.0 * d_a2 * d_rcut2 + 3.0;

            d_dc = 8.0 * d_a2 * d_a2 * d_a2 * d_rcut2 * d_rcut2 * d_rcut2 +
                   20.0 * d_a2 * d_a2 * d_rcut2 * d_rcut2 +
                   30 * d_a2 * d_rcut2 + 15.0;

            d_con = 1.0 / sqrt(box_size * box_size * box_size * d_a2 * d_a2 *
                               d_rcut2 * d_rcut2 * d_rcut2 * d_rcut2 * d_RCUT *
                               (double)n_c_part);
          ...
          // "func-tuned_accuracy/sqrt(2.)" known to lie between x1 and x2. The
          // root, returned as rtbis, will be refined until its accuracy is
          // +-xacc.

          double dp3m_rtbisection(double box_size, double prefac,
                                  double r_cut_iL, int n_c_part, double sum_q2,
            ...
            f = P3M_DIPOLAR_real_space_error(box_size, prefac, r_cut_iL,
                                             n_c_part, sum_q2, x1) -
                constant;
            fmid = P3M_DIPOLAR_real_space_error(box_size, prefac, r_cut_iL,
                                                n_c_part, sum_q2, x2) -
                   constant;
            if (f * fmid >= 0.0) {
              fprintf(
          ...
          void dp3m_calc_local_ca_mesh() {
            ...
            for (i = 0; i < 3; i++) {
              full_skin[i] = dp3m.params.cao_cut[i] + skin +
                             dp3m.params.additional_mesh[i];
            }

            /* inner left down grid point (global index) */
            for (i = 0; i < 3; i++) {
              dp3m.local_mesh.in_ld[i] = (int)ceil(
                  my_left[i] * dp3m.params.ai[i] - dp3m.params.mesh_off[i]);
            }
            /* inner up right grid point (global index) */
            for (i = 0; i < 3; i++) {
              dp3m.local_mesh.in_ur[i] = (int)floor(
                  my_right[i] * dp3m.params.ai[i] - dp3m.params.mesh_off[i]);
            }
            ...
            for (i = 0; i < 3; i++) {
              ind[i] =
                  (int)floor((my_right[i] + full_skin[i]) * dp3m.params.ai[i] -
                             dp3m.params.mesh_off[i]);
            }
            ...
            for (i = 0; i < 3; i++) {
              ...
              /* up right margin */
              for (i = 0; i < 3; i++) {
                dp3m.local_mesh.margin[(i * 2) + 1] =
                    ind[i] - dp3m.local_mesh.in_ur[i];
              }
              ...
              dp3m.local_mesh.q_2_off =
                  dp3m.local_mesh.dim[2] - dp3m.params.cao;
              dp3m.local_mesh.q_21_off =
                  dp3m.local_mesh.dim[2] *
                  (dp3m.local_mesh.dim[1] - dp3m.params.cao);
            }
            ...
            bool dp3m_sanity_checks_boxl() {
              ...
              for (i = 0; i < 3; i++) {
                ...
                if (dp3m.params.cao_cut[i] >= 0.5 * box_l[i]) {
                  runtimeErrorMsg()
                      << "dipolar P3M_init: k-space cutoff "
            ...
            bool dp3m_sanity_checks(const Vector3i &grid) {
              ...
              if (cell_structure.type != CELL_STRUCTURE_DOMDEC) {
                runtimeErrorMsg()
                    << "dipolar P3M at present requires the domain "
              ...
              if (dp3m.params.mesh[0] == 0) {
                runtimeErrorMsg()
                    << "dipolar P3M_init: mesh size is not yet set";
                ret = true;
              }
              if (dp3m.params.cao == 0) {
              ...
              if (grid[0] < grid[1] || grid[1] < grid[2]) {
                runtimeErrorMsg() << "dipolar P3M_init: node grid must be "
            ...
            void dp3m_calc_send_mesh() {
              ...
              for (i = 0; i < 6; i++) {
                ...
                if (node_neighbors[i] != this_node) {
                  /* two step communication: first all even positions than all
                   * odd */
                  for (evenodd = 0; evenodd < 2; evenodd++) {
                    if ((node_pos[i / 2] + evenodd) % 2 == 0) {
                      ...
                      MPI_Recv(&(dp3m.local_mesh.r_margin[j]), 1, MPI_INT,
                               node_neighbors[j], REQ_P3M_INIT_D, comm_cart,
                               &status);
                    }
                  }
                } else {
                ...
                for (j = 0; j < 3; j++) {
                  if (j == i) {
                    ...
                    dp3m.sm.r_ur[i * 2][j] = dp3m.sm.s_ur[i * 2][j] +
                                             dp3m.local_mesh.r_margin[2 * j];
                    dp3m.sm.r_ld[(i * 2) + 1][j] =
                        dp3m.sm.s_ld[(i * 2) + 1][j] -
                        dp3m.local_mesh.r_margin[(2 * j) + 1];
                    dp3m.sm.r_ur[(i * 2) + 1][j] =
                        dp3m.sm.s_ur[(i * 2) + 1][j] -
                        dp3m.local_mesh.margin[(2 * j) + 1];
                  } else {
            ...
            void dp3m_compute_constants_energy_dipolar() {
              ...
              P3M_TRACE(fprintf(
                  stderr, "%d: dp3m_compute_constants_energy_dipolar().\n",
                  this_node));

              double volume = box_l[0] * box_l[1] * box_l[2];
              Ukp3m = dp3m_average_dipolar_self_energy(box_l[0],
                                                       dp3m.params.mesh[0]) *
                      volume;
              ...
              dp3m.energy_correction =
                  -dp3m.sum_mu2 * (Ukp3m + Eself + 2. * PI / 3.);
            }
