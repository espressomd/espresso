...
inline double dp3m_add_pair_force(Particle *p1, Particle *p2, double *d,
  ...
  if ((p1->e->p.dipm == 0.) || (p2->e->p.dipm == 0.))
    return 0.;
...
#if USE_ERFC_APPROXIMATION
    double fac1 =
        coulomb.Dprefactor * p1->e->p.dipm * p2->e->p.dipm * exp(-adist * adist);
#else
    double fac1 = coulomb.Dprefactor * p1->e->p.dipm * p2->e->p.dipm;
#endif
...
inline double dp3m_pair_energy(Particle *p1, Particle *p2, double *d,
  ...
  if (dist < dp3m.params.r_cut && dist > 0) {
    ...
    /*  fac1 = coulomb.Dprefactor * p1->e->p.dipm*p2->e->p.dipm; IT WAS WRONG */ /* *exp(-adist*adist);
                                                                            */
#else
    erfc_part_ri = erfc(adist) / dist;
/* fac1 = coulomb.Dprefactor * p1->e->p.dipm*p2->e->p.dipm;  IT WAS WRONG*/
#endif
    ...
    /*
      ...
      C_r=%15.12f)\n",p1->e->p.identity,p2->e->p.identity,fac1*(mimj*B_r-mir*mjr*C_r),B_r,C_r);
    */
