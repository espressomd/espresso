...
// maggs_calc_dual_curl(int mue, int nue, double* field, int* Neighbor, int
// index); /* calculate curl in dual space */ void maggs_update_plaquette(int
// mue, int nue, int* Neighbor, int index, double delta); /* update fields on
// plaquette */

/****** setup everything: ******/
// int maggs_set_parameters(double prefactor, double f_mass, int mesh); /* set
// the system parameters */ void maggs_setup_neighbors(); /* setup the site
// neighbors */ void maggs_setup_local_lattice(); /* set lattice parameters */
// void maggs_calc_surface_patches(t_surf_patch* surface_patch); /* prepare for
// communication */ void maggs_prepare_surface_planes(int dim, MPI_Datatype *xy,
// MPI_Datatype *xz, MPI_Datatype *yz, t_surf_patch *surface_patch); /* prepare
// for communication */
...
// void maggs_perform_rot_move_inplane(int i, int n); /* calculate correction
// for plaquette */ void maggs_minimize_transverse_field(); /* B field
// minimization routine */ void maggs_calc_init_e_field(); /* initial solution
// */

/****** calculate currents and E-fields ******/
// void maggs_calc_charge_fluxes_1D(double q, double *help, double *flux, int
// dir); /* charge flux for current */ int maggs_check_intersect_1D(double
// delta, double r_new, int dir, int first, double *t_step, int identity); /*
// check if particles change cubes */ void maggs_calc_e_field_on_link_1D(int
// index, double *flux, double v, int dir); /* update field from current */ void
// maggs_add_current_on_segment(Particle *p, int ghost_cell); /* loop, find
// cube, add current */ void maggs_couple_current_to_Dfield() /* update field
// from current */
...
// calculate force from D-field */ void maggs_calc_forces(); /* calculate all
// forces correctly */
...
/****** init and exit ******/
// maggs_init(); /* initialize: check parameters, assign memory, calculate
// initial field */ maggs_exit(); /* free memory */
...
/** Turn the 3D index into a linear index.
    @p latticedim contains the dimensions of the lattice in
    the 3 directions: <tt>latticedim[0] = x_max, ..., latticedim[2] = z_max</tt>
...
/** Count the total number of charged particles on
    all processors.
...
/** Update all D-fields on links of the plaquette
    and the surroundings. delta used to be calculated
...
int maggs_sanity_checks() {
  ...
  if (!PERIODIC(0) || !PERIODIC(1) || !PERIODIC(2)) {
    ...
    runtimeErrorMsg()
        << "MEMD: meshsize is incompatible with number of processes";
    ret = -1;
  }
  /*
  ...
  for (ix = 0; ix < xsize; ix++) {
    ...
    for (iy = 0; iy < ysize; iy++) {
      ...
      for (iz = 0; iz < zsize; iz++) {
        ...
        if (ixminus < 0)
          neighbor[kount][XMINUS] = -1;
        else
          neighbor[kount][XMINUS] = kountxminus;
        if (ixplus >= xsize)
          neighbor[kount][XPLUS] = -1;
        else
          neighbor[kount][XPLUS] = kountxplus;

        if (iyminus < 0)
          neighbor[kount][YMINUS] = -1;
        else
          neighbor[kount][YMINUS] = kountyminus;
        if (iyplus >= ysize)
          neighbor[kount][YPLUS] = -1;
        else
          neighbor[kount][YPLUS] = kountyplus;

        if (izminus < 0)
          neighbor[kount][ZMINUS] = -1;
        else
          neighbor[kount][ZMINUS] = kountzminus;
        if (izplus >= zsize)
          neighbor[kount][ZPLUS] = -1;
        else
          neighbor[kount][ZPLUS] = kountzplus;
      }
    }
  }
  return;
...
void maggs_calc_surface_patches(t_surf_patch *surface_patch) {
  ...
  surface_patch[0].offset = lparams.dim[2] * lparams.dim[1] *
                            lparams.size[0]; /*(size[0],0,0) point */
  ...
  surface_patch[1].doffset = lparams.dim[2] * lparams.dim[1] *
                             lparams.inner_up_right[0]; /*(halo[0],0,0) point */
  ...
  surface_patch[2].offset =
      lparams.dim[2] * lparams.size[1]; /*(0,size[1],0) point */
  ...
  surface_patch[3].doffset =
      lparams.dim[2] *
      lparams.inner_up_right[1]; /*(0,inner_up_right[1],0) point */
  ...
  surface_patch[5].doffset =
      lparams.inner_up_right[2]; /*(0,0,inner_up_right[2]) point */
...
void maggs_prepare_surface_planes(int dim, MPI_Datatype *xy, MPI_Datatype *xz,
  ...
  MPI_Type_contiguous(dim * surface_patch[0].stride * sizeof(double), MPI_BYTE,
                      yz);
  MPI_Type_commit(yz);
...
void maggs_exchange_surface_patch(double *field, int dim, int e_equil) {
  ...
  if (init) {
    ...
    maggs_prepare_surface_planes(dim, &xyPlane, &xzPlane, &yzPlane,
                                 surface_patch);
    ...
    MPI_Type_create_hvector(2, 1 * sizeof(double), 2 * sizeof(double), MPI_BYTE,
                            &xz_plaq);
    ...
    MPI_Type_create_hvector(surface_patch[2].nblocks, 1,
                            dim * surface_patch[2].skip * sizeof(double),
                            oneslice, &xzPlane2D);
    MPI_Type_commit(&xzPlane2D);
    /* create data type for a 2D xy plane */
    MPI_Type_vector(surface_patch[4].nblocks, 2, dim * surface_patch[4].skip,
                    MPI_DOUBLE, &xyPlane2D);
  ...
  for (s_dir = 0; s_dir < 6; s_dir++) {
    ...
    if (node_neighbors[s_dir] != this_node) {
      ...
      case 1:
        if (e_equil || dim == 1) {
          /* MPI_Sendrecv( sendbuf, sendcount, sendtype, dest, sendtag, recvbuf,
           ...
           MPI_Sendrecv(&send_data, 1, MPI_FLOAT, next, 42, &recv_data, 1,
           MPI_FLOAT, prev, 42, comm, 0); MPI_Sendrecv_replace(&data, 1,
           MPI_FLOAT, next, 42, prev, 42, comm, 0);
           */
          MPI_Irecv(&field[doffset], 1, yzPlane, node_neighbors[s_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[0]);
          MPI_Isend(&field[offset], 1, yzPlane, node_neighbors[r_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[1]);
        } else {
          MPI_Irecv(&field[doffset + 1], 1, yzPlane2D, node_neighbors[s_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[0]);
          MPI_Isend(&field[offset + 1], 1, yzPlane2D, node_neighbors[r_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[1]);
        }
      ...
      case 3:
        if (e_equil || dim == 1) {
          MPI_Irecv(&field[doffset], 1, xzPlane, node_neighbors[s_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[0]);
          MPI_Isend(&field[offset], 1, xzPlane, node_neighbors[r_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[1]);
        } else {
          MPI_Irecv(&field[doffset], 1, xzPlane2D, node_neighbors[s_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[0]);
          MPI_Isend(&field[offset], 1, xzPlane2D, node_neighbors[r_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[1]);
        }
        MPI_Waitall(2, request, status);
        break;
      ...
      case 5:
        if (e_equil || dim == 1) {
          MPI_Irecv(&field[doffset], 1, xyPlane, node_neighbors[s_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[0]);
          MPI_Isend(&field[offset], 1, xyPlane, node_neighbors[r_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[1]);
        } else {
          MPI_Irecv(&field[doffset], 1, xyPlane2D, node_neighbors[s_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[0]);
          MPI_Isend(&field[offset], 1, xyPlane2D, node_neighbors[r_dir],
                    REQ_MAGGS_SPREAD, comm_cart, &request[1]);
        }
        MPI_Waitall(2, request, status);
        break;
...
void maggs_interpolate_charge(int *first, double *rel, double q) {
  ...
  FOR3D(i) {
    ...
    if (temp_ind == NOWHERE)
      help_index[i] = lparams.volume; /* force huge index */
    else {                            /* incr. for x-neighbor */
      help_index[i] = maggs_get_offset(lattice[neighbor[index][i]].r[i],
                                       first[i], i, lparams.dim);
    }
  }
...
/** Find current lattice site of each particle.
...
/** Find correct lattice cube for particles.
...
void maggs_calc_init_e_field() {
  ...
  if (node_pos[2] != 0) {
    MPI_Recv(&tmp_field, 1, MPI_DOUBLE, node_neighbors[4], REQ_MAGGS_EQUIL,
             comm_cart, &status);
    for (iy = lparams.inner_left_down[1]; iy < lparams.inner_up_right[1];
         iy++) {
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
           ix++) {
        index = maggs_get_linear_index(ix, iy, lparams.inner_left_down[2],
                                       lparams.dim);
  ...
  localqz = 0.;
  for (iz = lparams.inner_left_down[2]; iz < lparams.inner_up_right[2];
       iz++) { /* loop over z-planes */
    localqz = 0.;
    for (iy = lparams.inner_left_down[1]; iy < lparams.inner_up_right[1];
         iy++) {
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
    ...
    for (iy = lparams.inner_left_down[1]; iy < lparams.inner_up_right[1];
      ...
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
        ...
        Dfield[3 * index + ZPLUS] =
            Dfield[3 * neighbor[index][ZMINUS] + ZPLUS] + qplane;
    ...
    if (iz >= lparams.inner_up_right[2] - 1) {
      if (node_pos[2] < node_grid[2] - 1) {
        if (node_grid[2] > 1) {
          MPI_Send(&Dfield[3 * index + ZPLUS], 1, MPI_DOUBLE, node_neighbors[5],
                   REQ_MAGGS_EQUIL, comm_cart);
        }
      } else if (fabs(Dfield[3 * index + ZPLUS]) > 100. * ROUND_ERROR_PREC) {
        fprintf(stderr, "%d: Error in the calculation of Ez(%d,%d,%d)=%f!!\n",
                this_node, lattice[index].r[0], lattice[index].r[1],
                lattice[index].r[2], Dfield[3 * index + ZPLUS]);
        fflush(stderr);
      }
    }

    if (node_pos[1] != 0) {
      MPI_Recv(&tmp_field, 1, MPI_DOUBLE, node_neighbors[2], REQ_MAGGS_EQUIL,
               comm_cart, &status);
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
           ix++) {
        index = maggs_get_linear_index(ix, lparams.inner_left_down[1], iz,
                                       lparams.dim);
    ...
    for (iy = lparams.inner_left_down[1]; iy < lparams.inner_up_right[1];
      ...
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
        ...
        Dfield[3 * index + YPLUS] =
            Dfield[3 * neighbor[index][YMINUS] + YPLUS] + qline;
      ...
      if (iy >= lparams.inner_up_right[1] - 1) {
        if (node_pos[1] < node_grid[1] - 1) {
          if (node_grid[1] > 1)
            MPI_Send(&Dfield[3 * index + YPLUS], 1, MPI_DOUBLE,
                     node_neighbors[3], REQ_MAGGS_EQUIL, comm_cart);
        } else if (fabs(Dfield[3 * index + YPLUS]) > 100. * ROUND_ERROR_PREC)
          fprintf(stderr, "%d: Error in the calculation of Ey(%d,%d,%d)=%f!!\n",
                  this_node, lattice[index].r[0], lattice[index].r[1],
                  lattice[index].r[2], Dfield[3 * index + YPLUS]);
      }

      if (node_pos[0] != 0) {
        MPI_Recv(&tmp_field, 1, MPI_DOUBLE, node_neighbors[0], REQ_MAGGS_EQUIL,
                 comm_cart, &status);
        index = maggs_get_linear_index(lparams.inner_left_down[0], iy, iz,
                                       lparams.dim);
      ...
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
        ...
        Dfield[3 * index + XPLUS] =
            Dfield[3 * neighbor[index][XMINUS] + XPLUS] +
      ...
      if (ix >= lparams.inner_up_right[0] - 1) {
        if (node_pos[0] < node_grid[0] - 1) {
          if (node_grid[0] > 1)
            MPI_Send(&Dfield[3 * index + XPLUS], 1, MPI_DOUBLE,
                     node_neighbors[1], REQ_MAGGS_EQUIL, comm_cart);
        } else if (fabs(Dfield[3 * index + XPLUS]) > 100. * ROUND_ERROR_PREC)
  ...
  for (iz = lparams.inner_left_down[2]; iz < lparams.inner_up_right[2]; iz++) {
    index = maggs_get_linear_index(lparams.inner_left_down[0],
                                   lparams.inner_left_down[1], iz, lparams.dim);
  ...
  for (iz = lparams.inner_left_down[2]; iz < lparams.inner_up_right[2]; iz++) {
    ...
    for (iy = lparams.inner_left_down[1]; iy < lparams.inner_up_right[1];
      ...
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
           ix++)
        Dfield[3 * maggs_get_linear_index(ix, iy, iz, lparams.dim) + YPLUS] -=
            gavgEy;
    }
  }
  ...
  for (iz = lparams.inner_left_down[2]; iz < lparams.inner_up_right[2]; iz++) {
    for (iy = lparams.inner_left_down[1]; iy < lparams.inner_up_right[1];
      ...
      for (ix = lparams.inner_left_down[0]; ix < lparams.inner_up_right[0];
           ix++) {
        avgEx +=
            Dfield[3 * maggs_get_linear_index(ix, iy, iz, lparams.dim) + XPLUS];
      }
  ...
  FORALL_INNER_SITES(ix, iy, iz) {
    i = maggs_get_linear_index(ix, iy, iz, lparams.dim);
    FOR3D(k) { Eall[k] += Dfield[3 * i + k]; }
  }

  MPI_Allreduce(Eall, gEall, 3, MPI_DOUBLE, MPI_SUM, comm_cart);

  FOR3D(k)
  gEall[k] /= (n_nodes * lparams.size[0] * lparams.size[1] * lparams.size[2]);
...
int maggs_check_intersect_1D(double delta, double r_new, int dir, int first,
  ...
  if (f_crossing) {
    MAGGS_TRACE(
        fprintf(stderr,
                "Cube crossing in dir %d for particle %d at time = %f:\n", dir,
                identity, sim_time);
        fprintf(stderr, "  rold[%d]=%f, rnew[%d]=%f\n", dir,
                (first + r_old - 1.) * maggs.a, dir,
                (first + r_new - 1.) * maggs.a);
        fflush(stderr););
...
void maggs_calc_e_field_on_link_1D(int index, double *flux, double v, int dir) {
  ...
  if (temp_ind == NOWHERE)
    help_index[0] = lparams.volume;
  else
    help_index[0] = maggs_get_offset(lattice[temp_ind].r[dir1],
                                     anchor_site->r[dir1], dir1, lparams.dim);
  temp_ind = anchor_neighb[dir2];
  if (temp_ind == NOWHERE)
    help_index[1] = lparams.volume;
  else
    help_index[1] = maggs_get_offset(lattice[temp_ind].r[dir2],
                                     anchor_site->r[dir2], dir2, lparams.dim);
...
void maggs_add_current_on_segment(Particle *p, int ghost_cell) {
  ...
  if (ghost_cell) {
    FOR3D(d)
    if (first[d] < lparams.halo_left_down[d] ||
        first[d] >= lparams.halo_upper_right[d]) {
      flag_update_flux = 0;
      break;
    }
  }
  ...
  if (flag_update_flux) {
    lat_index =
        maggs_get_linear_index(first[0], first[1], first[2], lparams.dim);
  }
  ...
  for (dir = 0; dir < 5; dir++) {
    ...
    if (f_crossing) {
      ...
      if (ghost_cell) {
        if (flag_update_flux) {
          if (first[icoord] < lparams.halo_left_down[icoord] ||
              first[icoord] >= lparams.halo_upper_right[icoord]) {
            flag_update_flux = 0;
          }
        } else {
          flag_update_flux = 1;
  ...
  for (x = 0; x < lparams.size[0]; x++) {
    for (y = 0; y < lparams.size[1]; y++) {
      for (z = 0; z < lparams.size[2]; z++) {
        ...
        index = 3 * i;
        Bfield[index + 0] +=
            -help * maggs_calc_dual_curl(1, 2, Dfield, neighbor[i], index);
        Bfield[index + 1] +=
            -help * maggs_calc_dual_curl(2, 0, Dfield, neighbor[i], index);
        Bfield[index + 2] +=
            -help * maggs_calc_dual_curl(0, 1, Dfield, neighbor[i], index);
      }
      offset += yoffset;
    }
    offset += xoffset;
  ...
  for (x = 0; x < lparams.size[0]; x++) {
    for (y = 0; y < lparams.size[1]; y++) {
      for (z = 0; z < lparams.size[2]; z++) {
        ...
        index = 3 * i;
        Dfield[index] +=
            help * maggs_calc_curl(2, 1, Bfield, neighbor[i], index);
        Dfield[index + 1] +=
            help * maggs_calc_curl(0, 2, Bfield, neighbor[i], index);
        Dfield[index + 2] +=
            help * maggs_calc_curl(1, 0, Bfield, neighbor[i], index);
      }
      offset += yoffset;
    }
    offset += xoffset;
  ...
  for (k = 0; k < 2; k++) {     /* jumps from x- to x+ */
    for (l = 0; l < 2; l++) {   /* jumps from y- to y+ */
      for (m = 0; m < 2; m++) { /* jumps from z- to z+ */
        // without q!!!
        if (k == 0)
          rho[index] += -help[0] * grad[grad_ind];
        else {
...
void maggs_calc_interpolated_self_force(Particle *p) {
  ...
  /*
    ...
    FOR3D(k) {
      position[k]           = (p->r.p[k] - lparams.left_down_position[k]) *
    maggs.inva; left_down_position[k] = floor(position[k]); relative_position[k]
    = position[k] - left_down_position[k]; self_force[k] = 0.0; local_D_field[k]
    = 0.0;
    }
      ...
      for (iy=0;iy<ymax;iy++) {
        for (ix=0;ix<xmax;ix++) {
          ...
          globalindex = maggs_get_linear_index((left_down_position[0]+ix),
                                               ...
                                               (left_down_position[2]+iz),
    lparams.dim); local_permittivity[index] =
    lattice[globalindex].permittivity[0]; local_rho[index] = 0.0;
        }
      }
    }
...
void maggs_calc_self_influence(Particle *P) {
  ...
  double position[SPACE_DIM], left_down_position[SPACE_DIM],
      relative_position[SPACE_DIM];
  //	int index = 0;
  //	int globalindex = 0;
  ...
  /*
    ...
    self_influence_correction[index].permittivity =
    lattice[globalindex].permittivity; self_influence_correction[index].charge =
    0.0; //background_charge; FOR3D(k) { charge_gradient[index][k] = 0.0;
  ...
  */
  ...
  // maggs.prefactor = 3.544907702 = sqrt(4. * M_PI * maggs.bjerrum *
  // temperature); Fehlender Faktor: 4*pi=12.5663706 a^3/c = 2.58448064965803
...
void maggs_calc_part_link_forces(Particle *p, int index, double *grad) {
  ...
  if (init) {
    ...
    anchor_site = &lattice[index];
    FOR3D(j) {
      help_index[j] = maggs_get_offset(lattice[neighbor[index][j]].r[j],
                                       anchor_site->r[j], j, lparams.dim);
    }
    init = 0;
  }
...
double maggs_magnetic_energy() {
  ...
  FORALL_INNER_SITES(x, y, z) {
    i = maggs_get_linear_index(x, y, z, lparams.dim);
    result += Utils::sqr(Bfield[i * 3]) + Utils::sqr(Bfield[i * 3 + 1]) +
              Utils::sqr(Bfield[i * 3 + 2]);
  }
