...
int maggs_sanity_checks() {
  ...
  */
  ...
  for (auto const &p : local_cells.particles()) {
    if ((p.q != 0.0) & p.e->p.ext_flag & COORDS_FIX_MASK) {
  ...
  */
  ...
  for (auto const &p : local_cells.particles())
    for (int dim = 0; dim < 3; dim++)
      local_dipole_moment[dim] += p.r.p[dim] * p.q;
  ...
  for (auto &p : local_cells.particles())
    for (int dim = 0; dim < 3; dim++)
      p.f.f[dim] += p.q * dipole_prefactor * dipole_moment[dim];
}
...
void maggs_add_current_on_segment(Particle *p, int ghost_cell) {
  ...
  for (dir = 0; dir < 5; dir++) {
    ...
    /* calculate flux */
    if (flag_update_flux) {
      maggs_calc_charge_fluxes_1D(p->q, help, flux, icoord);
