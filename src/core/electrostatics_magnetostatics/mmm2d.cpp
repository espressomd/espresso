...
static void add_z_force() {
  ...
  if (mmm2d_params.const_pot_on) {
    ...
    for (auto const &p : local_cells.particles()) {
      lcl_dm_z += p.q * (p.r.p[2] + p.e->l.i[2] * box_l[2]);
    }
  ...
  for (int c = 1; c <= n_layers; c++) {
    ...
    auto part = cells[c].part;
    for (int i = 0; i < np; i++) {
      part[i].f.f[2] += part[i].q * (add + field_tot);
...
static void setup_z_energy() {
  ...
  for (c = 1; c <= n_layers; c++) {
    ...
    for (i = 0; i < np; i++) {
      lclcblk[size * c + ABEQQP] += part[i].q;
      lclcblk[size * c + ABEQZP] += part[i].q * part[i].r.p[2];
    }
...
static double z_energy() {
  ...
  for (c = 1; c <= n_layers; c++) {
    ...
    part = cells[c].part;
    for (i = 0; i < np; i++) {
      eng += part[i].q * (part[i].r.p[2] * othcblk[ABEQQP] - othcblk[ABEQZP] -
...
static void setup(int p, double omega, double fac, int n_sccache,
  ...
  for (c = 1; c <= n_layers; c++) {
    ...
    for (i = 0; i < np; i++) {
      ...
      partblk[size * ic + POQESM] = part[i].q * sccache[o + ic].s / e;
      partblk[size * ic + POQESP] = part[i].q * sccache[o + ic].s * e;
      partblk[size * ic + POQECM] = part[i].q * sccache[o + ic].c / e;
      partblk[size * ic + POQECP] = part[i].q * sccache[o + ic].c * e;
      ...
      if (mmm2d_params.dielectric_contrast_on) {
        if (c == 1 && this_node == 0) {
          ...
          lclimgebot[POQESP] += part[i].q * sccache[o + ic].s * e;
          lclimgebot[POQECP] += part[i].q * sccache[o + ic].c * e;
        } else
        ...
        if (c == n_layers && this_node == n_nodes - 1) {
          ...
          lclimgetop[POQESM] += part[i].q * sccache[o + ic].s * e;
          lclimgetop[POQECM] += part[i].q * sccache[o + ic].c * e;
        } else
        ...
        lclimge[POQESP] += part[i].q * sccache[o + ic].s * e_di_l;
        lclimge[POQECP] += part[i].q * sccache[o + ic].c * e_di_l;
        lclimge[POQESM] += part[i].q * sccache[o + ic].s * e_di_h;
        lclimge[POQECM] += part[i].q * sccache[o + ic].c * e_di_h;
      }
  ...
  for (c = 1; c <= n_layers; c++) {
    ...
    for (i = 0; i < np; i++) {
      ...
      partblk[size * ic + PQESSM] =
          scxcache[ox + ic].s * scycache[oy + ic].s * part[i].q / e;
      partblk[size * ic + PQESCM] =
          scxcache[ox + ic].s * scycache[oy + ic].c * part[i].q / e;
      partblk[size * ic + PQECSM] =
          scxcache[ox + ic].c * scycache[oy + ic].s * part[i].q / e;
      partblk[size * ic + PQECCM] =
          scxcache[ox + ic].c * scycache[oy + ic].c * part[i].q / e;
      ...
      if (mmm2d_params.dielectric_contrast_on) {
        ...
        lclimge[PQESSP] +=
            scxcache[ox + ic].s * scycache[oy + ic].s * part[i].q * e_di_l;
        lclimge[PQESCP] +=
            scxcache[ox + ic].s * scycache[oy + ic].c * part[i].q * e_di_l;
        lclimge[PQECSP] +=
            scxcache[ox + ic].c * scycache[oy + ic].s * part[i].q * e_di_l;
        lclimge[PQECCP] +=
            scxcache[ox + ic].c * scycache[oy + ic].c * part[i].q * e_di_l;

        lclimge[PQESSM] +=
            scxcache[ox + ic].s * scycache[oy + ic].s * part[i].q * e_di_h;
        lclimge[PQESCM] +=
            scxcache[ox + ic].s * scycache[oy + ic].c * part[i].q * e_di_h;
        lclimge[PQECSM] +=
            scxcache[ox + ic].c * scycache[oy + ic].s * part[i].q * e_di_h;
        lclimge[PQECCM] +=
            scxcache[ox + ic].c * scycache[oy + ic].c * part[i].q * e_di_h;
      }
  ...
  if (this_node == 0) {
    ...
    for (i = 0; i < npl; i++) {
      ...
      for (j = 0; j < npl; j++) {
        ...
        dist2 = sqrlen(d);
        charge_factor = p1->q * pl[j].q * mmm2d_params.delta_mid_bot;
  ...
  if (this_node == n_nodes - 1) {
    ...
    for (i = 0; i < npl; i++) {
      ...
      for (j = 0; j < npl; j++) {
        ...
        layered_get_mi_vector(d, p1->r.p.data(), a);
        dist2 = sqrlen(d);
        charge_factor = p1->q * pl[j].q * mmm2d_params.delta_mid_top;
        add_mmm2d_coulomb_pair_force(charge_factor, d, sqrt(dist2), dist2,
                                     force);
        /* remove unwanted 2 pi |z| part (cancels due to charge neutrality) */
        force[2] += pref * charge_factor;
  ...
  if (this_node == 0) {
    ...
    for (i = 0; i < npl; i++) {
      ...
      for (j = 0; j < npl; j++) {
        ...
        layered_get_mi_vector(d, p1->r.p.data(), a);
        dist2 = sqrlen(d);
        charge_factor = mmm2d_params.delta_mid_bot * p1->q * pl[j].q;
  ...
  if (this_node == n_nodes - 1) {
    ...
    for (i = 0; i < npl; i++) {
      ...
      for (j = 0; j < npl; j++) {
        ...
        layered_get_mi_vector(d, p1->r.p.data(), a);
        dist2 = sqrlen(d);
        charge_factor = mmm2d_params.delta_mid_top * p1->q * pl[j].q;
        /* last term removes unwanted 2 pi |z| part (cancels due to charge
         * neutrality) */
        eng += mmm2d_coulomb_pair_energy(charge_factor, d, dist2, sqrt(dist2)) -
               pref * charge_factor * d[2];
