...
void ScafacosData::update_particle_forces() const {
  ...
  for (auto &p : local_cells.particles()) {
    if (!dipolar()) {
      p.f.f[0] += coulomb.prefactor * p.q * fields[it++];
      p.f.f[1] += coulomb.prefactor * p.q * fields[it++];
      p.f.f[2] += coulomb.prefactor * p.q * fields[it++];
    } else {
#ifdef SCAFACOS_DIPOLES
      // Indices
...
void add_pair_force(Particle *p1, Particle *p2, double *d, double dist,
  ...
  const double fak = p2->q * p1->q * field * coulomb.prefactor / dist;
...
double pair_energy(Particle *p1, Particle *p2, double dist) {
  if (dist <= get_r_cut())
    return coulomb.prefactor * p1->q * p2->q * scafacos->pair_energy(dist);
  else
    return 0.;
}
