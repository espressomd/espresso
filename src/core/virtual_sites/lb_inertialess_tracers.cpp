...
void IBM_ForcesIntoFluid_CPU() {
  ...
  Usually the reset would be done by Espresso after the LB update. But we need
  to keep the forces till after the position update for the f/2 term
  ****************/
  ...
  Interpolates LB velocity at the particle positions and propagates the
  particles
  **************/
    ...
    for (int c = 0; c < local_cells.n; c++) {
      ...
      for (int j = 0; j < cell->n; j++) {
        if (p[j].p.is_virtual) {
          ...
          // Check if the particle might have crossed a box border (criterion
          // see e-mail Axel 28.8.2014) if possible resort_particles = 1
  ...
  This function puts the momentum of a given particle into the LB fluid - only
  for CPU
  **************/
      ...
      for (int y = 0; y < 2; y++) {
        for (int x = 0; x < 2; x++) {
          ...
          if (!IsHalo(node_index[(z * 2 + y) * 2 + x])) {
            ...
            local_f[0] += delta[3 * x + 0] * delta[3 * y + 1] *
                          delta[3 * z + 2] * delta_j[0];
            local_f[1] += delta[3 * x + 0] * delta[3 * y + 1] *
                          delta[3 * z + 2] * delta_j[1];
            local_f[2] += delta[3 * x + 0] * delta[3 * y + 1] *
                          delta[3 * z + 2] * delta_j[2];
          }
        }
      }
    }
