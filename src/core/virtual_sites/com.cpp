 ...
 * - weighted_position: The sum of (mass * position) for all constituent
 * particles.
 *
 ...
 *
 * Complexity: O(N) per rank, where N is the total number of key-value pairs
 * across all ranks.
 *
 * All ranks must call this collectively. Requires Boost.MPI and compatible
 * serialization for T1, T2.
 */
template <typename T1, typename T2>
void communicate_map(std::unordered_map<T1, T2> &map_data,
                     boost::mpi::communicator const &comm_cart) {
...
void vs_com_update_particles(CellStructure &cell_structure,
  ...
  local_mol_ids.erase(std::unique(local_mol_ids.begin(), local_mol_ids.end()),
                      local_mol_ids.end());
  ...
  for (const auto &mol_id_list : all_mol_ids) {
    flattened_mol_ids.insert(flattened_mol_ids.end(), mol_id_list.begin(),
                             mol_id_list.end());
  }
  std::sort(flattened_mol_ids.begin(), flattened_mol_ids.end());
  flattened_mol_ids.erase(
      std::unique(flattened_mol_ids.begin(), flattened_mol_ids.end()),
      flattened_mol_ids.end());
  ...
  for (const auto mol_id : flattened_mol_ids) {
    ...
    boost::mpi::all_reduce(comm_cart, local_total_mass, global_total_mass,
                           std::plus());
    boost::mpi::all_reduce(comm_cart, local_weighted_position,
                           global_weighted_position, std::plus());
...
void vs_com_back_transfer_forces_and_torques(CellStructure &cell_structure) {
  ...
  cell_structure.for_each_local_particle([&](Particle &p) {
    if (is_vs_com(p))
      return; // Check if particle is a virtual site center of mass
