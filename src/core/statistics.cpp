...
double mindist(PartCfg &partCfg, IntList const &set1, IntList const &set2) {
  ...
  void calc_part_distribution(PartCfg & partCfg, int const *p1_types, int n_p1,
                              ...
                              double r_max, int r_bins, int log_flag,
  ...
  void calc_rdf(PartCfg & partCfg, int const *p1_types, int n_p1,
    ...
    void calc_rdf_av(PartCfg & partCfg, int const *p1_types, int n_p1,
      ...
      void calc_structurefactor(PartCfg & partCfg, int const *p_types,
                                int n_types, int order, double **_ff) {
        ...
        *_ff = ff = Utils::realloc(ff, 2 * order2 * sizeof(double));
        ...
        std::vector<std::vector<double>> modify_stucturefactor(
            int order, double const *sf) {
          int length = 0;
        ...
        // configurations
        void density_profile_av(PartCfg & partCfg, int n_conf, int n_bin,
                                double density, int dir, double *rho_ave,
                                int type) {
          int i, j, k, m, n;
          double r;
          double r_bin;
          ...
          while (k < n_configs) {
            ...
            while (r < box_l[dir]) {
              ...
              for (auto const &p : partCfg) {
                // com particles
                if (p.p.type == type) {
                  auto const pos = folded_position(
                      {&configs[k][3 * i], &configs[k][3 * i] + 3});
              ...
              rho_ave[j] +=
                  (double)(n) / (box_l[1] * box_l[2] * r_bin) / density;
              j++;
              r += r_bin;
            }
            k++;
        ...
        int calc_cylindrical_average(
          ...
          for (unsigned int type = 0; type < types.size(); type++) {
            ...
            for (int index_radial = 0; index_radial < bins_radial;
                 index_radial++) {
              distribution["density"][type][index_radial].assign(bins_axial,
                                                                 0.0);
          ...
          for (auto const &p : partCfg) {
            for (unsigned int type_id = 0; type_id < types.size(); type_id++) {
              if (types[type_id] == p.p.type || all_types) {
                ...
                if (height == 0) {
                  v_radial =
                      vector_product(vel, direction).norm() / norm_direction;
                } else {
                ...
                index_axial = static_cast<int>(
                    floor((dist + 0.5 * length) / binwd_axial));
          ...
          for (unsigned int type_id = 0; type_id < types.size(); type_id++) {
            for (int index_radial = 0; index_radial < bins_radial;
              ...
              // The volume is thus: binvolume = pi*(r_outer - r_inner)^2 *
              // length
              if (index_radial == 0) {
              ...
              for (int index_axial = 0; index_axial < bins_axial;
                   index_axial++) {
                if (distribution["density"][type_id][index_radial]
                                [index_axial] != 0) {
                  distribution["v_r"][type_id][index_radial][index_axial] /=
                      distribution["density"][type_id][index_radial]
                                  [index_axial];
                  distribution["v_t"][type_id][index_radial][index_axial] /=
                      distribution["density"][type_id][index_radial]
                                  [index_axial];
        ...
        void obsstat_realloc_and_clear(
            Observable_stat * stat, int n_pre, int n_bonded, int n_non_bonded,
            int n_coulomb, int n_dipolar, int n_vs, int c_size) {

          // Number of doubles to store pressure in
          const int total = c_size * (n_pre + bonded_ia_params.size() +
                                      n_non_bonded + n_coulomb + n_dipolar +
                                      n_vs + Observable_stat::n_external_field);
        ...
        void obsstat_realloc_and_clear_non_bonded(
            Observable_stat_non_bonded * stat_nb, int n_nonbonded, int c_size) {
          ...
          stat_nb->non_bonded_inter =
              stat_nb->non_bonded_intra + c_size * n_nonbonded;
        ...
        void update_pressure(int v_comp) {
          int i;
          double p_vel[3];
          /* if desired (v_comp==1) replace ideal component with instantaneous
           * one */
