...
public:
  ...
  double r_bin_size = (max_r - min_r) / n_r_bins;
  double phi_bin_size = (max_phi - min_phi) / n_phi_bins;
  double z_bin_size = (max_z - min_z) / n_z_bins;
  virtual int n_values() const override { return n_r_bins * n_phi_bins * n_z_bins; };
  ::Vector<3, double> transform_to_cylinder_coordinates(::Vector<3, double> const &pos, ::Vector<3, double> const &center) const {
    double x = pos[0] - center[0];
    double y = pos[1] - center[1];
    double r = std::sqrt(x * x + y * y);
    double phi = std::atan2(y, r);
