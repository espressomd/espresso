...
public:
  ...
  void calculate_sampling_positions() {
    ...
    const auto n_samples_x = static_cast<size_t>(
        std::rint((box_geo.length()[0] - sampling_offset_x) / sampling_delta_x));
    const auto n_samples_y = static_cast<size_t>(
        std::rint((box_geo.length()[1] - sampling_offset_y) / sampling_delta_y));
    const auto n_samples_z = static_cast<size_t>(
        std::rint((box_geo.length()[2] - sampling_offset_z) / sampling_delta_z));
