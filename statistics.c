...
#ifdef WATER
int h2o_com_unper_k(int knr,int pnr,double *p_com)
{
	int i,p;
	double M;
	#ifdef WATER_DEBUG
	...
	if (partCfg[pnr].p.type == 0){
		...
		for (p=1; p<3; p++) {
			...
			{
				fprintf(stderr,"%ith part is not type 2 (in h2o_com_unper_k)! knr=%i pnr=%i\n",p,knr,pnr);
				exit(182);
			}
			for (i=0;i<3;i++)
			{
		...
		{
			fprintf(stderr,"M unequal in h2o_com_unper_k ! knr=%i pnr=%i\n",knr,pnr);
			exit(182);
		}
		#endif
		for (i=0;i<3;i++)
	...
	else if ((partCfg[pnr].p.type == 4) || (partCfg[pnr].p.type == 5 ))
	{
		for (i=0;i<3;i++){
			p_com[i]=configs[knr][3*(pnr)+i];
		}
		return 1;
	}
	else
...
int h2o_com_unper(int pnr,double *p_com)
{
	int i,p;
	double M;
	#ifdef WATER_DEBUG
	...
	if (partCfg[pnr].p.type == 0){
		for (i=0;i<3;i++)
		{
			p_com[i]=partCfg[pnr].r.p[i]*PMASS(partCfg[pnr]);
			#ifdef WATER_DEBUG
			p_O[i]=partCfg[pnr].r.p[i];
			#endif
		}
		M=PMASS(partCfg[pnr]);
		for (p=1; p<3; p++) {
		...
		if (fabs(M -1.1250)> 0.001)
		{
			fprintf(stderr,"M unequal in h2o_com_unper ! pnr=%i\n",pnr);
			exit(182);
		}
		#endif
		for (i=0;i<3;i++)
...
int h2o_com_vel_unper(int pnr,double *v_com)
{
	int i,p;
	double M;
	if (partCfg[pnr].p.type == 0){
...
double calc_diffusion_coef_h2o(Tcl_Interp *interp, int n_time_steps,int n_conf)
{
  ...
  for(j=0; j<n_total_particles; j++) {
     ...
     for(i=start_value;i<n_configs;i++) {
        if(h2o_com_unper_k(i,j,p1) == 3) {
            p1[0]=p1[0]-p_x[i];
            p1[1]=p1[1]-p_y[i];
            p1[2]=p1[2]-p_z[i];
            for (k=i;k<n_configs;k++)
            {
            	if(h2o_com_unper_k(k,j,p2)==3){
			p2[0]=p2[0]-p_x[k];
			p2[1]=p2[1]-p_y[k];
			p2[2]=p2[2]-p_z[k];
			MSD[k-i]+=distance2(p1, p2);
...
static int parse_and_print_Ekin_h2o(Tcl_Interp *interp,int argc, char **argv)
{
   ...
   for (i=0;i<n_total_particles;i++)
   {
      if (h2o_com_vel_unper(i,v_com) == 3 )
      {
         E_kin+=1.1250*sqrlen(v_com);
      }
      else if (h2o_com_vel_unper(i,v_com) == 1 )
      {
         E_kin+=PMASS(partCfg[i])*sqrlen(v_com);
      }
   }
...
// void h2o_com_force_unper(int pnr,double *f_com)
// {
// 	int i,p;
// 	#ifdef WATER_DEBUG
// 	double M;
...
static int parse_and_print_pressure_h2o(Tcl_Interp *interp,int argc, char **argv)
{
   int i,j,k,l;
   char buffer[TCL_DOUBLE_SPACE];
   double psum=0,dist2,dist;
   double r_com[3],f_ij[3],com_dist[3];
   double r2_com[3],vec12[3];
   int is_water_1,is_water_2;
   Particle *p1,*p2;
   updatePartCfg(WITHOUT_BONDS);
   if (!sortPartCfg()) {
      char *errtxt = runtime_error(128);
      ERROR_SPRINTF(errtxt, "{059 parse_and_print_pressure_h2o: could not sort particle config, particle ids not consecutive?} ");
      return TCL_ERROR;
   }
   for (i=0;i<n_total_particles-1;i++)
   {
      if ( (is_water_1=h2o_com_unper(i,r_com)) != -1 )
      {
         for (j=i+1;j<n_total_particles;j++)
         {
             if ( (is_water_2=h2o_com_unper(j,r2_com)) != -1 )
             {
                get_mi_vector(com_dist,r_com,r2_com);
                for (k=0;k<3;k++){
                    f_ij[k]=0;
                }
                for (k=0;k<is_water_1;k++){
                    p1=&partCfg[i+k];
                    for (l=0;l<is_water_2;l++){
                        p2=&partCfg[j+l];
