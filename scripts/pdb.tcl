...
proc writepdb_h2o {file {l_scale 1.0}} {
    set f [open $file "w"]
    puts $f "REMARK generated by Espresso writepdb_h2o"
    # write atoms
    set mp [setmd max_part]
    for {set p 0} { $p <= $mp } { incr p } {
	set tp [part $p print type]
	if { $tp != "na" } {
	    if { $tp == 0 } {
	       set mytype " O";
	    } elseif { $tp == 2 } {
	    ...
	    set nr [expr $p+1]
	    set pos [part $p print folded_position]
	    set myx [expr  [lindex $pos 0]*$l_scale]
	    set myy [expr  [lindex $pos 1]*$l_scale]
	    set myz [expr  [lindex $pos 2]*$l_scale]
	    puts $f [format "ATOM %6d %2s               %8.3f%8.3f%8.3f" $nr $mytype $myx $myy $myz]
	}
    }
    set cnt 1
    for {set p 0} { $p <= $mp } { incr p } {
	set tp [part $p print type]
	if { $tp == 0 } {
	    puts $f [format "CONECT%4d%4d%4d" [expr $p+1] [expr $p+2] [expr $p+3]]
	    incr cnt
	}
    }
    close $f
