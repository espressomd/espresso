...
for config in ${configs}; do
  ...
  cmake -D ESPRESSO_BUILD_BENCHMARKS=ON -D ESPRESSO_TEST_TIMEOUT=1200 -D ESPRESSO_BUILD_WITH_CUDA=OFF -D ESPRESSO_BUILD_WITH_WALBERLA_LIB=ON -D ESPRESSO_BUILD_WITH_CCACHE=OFF ..
  make -j$(nproc)
