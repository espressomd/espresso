...
if [ -z ${CI_JOB_STAGE+x} ]; then
    stage_build="true"
    stage_checks="true"
else
    [ "${CI_JOB_STAGE}" = "build" ] && stage_build="true" || stage_build="false"
    [ "${CI_JOB_STAGE}" = "checks" ] && stage_checks="true" || stage_checks="false"
fi
