...
echo "Files that are missing the project and the current year..."
files=`egrep -L "Copyright.*$current_year.*The ESPResSo project" $files`
num_files=`echo $files | wc -w`
echo "  $num_files files."
...
echo "Files that are missing the current year..."
num_files=`echo $noyear_files | wc -w`
echo "  $num_files files."
echo "  Adding year to copyright disclaimer..."
echo "    \"$current_year\""
tmpfile=`mktemp`
...
echo "Files that are missing the project..."
