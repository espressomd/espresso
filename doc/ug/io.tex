...
The checkpointing and writing of blockfiles in \es{} is still working,
but is assigned a major overhaul. Be aware that things might change in
the future and that this documentation chapter might not be completely
up-to-date and not complete or well-written.
...
\todo{Sample C-code doesn't work, as \es-library has been removed!}
...
\emph{Warning: the checkpointing commands are deprecated!\\
They are no longer properly maintainged and it is possible
that new features are not checkpointed. Therefore it is 
recommended that you write your own problem-specific
