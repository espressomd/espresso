...
To write data on a hdf5-file, first an object of the class \texttt{h5md} has to be created and linked to the respective hdf5-file. This may, for example, look like:
\begin{pycode}
from espressomd import h5md
system = espressomd.System()
h5=h5md.h5md("File.h5",system)
\end{pycode}
The command \texttt{userdefined} writes a single value to the hdf5-file:
\begin{pyessyntax}
h5.write_to_h5.userdefined(\var{value},\var{groupname},\var{datasetname},\var{datatype},\\\var{datasetindex},\var{chunks})
\end{pyessyntax}
The \texttt{h5md} sub-class \texttt{write_to_h5} contains all write commands such as \texttt{userdefined}.
\begin{arguments}
\item[\var{value}] To be written value.
\item[\var{groupname}] Name of the group containing the dataset.
\item[\var{datasetname}] Name of the dataset containing the values.
\item[\var{datatype}] Data type of the values.
...
\end{arguments}
Particle properties can be written on the hdf5-file via a single command, writing the values of all particles at once:
\begin{pyessyntax}
h5.write_to_h5.particle_property(\var{timestep},\var{groupname,datasetname},\var{chunks})
\end{pyessyntax}
The command \texttt{particle_property} is a short cut for the various particle property commands such as \texttt{pos} (position) and \texttt{f} (force). When leaving out the variables \var{groupname}, \var{datasetname}, and \var{chunks}, the predefined values of \var{groupname} and \var{datasetname} are used (H5MD-format) and the chunk size is set automatically.
\begin{arguments}
\item[\var{timestep}] Time index in the hdf5-file. If the value is time independent, \var{timestep} has to be set to -1 or leaved out.
\end{arguments}
ESPResSo observables can be written as follows:
\begin{pyessyntax}
h5.write_to_h5.observable(\var{timestep},\var{groupname},\var{datasetname},\var{chunks})
\end{pyessyntax}
The command \texttt{observable} is a short cut for the various observable commands such as \texttt{analyze_energy} (energy).\\\\
The sample python script ``h5md_WriteFile.py'' shows the usage of writing on a hdf5-file.

\section{Reading H5MD-files with Python}
To read from a hdf5-file, again an object of the class \texttt{h5md} has to be created and linked to the respective hdf5-file. The command \texttt{userdefined} reads a single value from the hdf5-file:
\begin{pyessyntax}
h5.read_from_h5.userdefined(\var{groupname},\var{datasetname},\var{datasetindex})
\end{pyessyntax}
The \texttt{h5md} sub-class \texttt{read_from_h5} contains all read commands such as \texttt{userdefined}.\\\\
Particle properties can be read from the hdf5-file and stored in the respective ESPResSo variable as follows:
\begin{pyessyntax}
h5.read_from_h5.particle_property(\var{timestep},\var{groupname},\var{datasetname})
\end{pyessyntax}
The command \texttt{particle_property} is again a short cut for the various particle property commands such as \texttt{pos} (position) and \texttt{f} (force). When leaving out the variables \var{groupname} and \var{datasetname}, the predefined values of \var{groupname} and \var{datasetname} are used (H5MD-format).\\\\ 
