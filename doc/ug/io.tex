...
\url{http://nongnu.org/h5md/}). The init command assumes that the filename ends
with the suffix ".h5". If there is no filename present matching with the given,
the filename will be given the suffix "_1.h5". In order to safely start from a
checkpoint the init command automatically copies the old file to a new file
with incremented suffix index (e.g. "_2.h5" for the first start from a
checkpoint). The optional argument \var{list\_of\_p\_ids} can be provided. In
...
h5=h5md.H5md("File.h5")
\end{pycode}
At the initialization of the H5md object a temporary filename with the suffic "_tmp" will be used and to ensure a valid trajectory in case of a crashed simulation. Therefore the class method "close()" has to be called at the end of the simulation to perform the deletion of the old file and a renaming of the output file to the given filename.
Note that you have first to add all your particles to the system, since the dataset structure in the hdf5 file depends on that number.

To write data to the hdf5 file, currently three methods are implemented for writing positions, velocities and forces:
\begin{pycode}
h5.write_positions()
h5.write_velocities()
h5.write_forces()
\end{pycode}
Until now, the code only works for single-core simulations, but since the implementation is in-core, there will be a multi-core support soon.
