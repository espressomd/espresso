...
\es with the python interface features two possibilities for
...
specify the appearance of the output. Additional requirements: XXXXXX.
...
can be adjusted by a set of parameters. Additional requirements: installed
python module \emph{PyOpenGL}.  
\end{enumerate}
...
choice and pass it the \lstinline{espressomd.System()}. Then write your
simulation script in a seperate function, which is started in a non-blocking
thread. Whenever needed, call \lstinline{update()} to synchronize the renderer
with your system.  Finally start the blocking visualization window. The \es
visualization and threading modules can be imported with
...
\begin{pysyntax}
  \attributes{
    ...
    mayaviLive | openGLLive
  }{
    start()
}
\end{pysyntax}

Starts the visualizer window.
...
\begin{pysyntax}
  \attributes{
    ...
    registerCallback(callback, interval=<int>)
}
\end{pysyntax}

Registers the method \keyword{callback}, which is called every \keyword{intervall} milliseconds.
...
\begin{pysyntax}
  \object{
    ...
    particle_sizes=\arg{"auto" | callable | list}
  ]
\end{pysyntax}
...
\keyword{"auto"} (default): The Lennard-Jones sigma value of the self-interaction 
is used for the particle diameter. 
\keyword{callable}: A lambda function with one argument. Internally, the numerical 
particle type is passed to the lambda function to determine the particle radius. 
\keyword{list} A list of particle radii, indexed by the particle type.
\end{arguments}

\subsection{OpenGL visualizer}

The optional parameters to adjust the appearance of the visualization
have suitable default values. Colors for particles, bonds and constraints are
specified by RGBA arrays, materials by an array for the ambient, diffuse and
specular (ADS) components.  To distinguish particle groups, arrays of RGBA or
ADS arrays are used, which are indexed circularly by the numerical particle
type.
...
\begin{pysyntax}
  \object{
    ...
    window_size=\arg{array of 2 ints},                        
    name=\arg{str},	 
    background_color=\arg{array of 3 floats},
    update_fps=\arg{float},
    periodic_images=\arg{array of 3 floats},
    draw_box=\arg{bool},
    quality_spheres=\arg{int},
    quality_bonds=\arg{int},          
    quality_arrows=\arg{int},            
    close_cut_distance=\arg{float},
    far_cut_distance=\arg{float},       
    camera_position=\arg{"auto" | array of 3 floats },
    camera_rotation=\arg{array of 2 floats},
    particle_sizes=\arg{"auto" | callable | list},
    particle_coloring=\arg{"auto", "id", "charge", "type"},
    particle_type_colors=\arg{array of arrays of 4 floats},
    particle_type_materials=\arg{array of arrays of 3 floats},
    particle_charge_colors=\arg{array of 2 arrays of 4 floats},	 
    draw_constraints=\arg{bool},			 
    rasterize_pointsize=\arg{float},	     
    rasterize_resolution=\arg{float},	     
    quality_constraints=\arg{int},          
    constraint_type_colors=\arg{array of arrays of 4 floats},
    constraint_type_materials=\arg{array of arrays of 3 floats},
    draw_bonds=\arg{bool},				 
    bond_type_radius=\arg{array of floats},		     
    bond_type_colors=\arg{array of arrays of 4 floats},			 
    bond_type_materials=\arg{array of arrays of 3 floats},	     
    ext_force_arrows=\arg{bool}, 		 
    ext_force_arrows_scale=\arg{array of floats}, 	 
    drag_enabled=\arg{bool},		   		 
    drag_force=\arg{float},		   		 
    light_pos=\arg{"auto" | array of 3 floats},				 
    light_color=\arg{array of 3 floats},				 
    light_brightness=\arg{float},   		 
    light_size=\arg{float}		   		 
  ]
\end{pysyntax}
...
\item[\opt{camera_position}] The initial camera position.
\item[\opt{camera_rotation}] Camera angles.
...
\keyword{"auto"} (default): Colors of charged particles are specified by 
\keyword{particle_charge_colors}, neutral particles by \keyword{particle_type_colors}\\
\keyword{"charge"}: Minimum and maximum charge of all particles is determined by the visualizer.
All particles are colored by a linear interpolation of the two colors given by 
\keyword{particle_charge_colors} according to their charge.\\
\keyword{"type"}: Particle colors are specified by \keyword{particle_type_colors}, 
...
\item[\opt{light_pos}] If \keyword{"auto"} (default) is used, the light is placed in the particle barycenter of the system dynamically. Otherwise, a fixed coordinate can be set.
