...
can be adjusted by a large set of parameters. Additional requirements: installed
...
The \es visualization and threading modules can be
imported with
...
Registers the method \keyword{callback}, which is called every \keyword{interval} milliseconds.
...
\item[\opt{light_pos}] If \keyword{"auto"} (default) is used, the light is placed dynamically
...
The camera is controlled via mouse (camera direction), WASD-Keyboard control
(camera movement) and the key pairs QE, RF, ZC (camera roll).
...
Finally, it is recommended to go through tutorial "02-Charged Systems" and "08-Visualization" for further code explanations and examples.
