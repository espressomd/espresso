...
This section describes an alternative, yet similar, way to include soft elastic objects as the previous chapter. In the Immersed Boundary Method (IBM), soft particles are considered as an infinitely thin shell filled with liquid (see e.g. ~\citewbibkey{Krueger_thesis, Crowl_2011}). When the shell is deformed by an external flow it responds by elastic restoring forces which are transmitted into the fluid. In the present case, the inner and outer liquid are of the same type. In \es, both liquids are simulated using Lattice-Boltzmann. 
...
Skalak <k1> <k2>
\end{verbatim}
which specifies the elastic law and its corresponding parameters as detailed in \citewbibkey{Krueger_thesis}. Note the final parameter needs to be passed as a single string, e.g.
\begin{verbatim}
...
\end{verbatim}
where <softID> is an ID assigned to identify the soft particle and <kv> is a volumetric spring constant  \citewbibkey{Krueger_thesis}.
\end{itemize}
