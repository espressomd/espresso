...
\item[\opt{quat \var{q1} \var{q2} \var{q3} \var{q4}}]
  If ROTATION is switched on, the rotational degrees of freedom are integrated
  using a quaternion integrator. The \var{quat} command can be used to access
  (read and write) the quaternions of a particle. You typically do not need to
  access these quaternion variables directly (the integrator takes care of this).
  You typically would access the rotational degrees of freedom by using the
  following to commands: \var{omega} and \var{torque}.
\item[\opt{omega \var{x} \var{y} \var{z}}]
  If ROTATION is switched on, particles have additionally 3 rotational degrees of freedom.
  This command can be used to initialize these or to read them. 
\item[\opt{torque \var{x} \var{y} \var{z}}]
  This command can be used to access the torques acting on the rotational degrees of
  freedom of particles (ROTATION must be switched on).
\item[\opt{fix \var{x} \var{y} \var{z}}] Fixes the particle in space.
  ...
  Fixing a particle means, that after the force calculation the velocities
  and the forces of the fixed particles are simply set to zero again.
  When calculating the temperature of your system bye summing up the 
  squared velocities, you have to take into account the reduced number
  of degrees of freedom yourself. \es does not do this for you.
