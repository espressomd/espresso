...
\item[\opt{omega_body/lab \var{x} \var{y} \var{z}}] The command \opt{omega_body} 
  ...
  frame (or laboratory frame). N.B. When the command \opt{omega} is given it is 
  understood that the coordinates are set in the body frame, and printed in the
  laboratory frame, to ensure backward compatibility with older \es{} versions.
  Be very careful when invoking the \opt{omega} command, it should be avoided!
  If you are setting the angular momentum of the particle in the lab frame, the
  orientation of the particle (\opt{quat}) must be set before, otherwise the 
...
\item[\opt{torque_body/lab \var{x} \var{y} \var{z}}] The command \opt{torque_body} 
  ...
  frame (or laboratory frame). N.B. When the command \opt{torque} is given it is 
  understood that the coordinates are set in the body frame, and printed in the
  laboratory frame, to ensure backward compatibility with older \es{} versions.
  Be very careful when invoking the \opt{torque} command, it should be avoided!
  There is another command \opt{tbf} to print the torque in the body frame, also
  see the comment for the print statements. If you are setting the torque of the particle in the lab frame, the
  orientation of the particle (\opt{quat}) must be set before, otherwise the 
  conversion from lab to body frame will not be handled properly.
...
for instance if you output the variable in both frames! The old command \opt{tbf} 
still works and its output can still be read using the command \opt{torque} or
\opt{torque_body}. 
