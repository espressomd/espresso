...
\begin{pysyntax}
  \attributes{
    ...
    vs_relative=\arg{(particle_id, distance, quaternion},
    ...
    temp=\arg{float},
    gamma=\arg{float} | \arg{array of 3 floats},
    gamma_rot=\arg{float} | \arg{array of 3 floats},
    rotation=\arg{bool},
...
\begin{essyntax}
  ...
  \require{9}{\opt{gamma \alt{\var{gtran} \asep \var{gtranx}\fmark{15} \var{gtrany}\fmark{15} \var{gtranz}\fmark{15}}}}
  \require{2,9}{\opt{gamma\_rot \alt{\var{grot} \asep \var{grotx}\fmark{14} \var{groty}\fmark{14} \var{grotz}\fmark{14}}}}
  ...
  \begin{features}
    ...
    \required[14]{ROTATIONAL_INERTIA}
    \required[15]{PARTICLE_ANISOTROPY}
  \end{features}
\end{essyntax}
...
\item[\opt{gamma \var{g}}] If used in combination with the Langevin
  thermostat (as documented in section \ref{sec:thermostat}), sets the
  translational frictional coefficient \var{g} individually for the
  particle with id \var{pid}.This allows to simulate systems
...
\item[\opt{gamma\_rot \alt{\var{grot} \asep \var{grotx} \var{groty} \var{grotz}}}]
  ...
  particle with id \var{pid}. This allows to simulate systems
  containing particles with different diffusion constants.
  Feature \feature{ROTATIONAL\_INERTIA} provides possibility to specify
  different diagonal elements (\var{grotx}, \var{groty}, \var{grotz})
...
Particles can be easily deleted in Python using particle ids or 
ranges of particle ids. In TCL variant \variant{1}, the particle \var{pid} 
is deleted and all bonds referencing it. TCL variant \variant{2} will delete all
...
surface and applies to all charged particles. For \variant{10} the normal vector which is used in the implementation lies in z-direction.
...
$\langle$SHAPE$\rangle$ can be any of:
\begin{enumerate}
	\item \codebox{Wall} \begin{pycode}
			wall = Wall(normal=[n_x, n_y, n_z], dist=d)
		\end{pycode}
		creates a wall shape object with normal vector \codebox{(n_x, n_y, n_z)} at
		distance \codebox{d} from the origin in the direction of the normal vector.
	\item \codebox{Sphere} \begin{pycode} 
			sphere = Sphere(pos=[x, y, z], rad=R, direction=D)
		\end{pycode}
		creates a sphere object with its center at posistion \codebox{(x, y, z)} and
		radius \codebox{R}. 
	\item \codebox{Cylinder} \begin{pycode}
			cylinder = Cylinder(pos=[x, y, z], axis=[a_x, a_y, a_z], rad=R, 
			length=L, direction=D)
		\end{pycode}
		createa a cylinder object at position \codebox{(x, y, z)} with its axis
		pointing torwards \codebox{(a_x, a_y, a_z)}, radius \codebox{R} and length
		\codebox{L}.
	\item \codebox{Rhomboid} \begin{pycode}
			rhomboid = Rhomboid(pos=[x, y, z], a=[a_x, a_y, a_z], b=[b_x, b_y, b_z],
			c=[c_x, c_y, c_z], direction=D)
		\end{pycode}
		creates a rhomboid  defined by one corner located at
		\codebox{(x, y, z)} and three adjacent edges, defined by the three vectors connecting
		the corner p
		with it’s three neighboring corners, a \codebox{(a_x, a_y, a_z )}, b
		\codebox{(b_x, b_y, b_z )} and c \codebox{(c_x, c_y, c_z)}.
	\item \codebox{Maze} \begin{pycode}
			maze = Maze(nsphere=NS, dim=Dim, cylrad=CR, sphrad=SR)
		\end{pycode}
		creates a \codebox{Dim+1}-dimensional maze by \codebox{NS} spheres with
		radius \codebox{SR} per dimension. The spheres build a grid of simple cubic
		symmetry and are connected by cylinders with radius \codebox{CR}.
	\item \codebox{Pore} \begin{pycode}
			pore = Pore(pos=[x, y, z], axis=[a_x, a_y, a_z], length=L,
			smoothing_radius = SR, rad_left=LR, rad_right=RR, outer_rad_left=ORL,
			outer_rad_right=ORR)
		\end{pycode}
		creates a pore at position \codebox{(x, y, z)}, with its axis pointing in the
		direction of \codebox{(a_x, a_y, a_z)} and length \codebox{L}. 
		%TODO full explanation of the paramters
	\item \codebox{Stomatocyte} \begin{pycode}
			stomatocyte = Stomatocyte(position_x=x, position_y=y, position_z=z,
			orientation_x = o_x, orientation_y = o_y, orientation_z = o_z,
			outer_radius = OR, inner_radius = IR, layer_width = LW, direction = D)
		\end{pycode}
		creates a Stomatocyte at position \codebox{(x, y, z)} with orientation
		\codebox{o_x, o_y, o_z} whose outer radius is \codebox{OR}, its inner radius
		is \codebox{IR} and the layer width will be \codebox{LW}.
	\item \codebox{Slitpore} \begin{pycode}
		...
		\end{pycode}
		creates a Slitpore, the meaning of the geometrical parameters can be inferred
		from fig. \ref{fig:slitpore}.
	\item \codebox{SpheroCylinder} \begin{pycode}
			spherocylinder=SpheroCylinder(pos=[x, y, z], axis=[a_x, a_y, a_z],
			length=L, rad=R)
		\end{pycode}
		creates a Sphero-Cylinder at position \codebox{(x, y, z)}, whose cylindrical
		element is aligned in direction \codebox{(a_x, a_y, a_z)}. The cylinder will
		have a length of \codebox{L} and a radius of \codebox{R}. The spherical cap
		will have the same radius. 
	\item \codebox{HollowCone} \begin{pycode}
			hollowCone = HollowCone(position_x = x, position_y = y, position_z = z,
			orientation_x = o_x, orientation_y = o_y, orientation_z = o_z,
			outer_radius = OR, inner_radius = IR, width = W, opening_angle = a,
			direction = D)
		\end{pycode}
		creates a hollow cone whose axis is aligned to \codebox{(a_x, a_y, a_z)}
		located at position \codebox{(x, y, z)}, which inner and outer radii are
		\codebox{IR} and \codebox{OR}, respectively. The width and opening angle are
		given by \codebox{W} and \codebox{a}.
\end{enumerate}

The direction paramter for the shapes specifies wheter it will act torwards the
outside \codebox{D=1} or inside \codebox{D=-1}.
All those shapes can be used as constraints and added to the systems constraints by
passing a initialized shape object to \codebox{system.constraints}
\begin{pycode}
system.constraints.add(shape = shape_object, particle_type=p_type)
\end{pycode}
The extra argument \codebox{particle_type} specifies the nonbonded interaction to be
used with that constraint. There are two further optional parameters
\codebox{penetrable} and \codebox{only_positive} that can be used to fine tune the
behavior of the constraint. If penetrable is set to \codebox{1} then particles can
move through the constraint in this case the other option \codebox{only_positive}
controls wheter the particle is subject to the interaction potential of the wall. If
set to \codebox{1} then the constraint will only act in the direction of the normal
vector. 
