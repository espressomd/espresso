...
\begin{essyntax}
  ...
  \require{2}{\opt{torque_body \var{x} \var{y} \var{z}}}
...
\item[\opt{torque_body \var{x} \var{y} \var{z}}] Sets the rotational force
  of this particle, in the co-rotating frame (body frame). When using this
  command to print the particle torque, they are given in the body frame as well.
  {\bf However}, there exists an alternative command to obtain the torque in the
  laboratory frame, the one you are \emph{usually} interested in. This command
  can only be used when printing and is invoked with \opt{torque_lab}. See
  the section on printing below for more information.  
...
{\bf N.B.} There is a difference between the options \opt{torque_body} and 
\opt{torque_lab}. The first prints the torque in the co-rotating frame, whereas
the second gives the torque in the stationary frame, the body and laboratory 
frames, respectively. One would always want to output the torque in 
the laboratory frame, since it is the frame of interest. However for some tests
involving reading and writing the torque may be desireable to know the torque in
the body frame as well.
