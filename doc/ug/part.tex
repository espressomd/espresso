...
\begin{essyntax}
  ...
  \require{12}{\opt{swimming \alt{v_swim \var{v\_swim} \asep f_swim \var{f\_swim}}}}
  \begin{features}
...
\item[\opt{swimming \alt{v_swim \var{v\_swim} \asep f_swim \var{f\_swim}}}]% TODO: Comment in stuff as soon as pushers and pullers are implemented
%\item[\opt{swimming \alt{v_swim \var{v\_swim} \asep f_swim \var{f\_swim}} \alt{pusher \asep puller}}]
  Enables the particle to be self-propelled in the direction of its quaternion. For setting the quaternion of the particle see \keyword{quat}. The self-propelling will relax to either a constant velocity, when using it with \texttt{v_swim \var{v\_swim}} or to a constant force for \texttt{f_swim \var{f\_swim}}. You may only set one of the possibilities \texttt{\alt{v_swim \var{v\_swim} \asep f_swim \var{f\_swim}}} as you cannot relax to constant force \emph{and} constant velocity at the same time (only for rare special cases).
