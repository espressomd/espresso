...
\item[\opt{swimming \alt{\alt{v_swim \var{v\_swim} \asep f_swim
  ...
  self-propelling will relax to either a constant velocity, when using
  it with \keyword{v_swim \var{v\_swim}} or to a constant force for
  \keyword{f_swim \var{f\_swim}}. You may only set one of the
  possibilities \keyword{v_swim \var{v\_swim}} \emph{or}
  \keyword{f_swim \var{f\_swim}} as you cannot relax to constant force
  \emph{and} constant velocity at the same time (only for rare special
  cases). The option \keyword{off} resets \var{v\_swim} and
  \var{f\_swim} both to $0.0$ and hence disables swimming. This option
  only applies to the langevin thermostat, in this case there is no
  real difference between \keyword{v_swim} and
  \keyword{f_swim}. Still, there is a difference in Lattice-Boltzmann,
  see next item.
\item[{\parbox[t]{\linewidth}{\optlong{swimming \alt{%
  ...
  \keyword{pusher} or \keyword{puller}. You will also need to specify
  a \var{dipole\_length} which determines the distance of the source
  from the particle centre. The key \keyword{rotational_friction}
  allows to set the friction that the orientation of the particle
  experiences in a shear flow.

  You may ask: ``Why are there two methods \var{v\_swim} and
  \var{f\_swim}?''  There is a major difference between these two
  methods, namely that with \var{v\_swim} the flow field \emph{only}
  has a monopolar moment and \emph{only} while the particle is
  accelerating. As soon as the particle reaches a constant speed
  (given by \var{v\_swim}) this moment is gone and the flow field is
  zero! In contrast, \var{f\_swim} always, i.e., while accelerating
  \emph{and} while swimming at constant force, provides a dipolar flow
  field.
\end{arguments}
