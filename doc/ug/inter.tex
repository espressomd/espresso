...
zero force is applied at distances r and bigger. For distances smaller than 
\var{r}, the force is given by
\begin{equation}
  F(d)=F_{\text{max}} \cdot \left( 1 - \frac{d}{r} \right),
\end{equation}
for distances exceeding \var{r}, the force is zero.

The potential energy is given by
\begin{equation}
  V(d)=F_{\text{max}} \cdot (d-r) \cdot \left( \frac{d+r}{2r} - 1 \right),
\end{equation}
which is zero for distances bigger than \var{r} and continuous at distance 
\var{r}.

This is the standard conservative DPD potential and can be used in combination 
with \lit{inter DPD} \ref{sec:DPDinter}. Also it is useful for live 
demonstrations, where you might want to use a big time step to make it look 
interesting on a weak machine but do not want it to crash and the physics don't 
matter.
...
Make sure that you read the papers on ELC (\cite{arnold02c, tyagi08a})
before using it.
...
mainly\cite{arnold12a, tyagi10a, kesselheim11a} before using it.
