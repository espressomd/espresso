...
\var{r_\mathrm{cap}} is set, the maximal force that is generated by
this potential is the force at \var{r_\mathrm{cap}}.  By default, it
is set to 0, \ie no capping.
...
\begin{essyntax}
  ...
  \variant{2} inter coulomb \opt{\var{l_B} \var{method}}
  \opt{\var{parameters}} 
...
\begin{essyntax}
  \variant{1}inter coulomb \var{l_B} p3m 
  \var{r_\mathrm{cut}} \var{mesh} \var{cao} \var{alpha}

  \variant{2}inter magnetic \var{l_B} p3m 
  \var{r_\mathrm{cut}} \var{mesh} \var{cao} \var{alpha}
  \begin{features}
    \required[1]{ELECTROSTATICS}
    \required[2]{MAGNETOSTATICS}
  \end{features}
\end{essyntax}

Variant \variant{1} activates the P3M method to handle the
electrostatic interactions (charge-charge), whereas variant
\variant{2} activates the P3M method to handle the magnetostatic
...
\todo{Are dipole-monopole interactions implemented?}  If only magnetic
dipoles but not charges are present in the sytem, it is advisable to
deactivate the feature \texttt{ELECTROSTATICS} to improve the
performance of the program.  Mixed systems containing magnetic dipoles
and charges can be simulated without problems by enabling both
features, \texttt{ELECTROSTATICS} and \texttt{MAGNETOSTATICS}. For
each kind of particle one should use the prefered algorithm. For
magnetic dipole-dipole interactions the only algorithm implemented at
this moment is dipolar P3M. In principle there is no problem to use a
non P3M method to deal with charges (see folowing sections) while one
uses dipolar P3M to deal with the magnetic dipoles. In case you use
P3M for both charges and magnetic dipoles, make sure that you tune
each P3M method (coulomb/magnetic) to the desired accuracy.
...
There are some additional P3M parameters
\begin{description}
\item[\lit{epsilon}] The dielectric constant of the surrounding
  medium, metallic (\ie infinity) or some finite positive number.
  Defaults to \lit{metallic}.
\item[\lit{n_interpol}] Number of interpolation points for the charge
  assignment function.  When this is set to $0$, interpolation is
  turned off and the function is computed directly. Defaults to
  $32768$.
\item[\lit{mesh_off}] Offset of the first mesh point from the lower
  left corner of the simulation box in units of the mesh
  constant. Defaults to \codebox{{0.5 0.5 0.5}}.
\end{description}

\subsubsection{Tuning P3M}
\begin{essyntax}
  \variant{1}inter coulomb \var{l_B} p3m \alt{tune \asep tunev2}
  ...
  \variant{2}inter magnetic \var{l_B} p3m \alt{tune \asep tunev2}
  accuracy \var{accuracy}\\
  \opt{r_cut \var{r_\mathrm{cut}}}
  \opt{mesh \var{mesh}}
  \opt{cao \var{cao}}
...
\begin{essyntax}
  inter magnetic \var{l_B} mdlc \var{accuracy} \var{gap\_size}
  \opt{\var{far\_cutoff}}
  \begin{features}
    \required{MAGNETOSTATICS}
  \end{features}
\end{essyntax}
...
\subsection{Dipolar interaction, all with all, no replicas}
...
explicitly summing over all pairs. For those directions, in which the
system is periodic (as defined by \texttt{setmd periodic}), it applies
minimum image convention, i.e., the interaction is effectively cut-off
at a half box length.

This method should only be used if using p3m or dlc is not possible.

\subsection{Magnetic dipolar direct sum}
...
dipole-dipole interaction is obtained by explicitly summing over all
pairs. If the system is periodic in one or more directions, replicas
of the system will be explicitly added.  \var{value\_n\_cut} is the
number of replicas that are taken into account in each direction.

As it is very slow, this method is not intendeed to do simulations,
