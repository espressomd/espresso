...
\begin{essyntax}
  ...
  [\var{r_\mathrm{cap}}=0 \var{r_\mathrm{min}}=0]
...
form allows to choose arbitrary (unsigned integer) exponents (\var{e_1} and \var{e_2})
and arbitrary (float) prefactors (\var{b_1} and \var{b_2}) .
Both forms allow capping the force using \lit{inter ljforcecap}, see
...
The capping can be defined using the \var{r_\mathrm{cap}} variable, set to 0
by default (no capping).

An additional parameter can be used to restrict the interaction from a
\emph{minimal} distance \var{r_{\mathrm min}}. This is an optional parameter,
set to 0 by default. (Note: \var{r_{\mathrm min}} has only been implemented for 
\verb lennard-jones .) 
