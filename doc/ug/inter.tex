...
\begin{essyntax}
  \variant{1}inter coulomb p3m 
  \var{r_\mathrm{cut}} \var{mesh} \var{cao} \var{alpha}

  \variant{2}inter magnetic p3m 
  \var{r_\mathrm{cut}} \var{mesh} \var{cao} \var{alpha}
  \begin{features}
...
\end{equation}
Here $\vec{r}_{ij}=\vec{r}_{i}-\vec{r}_{j}$, and $\ell_{Bm}$ is a non
...
\begin{essyntax}
  \variant{1}inter coulomb p3m \alt{tune \asep tunev2}
  ...
  \variant{2}inter magnetic p3m \alt{tune \asep tunev2}
  accuracy \var{accuracy}\\
  \opt{r_cut \var{r_\mathrm{cut}}}
  \opt{mesh \var{mesh}}
  \opt{cao \var{cao}}
...
\begin{code}
inter coulomb \var{parameter\_name} \var{value}+
\end{code}

If only magnetic dipoles are present in the sytem (no charges
...
\begin{essyntax}
  inter magnetic \var{l_{Bm}} dawaanr
  \opt{\var{far\_cutoff}}
  \begin{features}
    \required{MAGNETOSTATICS}
  \end{features}
