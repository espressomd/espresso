...
particles has to be specified explicitly by the \lit{part bond}
...
\subsection{Membrane-collision interaction}
\index{membrane-collision interaction|mainindex}
\index{interactions!membrane-collision|mainindex}

\begin{essyntax}
  inter \var{type1} \var{type2}
  membrane \var{a} \var{n} \var{d_\mathrm{cut}} \var{d_\mathrm{offset}}
  \begin{features}
    \required{MEMBRANE_COLLISION}
  \end{features}
\end{essyntax}
This defines a membrane collision interaction between particles of the types
\var{type1} and \var{type2}, where particle of \var{type1} belongs to one OIF or OIF-like 
object and particle of \var{type2} belongs to another such object. 
...
be created using templates with keyword \var{normal}, because this implicitly 
...
particles (\eg in a simple chain molecule), one can use the same type \var{id}:
\begin{tclcode}
  inter 1 fene 10.0 2.0
...
\index{harmonic dumbbell bond|mainindex}
\index{interactions!harmonic dumbbell|mainindex}
...
\begin{essyntax}
  inter \var{bondid}
  harmonic_dumbbell \var{k_1} \var{k_2} \var{r} \opt{\var{r_\mathrm{cut}}}
  \begin{features}
    \required{ROTATION}
  \end{features}
\end{essyntax}
...
particles.  Additionally the \keyword{quat} of the first particle in
...
constant \var{k_2}.  Keep in mind that the \keyword{quat} will perform
an oscillation around the distance vector and some kind of friction
needs to be present for the \keyword{quat} to relax.

The role of the parameters \var{k_1}, \var{r}, and
\var{r_\mathrm{cut}} is exactly the same as for the harmonic bond.
...
\begin{itemize}
\item Tcl: Variant~\variant{1}, Python: \texttt{type="distance"} is a two body interaction depending on the distance of
...
\item Tcl: Variant~\variant{2}, Python: \texttt{type="angle"} is a three-body angle interaction similar to the
  ...
  forces.
\item Tcl: Variant~\variant{3}, Python: \texttt{type="dihedral"} tabulates a torsional dihedral angle potential (see
...
\index{bonded interactions oif|mainindex}
\index{interactions!bonded oif|mainindex}
...
\index{oif local force|mainindex}
\index{interactions!oif local force|mainindex}
\begin{essyntax}
  inter \var{bondid}
  oif_local_force
  \var{L^0_{AB}} \var{k_s} \var{k_{slin}} \var{\phi} \var{k_b} \var{A_1} \var{A_2} \var{k_{al}}  
     \begin{features}
    \required{OIF_LOCAL_FORCES}
  \end{features}
\end{essyntax}
...
\subsubsection*{Stretching} For each edge of the mesh, \var{L_{AB}} is the current distance between point A 
...
Similarly the analogical forces are assigned to \var{B} and \var{C}. 
...
\index{oif global force|mainindex}
\index{interactions!oif global force|mainindex}

\begin{essyntax}
  inter \var{bondid}
  oif_global_force
  \var{S^0} \var{k_{ag}}   \var{V^0} \var{k_{v}}
   \begin{features}
    \required{OIF_GLOBAL_FORCES}
  \end{features}
\end{essyntax}
...
\subsection{Out direction}
\index{out direction|mainindex}
\index{interactions!out direction|mainindex}
...
\begin{pysyntax}[P3M]
	...
	\object{
		...
		bjerrum_length = \arg{float},
		r_cut = \arg{float},
		mesh = \arg{array of 3 ints},
		cao = \arg{int},
		alpha = \arg{float},
		alpha = \arg{float},
		epsilon = metallic | \arg{float},
		inter = \arg{int}
	}[
		mesh_off = \arg{array of 3 floats}
        ]
	\begin{features}
...
\begin{essyntax}
  inter coulomb \var{l_B} p3m [gpu] \alt{tune \asep tunev2}
...
\keyword{Tune()} can be given a python dictionary that specifies the target parameters for 
the tuning algorithm. If it doesn't contain \keyword{bjerrum_length} or
\keyword{accuracy}, the previous values are used respectively. Other parameters are 
...
\begin{pysyntax}
	\object{
		...
		accuracy = \arg{float},
		precision = \arg{float},
		K_max = \arg{int} | \arg{array of 3 ints},
	}[
		alpha = \arg{float}
	]
	\begin{features}
...
\begin{pysyntax}
  ...
  \object{
    ...
    \begin{features}
        \required{ELECTROSTATICS}
        \required{COULOMB_DEBYE_HUECKEL}
    \end{features}
\end{pysyntax}
...
\begin{pysyntax}
	...
	\object{ 
		...
		delta_mid_top = \arg{float},
		delta_mid_bot = \arg{float}
	}[
		far_cut = \arg{float}
	]
\end{pysyntax}
...
\begin{pysyntax}
	...
	far_switch_radius=\arg{float}
	]
	\begin{features}
		\required{ELECTROSTATICS} \required{PARTIAL_PERIODIC}
	\end{features}
\end{pysyntax}
...
\begin{pysyntax}
	\object{
		...
		far_switch_radius = \arg{float},
		bessel_cutoff = \arg{int}
	]
	\begin{features}
		\required{ELECTROSTATICS} \required{CUDA} \required{PARTIAL_PERIODIC}
		\required{MMM1D_GPU}
	\end{features}
\end{pysyntax}
\begin{essyntax}
...
\begin{essyntax}
  inter coulomb \var{l_B}
  memd adaptive \var{scaling}
  parameters \var{f\_mass} \var{mesh}
  \begin{features}
    \required{ELECTROSTATICS}
  \end{features}
\end{essyntax}

The keyword \keyword{adaptive} after the \keyword{inter coulomb} command will use the implementation with dielectric permittivity dependent on the local salt concentration.
...
\begin{pysyntax}
	\object{
		...
		method_name=\arg{string},
		method_params=\arg{dict},
	}[
	]
	\begin{features}
		\required{SCAFACOS}
...
\index{Dipolar direct sum on gpu|mainindex}

\begin{essyntax}
  inter magnetic \var{l_{B}} dds-gpu
  \begin{features}
    \required{DIPOLES}
    \required{CUDA}
  \end{features}
...
\begin{essyntax}
  inter magnetic \var{l_b} scafacos \var{method} \opt{\var{parameters}} \opt{tolerance_field \var{prec}}
  \begin{features}
    \required{DIPOLES}
    \required{SCAFACOS_DIPOLES}
    \required{SCAFACOS}
