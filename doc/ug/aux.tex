...
returns a random double between 0 and 1 using the standard C++ Mersenne twister random number generator. For python please use the numpy random number generator. 
 \item
\begin{code}
...
To obtain the seeds of the random number generator (C++ Mersenne Twister) which is used in the C core of Espresso use the command
\begin{code}
t\_random seed
\end{code}
\begin{pycode}
...
each of the 'n\_nodes' nodes if they were set by the user, while
\begin{code}
...
\textit{unless you seed your random number generator} at the beginning of the simulation.
\end{itemize}
