...
\item Parameters of elastic moduli. Elastic behaviour can be described by five different elastic moduli: hyperelastic stretching, linear stretching, bending, local and global area preservation and volume preservation. Each of them has its own scaling parameter: $ks, kslin, kb, kal, kag, kv$. Their mathematical formulations have been taken from \cite{dupin07}.
\end{itemize}
...
\begin{essyntax}
  \require{1,2, possibly 3}{oif_add_object}
  object-id  \var{oid}
  template-id  \var{tid}
...
Using a previously defined template \var{tid}, this command creates a new object. Features \verb|OIF_GLOBAL_FORCES| are needed, if the template used the corresponding elastic moduli.
...
\begin{essyntax}
  \require{1,2, possibly 3}{oif_object_output}
  object-id  \var{oid}
...
\begin{essyntax}
  \require{1,2, possibly 3}{oif_object_analyze}
  object-id  \var{oid}
  \opt{origin}
...
\begin{essyntax}
  \require{1,2, possibly 3}{oif_object_set}
  object-id  \var{oid}
