...
an integer \texttt{steps} as parameter integrates the system for
\texttt{steps} time steps. This is implemented using CUDA, so \required{CUDA}
...
\item[sd_random_state] (int[2]) offset of the random number
  ...
  is well defined.
\item[sd_random_precission] (double) precission used for the approximation of
...
\subsection{Floatingpoint precission}
It is possible to switch between double and float as datatype in Stokesian
Dynamics. As GPUs have more single than double precission floating point
units, the single precission is faster. However this can lead to problems, if
the eigenvalues are not precisse enough and get negative. Therfore the default
is double precission. If you want to use single precission, use the feature
