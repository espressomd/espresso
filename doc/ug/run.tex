...
The reaction ensemble\cite{smith1994reaction} allows to simulate chemical reactions especially with varying particle numbers.  Therefore it is similar to the grand canonical ensemble however it applies some further restrictions since the particle numbers are not allowed to fluctuate arbitrarily. Therefore it can be viewed as a grand canonical simulation scheme with inherent rejection sampling with respect to the fluctuations in the particle numbers. Arbitrary chemical reactions can be simulated with the reaction ensemble as long as you know the educts and products of the reaction as well as the equilibrium constant of the reaction. Currently only the version of the reaction ensemble at constant volume and constant temperature is implemented (the version at constant pressure and constant temperature is not implemented). Note that the method also only allows to obtain static observables since the particle dynamics are changed by the Monte Carlo moves of the method.
...
\begin{essyntax}
  ...
  \variant{21} reaction\_ensemble cylindrical\_constraint\_in\_z\_direction center \var{cyl_x} \var{cyl_y} radius \var{radius}
  \variant{22} reaction\_ensemble volume \var{volume}
  \variant{24} reaction\_ensemble wall\_constraints\_in\_z\_direction \var{slab\_z\_start} \var{slab\_z\_end}
  \end{essyntax}
  Additionally we several commands for the Wang-Landau reaction ensemble are available.
  \begin{essyntax}
  \variant{7} reaction\_ensemble wang_landau add degree\_of\_association associated\_type \var{associated\_type} min \var{min\_value} max \var{max\_value} corresponding\_acid\_types \var{list\_of\_corresponding\_types}
  \variant{8} reaction\_ensemble wang\_landau add energy filename \var{energy\_boundary\_filename} delta \var{Delta\_E}
  \variant{9} reaction\_ensemble wang\_landau counter\_ion\_type \var{counter\_ion\_type} [polymer\_start\_id \var{polymer\_start\_id}] [polymer\_end\_id \var{polymer\_start\_id}] [fix_polymer_monomers] 
  \variant{10} reaction\_ensemble wang\_landau final\_wang\_landau\_parameter \var{final\_wang\_landau\_parameter}
  \variant{11} reaction\_ensemble wang\_landau full\_path\_to\_output\_filename \var{full\_path\_to\_output\_filename}
  \variant{12} reaction\_ensemble wang\_landau do\_not\_sample\_reaction\_partition\_function
  \variant{13} reaction\_ensemble wang\_landau update\_maximum\_and\_minimum\_energies\_at\_current\_state
  \variant{14} reaction\_ensemble wang\_landau write\_out\_preliminary\_energy\_run\_results \var{energy\_boundary\_filename}
  \variant{15} reaction_ensemble wang_landau \var{wang\_landau\_setps}
    \variant{16} reaction\_ensemble wang\_landau use\_hybrid\_monte\_carlo (Requires feature CONSTRAINTS)
  \variant{17} reaction\_ensemble [wang\_landau] do
  \variant{20} reaction\_ensemble monte\_carlo\_move\_for\_type \var{type}
  \variant{18} reaction\_ensemble wang\_landau write\_wang\_landau\_checkpoint \var{unique identifier}
  \variant{19} reaction\_ensemble wang\_landau load\_wang\_landau\_checkpoint \var{unique identifier}
  \variant{23} reaction\_ensemble print\_acceptance\_rate
...
\variant{4} sets the temperature for the reaction\_ensemble algorithm. This could in principle be different from the temperature that is used in a MD thermostat. Make sure not to use an MD thermostat in the case of using the Wang-Landau algorithm with Hybrid-Monte-Carlo moves (default off).
...
\variant{9} provides the counter\_ion\_type that gets moved by MC moves as well as optionally the ids of the monomers that correspond to a polymer that should be moved. In the case of no energy collective variable you may additionally (or instead) use MD to change configurations in your simulation. In the case of no energy collective variable \variant{9} is therefore optional. In the case of using an energy collective variable \variant{9} together with \var{counter\_ion\_type} has to be used. Also in this case \var{polymer\_start\_id} and \var{polymer\_start\_id} are optional, since you might not want to change the configuration of your polymer, e.g. if you are trying to simulate a rigid conformation.
\variant{18-19} can be used to checkpoint the Wang-Landau histogram, potential, parameter and the number of executed trial moves.
\variant{20} performs a global monte carlo move for all paricles of the given type.
\variant{21} can be used to constrain the monte carlo moves so that particles are only placed in a cylinder around the z-axis at the given center and with the given radius. In conjunction to that \variant{24} needs to be used to set the volume of the reaction ensemble method to the appropriate (constrained) value.
\variant{23} returns the acceptrance rate for all MC moves that are preformed with reaction\_ensemble monte\_carlo\_move\_for\_type
\begin{arguments}
...
Since the Reaction Ensemble acceptance transition probability can be derived from the grand canonical acceptance transition probability we can use the reaction ensemble to implement grand canonical simulation moves with the same transition probability as in the grand canonical ensemble. This is done via adding reactions that only have educts (for the deletion of particles) or only have products (for the creation of particles). There exists a one to one mapping of the expressions in the grand canonical transition probabilities and the expressions in the reaction ensemble transition probabilities. These kinds of moves are perfect to include the autodissociation of water into the system.
\subsection{How to add the water autodissociation to a simulation}
In order to add the water autodissociation 2 H2O $<=>$ H3O+ +OH-  to a simulation add the following reactions to Espresso ($\emptyset$ (read ex nihilo) means that the reaction has no educts, respectively products. Therefore if $\emptyset$ is a product, particles vanish and if $\emptyset$ is an educt, then particles are created ex nihilo. Do not provide educts or products if they are $\emptyset$.):
\begin{itemize}
\item $\emptyset -> H3O+ + OH-$ with reaction constant $K$
\item $H3O+ + OH- -> \emptyset$ with reaction constant $1/K$,
\end{itemize}
