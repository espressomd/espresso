...
\begin{essyntax}
  \variant{1} minimize_energy \var{f_max} \var{n_steps} \var{gamma} \var{max_displacement}
\end{essyntax}
...
It runs a simplet steepest descent algorithm:

$$p_i = p_i + \min(\gamma F_i, max_displacement).$$
while the maximal force is bigger than f_max or for at most n_steps times.
The combination of gamma and max_displacement can be used to get an poor man's adaptive update.
