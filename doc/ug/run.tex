...
equations of motion. The motion are overdamped. The command \texttt{integrate_sd} with an integer
\texttt{steps} as parameter integrates the system for \texttt{steps}
time steps.
This is implemented using CUDA, so \required{CUDA} has to be available in the system.
...
\item[sd_radius] The hydrodynamic particle radius of the (sperical)
  particles. Only one particle size is currently supported.
\item[sd_viscosity] The viscisity of the fluid.
\end{globvar}
