...
\begin{essyntax}
  ...
  \variant{4} reaction\_ensemble set\_default\_charge\_of\_type \var{type} \var{charge}
  \variant{5} reaction\_ensemble set\_water\_type \var{type}
  \variant{6} reaction\_ensemble length\_scales real \var{real\_length\_in\_meters} simulation \var{corresponding\_length\_in\_simulation\_units}
  \variant{7} reaction\_ensemble check (optional)
  \variant{8} reaction\_ensemble wang_landau add degree\_of\_association associated\_type \var{associated\_type} min \var{min\_value} max \var{max\_value} corresponding\_acid\_types \var{list\_of\_corresponding\_types}
  \variant{9} reaction\_ensemble wang\_landau add energy filename \var{energy\_boundary\_filename} delta \var{Delta\_E}
  \variant{10} reaction\_ensemble wang\_landau counter\_ion\_type \var{counter\_ion\_type} [polymer\_start\_id \var{polymer\_start\_id}] [polymer\_end\_id \var{polymer\_start\_id}] 
  \variant{11} reaction\_ensemble wang\_landau initialize
  ...
  \variant{14} reaction\_ensemble do_not_sample_reaction_partition_function
...
\item[\var{polymer\_start\_id}] the end id of the polymer, optional. Should be set when you have a non fixed polymer and want it to be moved by MC trail moves in order to sample its configuration space.
