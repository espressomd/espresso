...
\item[periodicity] (bool[3]) Specifies periodicity for the three
  ...
  readonly and gives the default setting (1,1,1). Cavet: Be aware of the fact that setting a dimension to be not periodic does not hinder particles from leaving the box in this direction. In this case for keeping particles in the simulation box a constraint has to be set. Further setting a certain dimension to be not periodic does not affect the minimum image convention for this dimension which means that the box length in this direction still has some effect.
