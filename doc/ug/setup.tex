...
\begin{essyntax}
  thermostat dpd \var{temperature} \var{gamma} \var{r\_cut} \opt{ WF \var{wf}  \var{tgamma} \var{tr\_cut} TWF \var{twf}}
\end{essyntax}
\subsubsection{Standard DPD thermostat}
\es implements the DPD thermostat exactly as it is described in \cite{soddeman03a}.
We use the standard {\it Velocity-Verlet} integration scheme, e.g. the DPD
only influences the calculation of the forces. No special measurements
have been taken into account to selfconsistently determine the velocities and
the dissipative forces as it is for example described in \cite{Nikunen03}.
DPD adds a velocity dependent dissipative force and a random force to
the usual conservative pairforces (e.g. Lennard-Jones). 
...
The parameters \var{gamma} \var{r\_cut} define the strength of the friction $\zeta$ and the cutoff radius.

According to the optional parameter WF (can be set to 0 or 1, default is 0) of the thermostat command
the functions $w^D$ and $w^R$ are chosen in the following way ( $ r_{ij} < \var{r\_cut} $ ) :

$$ 
w^D (r_{ij}) = ( w^R (r_{ij})) ^2 = 
   \left\{
   \begin{array}{clcr} 
             1 - \frac{r_{ij}}{r_c} & , \; WF = 0 \\
             1                      & , \; WF = 1
   \end{array}
   \right.
$$
...
The friction (dissipative) and noise (random) term are coupled via the fluctuation-
...
\begin{itemize}
\item As soon as at least one of the two interacting particles is fixed
      (see~\ref{chap:part} on how to fix a particle in space) the dissipative and the
      stochastic force part is set to zero for both particles (you should only change
      this hardcoded behaviour if you are sure not to violate the dissipation fluctuation theorem). 
\item DPD does not take into account any internal rotational degrees of freedom of the particles
      if  ROTATION is switched on. Up to the current version DPD only acts on the translatorial
      degrees of freedom.
\end{itemize}
\subsubsection{Transverse DPD thermostat}
This is an extension of the above thermostat\cite{junghans2008}, which dampes the degrees of freedom
perpendicular on the axis between two particles.
The dissipative force is calculated by
...
The parameters \var{tgamma} \var{tr\_cut} define the strength of the friction and the cutoff in the same way as above.
NOTE: That this thermostat does NOT conserve angular momentum!!!!
\subsubsection{Interaction DPD thermostat}
The DPD thermostat can also be set up as a normal interaction to make friction between different particle types different.
\begin{essyntax}
...
\end{essyntax}
\subsubsection{Other DPD extension}
The feature DPD_MASS make the friction constant mass dependent:
$$ \zeta \to \zeta M_{ij} $$ 
and 
$$ \zeta \to \zeta M_{ij} $$
There are two case implemented DPD_MASS_RED uses the reduced mass:
$$ M_{ij}=2\frac{m_i m_j}{m_i+m_j} $$
There are two case implemented DPD_MASS_LIN uses the reduced mass:
$$ M_{ij}=\frac{m_i+m_j}{2} $$
The prefactor are due to make equal masses have a factor 1.
