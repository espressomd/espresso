...
With the help of the feature \feature{COLLISION_DETECTION}, bonds between
...
Here, \var{d} is the distance at which the bond is created, \var{bond1} denotes
the type of the bond created between the colliding partecles, and \var{bond2}
determines the type of the bond created between the virtual sites (if applicable).
\var{bond1} needs to denote a pair bond, like FENE or harmonic, \var{bond2} can
...
particles around the virtual ones. \var{Type} denotes the particle type of the
...
With the help of the feature \feature{REACTIONS}, one can define three particle
...
Variant \variant{0} defines a reaction with particles of type number \var{rt} 
as reactant, type \var{ct} as catalyzer and type \var{pt} as product. The 
reaction rate is given by \var{r} and to override the dynamically determined 
rate for the back reaction, one can specify it by \var{k_\text{back}}.

Variant \variant{1} disables the reaction. Note that at the moment, there can 
