...
With the help of the feature \feature{REACTIONS}, one can define three particle types to act as reactant, catalyzer, and product. Using these reaction categories, we model the following chemical reaction system:
\begin{eqnarray}
...
\begin{eqnarray}
k_{\text{eq,$\pm$}} & = & \pm \frac{d[pr]}{dt} \;\: = \;\: \mp \frac{d[rt]}{dt} ; \\
k_{\text{ct}} & = & -\frac{d[rt]}{dt} \;\: = \;\: \frac{d[pt]}{dt} ,
\end{eqnarray}
respectively.
...
In future versions of \es the capabilities of the \feature{REACTIONS} feature may be generalized to handle multiple reactant, catalyzer, and product types, as well as more general reaction schemes. Other changes may involve merging the current implementation with the \feature{COLLISION_DETECTION} feature.
