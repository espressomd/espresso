...
\item Variant \variant{0} defines a reaction with particles of type
  ...
  In the default setup a particle of type \emph{reactant\_type} is
  converted to a particle of type \emph{product\_type} and vice versa,
  when the particle is within the reaction range and a reaction move
  takes place.  Unfortunately, in this scheme the simulation does not
  conserve the particle number of each type, respectively.  This is
  the (default) case when the parameter \texttt{swap} is set to
  \emph{off}.  Switching it to \emph{on} enables an alternative
  reaction algorithm in which two particles of opposing types
  (\emph{reactant\_type} and \emph{product\_type}) have to be within
  the reaction range.  Imagine the reaction range around the
  \emph{catalyzer\_type} particle as a sphere.  Upper and lower
  hemisphere are distinguished by the \texttt{quat} property (i.e.\
  the direction) of the catalyzer.  In the \texttt{swap} algorithm it
  does not suffice for these two particles to be anywhere within the
  reaction range, but a particle of \emph{reactant\_type} has to be in
  the lower hemisphere and the partner of \emph{product\_type} in the
  upper hemisphere.  As you may have noticed this scheme models a
  Janus particle where one hemisphere is coated with a catalyzing
  surface (in most cases Pt).  When the reaction move takes place the
  types \emph{and the charges}\footnote{requires
    \texttt{ELECTROSTATICS}} of the pair are interchanged.  This
