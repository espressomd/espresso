...
set up: \lit{agrid}, \lit {dens}, \lit{visc}, \lit{tau}, \lit{friction}. For the other
parameters, the following are taken: \var{bulk\_viscosity}=0, ,
\var{gamma\_odd}=0, \var{gamma\_even}=0,  \var{ f_x} = \var{ f_y} = \var{ f_z} = 0.
...
The LBM implementation provides a fully thermalized LB fluid, \ie all nonconserved modes,
...
\begin{essyntax}
  ...
  \end{features}
\end{essyntax}
The \lit{lbnode} command allows to inspect (\lit{print}) and modify (\lit{set}) single LB nodes. 
...
\section{GPU implementation of the LBM}
...
by configuring with \lit{configure --with-cuda=/path/to/cuda} and compiling
with the  \lit{LB_GPU} flag set in \lit{myconfig.h}. The GPU is switched on
by the command \lit{lbfluid gpu} implying that all following LB-related commands
will be executed for the GPU implementation. If, however the CPU implementation
is to be used, this can be specified by the command  \lit{lbfluid cpu}.
Currently only a subset of
the CPU commands are available for the GPU implementation.
For boundary conditions analogous to the CPU implementation, the flag \lit{LB_BOUNDARY_GPU} 
has to be set in \lit{myconfig.h}.
