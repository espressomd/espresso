...
\keyword{analyze} command can hereby has into two main uses:
Calculation of observables (\keyword{analyze \var{observable}}) and
definition and analysis of topologies in the system (\keyword{analyze
  \var{topology command}}).
...
\begin{essyntax}
  ...
  \{axisofrotation\} \{centerofrotation\} \{beadtypelist\}
  \opt{thetabins}
\end{essyntax}
...
\begin{itemize}
\item \keyword{xbins} histogram bins in x direction.
\item \keyword{ybins} histogram bins in y direction.
\item \keyword{xrange} range for analysis in x direction.
\item \keyword{yrange} range for analysis in y direction.
\item \keyword{axisofrotation} rotate around given axis. (x, y, or z)
\item \keyword{centerofrotation} rotate around given point.
\item \keyword{beadtypelist} only analyze beads of given types.
\item \keyword{thetabins} histogram bins in angle theta.
\end{itemize}
...
be used to shift the not seperated molecules if needed.
...
\todo{Topologies is probably not the best name for this section}
...
If \lit{<rh>} is used, the hydodynamic radius is averaged over all stored
configurations (see section \vref{sec:stored-configs}).
...
For more information, see Chapter 4 and eqaution 4.102 in~\cite{doi86a}.
\minisec{Output format}
\begin{code}
...
\todo{Why is this called bond distances? It has nothing to do with bonds.}
\begin{essyntax}
...
The correlation analysis provides a framework for the calculation of two-time 
averages, as like the correlation of two variables. The general idea is that
a general function $f\left(A\left(t\right),B\left(t+\tau\right)\right)$ is to 
be averaged over time. Several of these quantities can be calculated at
the same time. Each of them is adressed by \lit{analyze correlation \$n}
where \lit{\$n} stands for the number of the corresponding correlation
that is to be calculated.

The functions $A$ and $B$ can either depend on the MD 
state of the system, or on external data sources (files), or can be
given explicitly from the tcl interface. After proper initalization
the correlation module automatically fetches new data from the corresponding
data sources, when the function \lit{analyze correlation \$n update} is called.
$A$, $B$ and also the function $f$ are vector-valued, and their dimensionality
does not need to match. The correlation module keeps a memory of $A$ and $B$
that is uses a hierarchical blocking mechanism (see Frenkel/Smit book for the details)
to reduce the amount of memory necessary to store the data, and also to achieve
nonlinear spacing in the time domain. 

For an example of the usage, please see the script \lit{correlation.tcl} in the samples
directory.

\begin{essyntax}
  \variant{1} analyze correlation \var{n} first_obs \var{A} second_obs \var{B} corr_operation \var{f}
  \opt { tau_lin \var{\tau_{lin}} } \opt { hierarchy_depth \var{h} } \opt{ delta_t \var{\delta_t} }
  \variant{2} analyze correlation update_from_file
  \variant{3} analyze correlation update
  \variant{4} analyze correlation update  \var{A} \var{B} 
\end{essyntax}

Variant \variant{1} initializes the correlations, and while the other variants feed
the correlation with new data depending on their input data flavour. 
Arguments of the initialization are:

\begin{arguments}
\item \lit{first_obs} and \lit{second_obs}: The observables A and B that are to correlated.
  ...
  in. The following functions are already implemented:
  \begin{itemize}
    \item \lit{particle_velocities}
    \item \lit{particle_positions}
    \item \lit{textfile \var{textfilename} \opt{\var{column_1} \dots \var{column_n} }}
      This option allows to read data from an arbitrary text file, organized in columns.
      The name of the textfile is 
    \item \lit{tclinput \var{dimQ} }
      TCL input of length  \var{dimQ} is used as ``observable''. Note that on calling 
      \lit{analyze correlation update} the following numbers are intepreted
      first as components of $A$ and then of $B$.
  \end{itemize}
\item \lit{corr_operation}
  The function $f$ that is calculated from $A$ and $B$. Here also the statement is valid that
  some functions are already implemented and others can be added easily. The following is 
  available:
  \begin{itemize}
    \item hallo
  \end{itemize}
\item \lit{tau_lin}: The number of datapoints taken with linear distance. The default value is 16.  
\item \lit{hierarchy_depth}: The number of levels in the hierarcharchical history of the
  variable. The last datapoint is taken at $t=2^{h}t_{lin}$. The default value is 4.
\item \lit{delta_t}: The time between subsequent updates of $A$, $B$ and $f$. This
  is only needed for the output to have the correct time units. The default value is 1.
\end{arguments}
After correct initialization the call \lit{analyze correlation update} automatically
updates $A$, $B$ and the correlation estimate. All the memory needed for the correlation
estimate and the memory of $A$ and $B$ are allocated, so this method can not run out
of memory once it is initialized. The number of datapoints to be taken are only limited by the
