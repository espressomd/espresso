...
section and in the previous one. All the observables defined and computed
in the previous section were temporary variables in the simulation kernel
and ceased to exist when they were returned at the script level.
The new concept of observables defined here creates variables in the 
kernel which exist for the entire simulation and when desired, their
value is updated and either returned at the script level, or used
by the kernel for further processing.

The ultimate goal of the whole framework is to provide a tool for
data processing on the fly. This is often desirable for 
the calculation of correlation functions. In general,
a correlation function is any function of the form
\begin{equation}
...
arbitrary operator which produces a vector (or tensor) quantity 
$C$ from the two vectors $A$ and $B$. When both $A$ and $B$ are
...
which are useful in the Gree-Kubo relations. In this case the observables
...
a function, but now the observables are aprticle positions and
...
simple liquid decays to zero after several integration steps. 
...
a significant part of cpu time would be spent on passing the control
from script to kernel and back. In more complex systems, the interesting
...
drive to store the data. In addition, efficient algorithms have to be
used for long-time correlations. The trivial correlation algorithm
...
or values entered through the scripting interface. Thus, apart from
...
An example of usage observables and correlations is provided in the script 
\lit{correlation.tcl} in the samples directory.
...
\end{essyntax}
\todo{this is the desired state but not the current behaviour!}
...
\subsubsection{Implemented obersvables and their arguments}
The generic arguments to any observable for which they make sense are
  \begin{itemize}
    \item \lit{types} \var{ type\_list | all } \\
          Restricts observable calculation to a given particle type.
    \todo{All should not be a keyword but a default value?}
    \item \lit{id} \var{ id\_list | all } \\
          Restricts observable calculation to a given list of particle ids.
...
\todo{Missing description of parameters}
  \begin{itemize}
    \item \lit{particle_positions} \\
          Types or ids have to be specified as described above.
    \item \lit{particle_velocities}\\
          Types or ids have to be specified as described above.
    \item \lit{com_velocity}
    \item \lit{com_position}
    \item \lit{particle_positions}
    \item \lit{particle_positions_conditional} \todo{this has been lost in a git stash and has to be re-implemented.}
    \item \lit{stress_tensor}
    \item \lit{stress_tensor_acf_obs}
    \item \lit{particle_currents}
    \item \lit{currents}
    \item \lit{dipole_moment}
    \item \lit{structure_factor}
    \item \lit{interacts_with types \var{type\_list\_1} types \var{type\_list\_2} \var{cutoff} } \\
          For each particle belonging to \var{type\_list\_1} the observable is unity if 
	  a neighbour of a type from \var{type\_list\_2} is found within the distance 
	  defined by the \var{cutoff}. If no such neighbour is found, the observable is
	  zero. 
    ...
    \item \lit{nearest_neighbour_conditional} \\
	  ...
	  incremented when all partners are lo
    ...
    \todo{the following observables do not work propely yet}
...
Once we have defined observables, it is possible to compute correlation functions
from them. This may consume a considerable amount of computer time and requires 
some history to be stored in buffers.
...
Variants\variant{1} to \variant{5} operate only on exisitng correlations.
Variant\variant{6} creates an new correlation.
...
\variant{2} 
With \lit{autoupdate start} it automatically updating the correlation estimates as often as required by the specified \var{dt}.
With \lit{update} it updates the correlation estimates based on the instantaneous
state of the system.
...
we are not aware of any case where it would make physical sense.

\variant{3} 
Correlates all data from history which are left in the buffers. Once this 
...
\item \lit{obs1} and \lit{obs2} \\ 
  are ids of the observables A and B that are to correlated.
...
\item \lit{tau_lin} \\
  ...
  you know how it works. By default, it is set equal to \var{tau\_max} which results
  in the trivial linear correlator. By setting \var{tau\_lin} < \var{tau\_max} the multiple
  tau correlator is switched on. In many cases, \var{tau\_lin}=16 is a good choice
...
\item \lit{compress1} and \lit{compress2} \\
  ...
  Default is \lit{discard} which takes one of the observable
  values and discards the other one. This is safe for all observables but produces poor 
  statistics in the tail. For some observables, \lit{linear} compression can be used which
...
For a more detailed description, please read the cited literature.
