...
\begin{essyntax}
  \variant{1} analyze mindist \opt{\var{type_list_a} \var{type_list_b}}
  \variant{2} analyze distto \var{part_id}
  \variant{3} analyze distto \var{posx} \var{posy} \var{posz}
\end{essyntax}
...
particle \var{part_id} (Variant \variant{2}) or to the coordinates
(\var{posx}, \var{posy}, \var{posz}) (Variant \variant{3}).
...
\begin{essyntax}
 \variant{1} analyze nbhood \var{part_id} \var{r_catch}
 \variant{2} analyze nbhood \var{posx} \var{posy} \var{posz}
 \var{r_catch}
\end{essyntax}
Returns a Tcl-list of the particle ids of all particles within a given
radius \var{r_catch} around the position of the particle with number
\var{part_id} in variant \variant{1} or around the spatial coordinate
(\var{posx}, \var{posy}, \var{posz}) in variant \variant{2}.
...
\begin{essyntax}
  analyze distribution \var{part_type_list_a} \var{part_type_list_b}
  \opt{\var{r_min} \opt{\var{r_max} \opt{\var{r_bins} 
        \opt{\var{log_flag} \opt{\var{int_flag}}}}}}
\end{essyntax}
Returns its parameters and the distance distribution of particles with
types specified in \var{part_type_list_a} (group a) around
particles with types specified in \var{part_type_list_b} (group b)
with distances between \var{r_min} and \var{r_max}, binned into
\var{r_bins} bins. The bins are either equidistant
(\var{log_flag}=0) or logarithmically equidistant ($\var{log_flag}
\geq 1$). If an integrated distribution is required, use
\var{int_flag}=1. The distance is defined as the minimal distance
...
\begin{essyntax}
  ...
  \var{part_type_list_a} \var{part_type_list_b} 
  \opt{\var{r_min} \var{r_max} \var{r_bins}}
\end{essyntax}
Returns its parameters and the radial distribution function (rdf) of
particles with types specified in \var{part_type_list_a} (group a)
around particles with types specified in \var{part_type_list_b} (group
b). The range is given by \var{r_min} and \var{r_max} and is divided
into \var{r_bins} equidistant bins. 
...
\vref{analyze:append}) are stored at equidistant time intervals.
...
\begin{essyntax}
  \variant{1} analyze momentofinertiamatrix { \var{part_type} } 
  \variant{2} analyze find_principal_axis \var{part_type}
\end{essyntax}
Variant \variant{1} returns the moment of inertia matrix for particles
of given type \var{part_type}. The output is a list of all the
elements of the 3x3 matrix. Variant \variant{2} returns the
...
\begin{essyntax}
  analyze aggregation \var{dist_criteria} \var{s_mol_id}
  \var{f_mol_id} \opt{\var{min_contact} \opt{\var{charge_criteria}}}
\end{essyntax}
Returns the aggregate size distribution for the molecules in the
molecule id range \var{s_mol_id} to \var{f_mol_id}. If any monomers in
two different molecules are closer than \var{dist_criteria} they are
considered to be in the same aggregate. One can use the optional
\var{min_contact} parameter to specify a minimum number of contacts
such that only molecules having at least \var{min_contact} contacts
will be considered to be in the same aggregate. The second optional
parameter \var{charge_criteria} enables one to consider aggregation
...
\begin{essyntax}
 analyze necklace \var{pearl_treshold} \var{back_dist} \var{space_dist}
...
\var{pearl_treshold} is the minimal number of monomers in a pearl.
\var{back_dist} is the number of monomers along the chain backbone
which are excluded from the space distance criterion to form clusters.
\var{space_dist} is the distance between two monomers up to which they
...
\begin{essyntax}
  analyze holes \var{prob_part_type_number} \var{mesh_size} 
\end{essyntax}
...
\begin{essyntax}
  ...
  \variant{3} analyze energy bonded \var{bond_type}
  \variant{4} analyze energy nonbonded \var{part_type1} \var{part_type2}
\end{essyntax}
...
\begin{essyntax}
  ...
  \variant{4} analyze pressure bonded \var{bond_type}
  \variant{5} analyze pressure nonbonded \var{part_type1} \var{part_type2}
  \variant{6} analyze pressure nonbonded_intra \opt{\var{part_type}}
  \variant{7} analyze pressure nonbonded_inter \opt{\var{part_type}}
\end{essyntax}
...
contributions to the total pressure; in that case only a two-value
tcl-list is returned, consisting of the requested contribution and its
square.  

\todo{Document arguments nb_inter, nb_intra, tot_nb_inter and
  tot_nb_intra}

Note that the ideal pressure is derived using the temperature, hence
for systems with $\var{temp} = 0$ it will be zero; in those cases it
is preferable to use \codebox{analyze p_IK1} (see
\vref{analyze:p-IK1}) which looks at the velocities of the particles.
...
\begin{code}
\{ \var{total\_pressure} \var{pressure\_square} 
   \{ ideal \var{ideal\_gas\_pressure} \} 
   \{ \{ \var{bond\_type} \var{pressure} \var{pressure\_square} \}
      \vdots
   \}
   \{ \{ \var{nonbonded\_type} \var{pressure} \var{pressure\_square} \}
      \vdots
   \}
   \{ coulomb \var{pressure} \var{pressure\_square}\}
\}
\end{code}
specifying the pressure, its square, the ideal gas pressure, the
...
\label{analyze:p-IK1}
\analyzeindex{pressure tensor}

\begin{essyntax}
 analyze p_IK1 \var{bin_volume} \var{ind_list} \var{flag_all}
\end{essyntax}

Computes the pressure-tensor and all its contributions in the system.
 Note that the
tensorial entries $p^{(k,l)}$ are directly derived from

\begin{equation}
  p^{(k,l)} = \sum_{j>i\in \var{bin}} F_{ij}^{(k)} \cdot r_{ij}^{(l)}
\end{equation}
where \eg $ F_{ij}^{(k)} $ denotes the $k$th entry of the difference
vector between the force vectors of particles $i$ and $j$, while the
ideal pressure is determined from the particles' velocities, \ie

\begin{equation}
  p^{(k,l)}_{\var{ideal}} = \sum_{i\in \var{bin}} v_{i}^{(k)} \cdot
  v_{i}^{(l)}
\end{equation}
contrary to \keyword{analyze pressure ideal} which uses the
temperature. In both cases, the sum is taken over only those particles
whose identities have been provided in \var{ind_list} if
\var{flag_all}=0, otherwise $i$ loops over \var{ind_list} while $j$
represents all particles in the system. Since this command is intended
to follow the Kirkwood-scheme IK1, all particles given in
\var{ind_list} should represent a bin geometry with volume
\var{bin_volume} for the algorithm to make sense, although this is not
checked; hence it is preferable to obtain \var{ind_list} from
\keyword{analyze bins}.

This command is executed on the master node only, hence a higher
number of nodes only slows it down due to the increasing amount of
communication processes required between the nodes to gather all
needed particle information. Furthermore, all interactions including
electrostatic ones are treated with the virial ansatz given above,
which makes non-periodic systems highly recommendable, and which slows
electrostatic systems down due to the long-range interactions.
...
\begin{code}
\{ \var{total\_pressure} \var{pressure\_square}
   \{ total \var{pxx} \var{pxy} \var{pxz} \var{pyx} \dots \} 
   \{ ideal \var{pxx} \var{pxy} \var{pxz} \var{pyx} \dots \} 
   \{ bonded \var{pxx} \var{pxy} \var{pxz} \var{pyx} \dots \} 
     \{ \var{bond\_type} \var{pxx} \var{pxy} \var{pxz} \var{pyx} \dots \}
     \vdots
   \}
   \{ nonbonded \var{pxx} \var{pxy} \var{pxz} \var{pyx} \dots \}
\}
\end{code}

specifying the pressure, its square, its tensorial form, and the
tensorial form of its components ideal, bonded, nonbonded, coulombic
pressure (if applicable); the bonded component is further split up
into its different contributions (FENE, ANGLE, HARMONIC) sorted
according to the interaction type number specified by the
\keyword{inter} command (see section \vref{tcl:inter}).
...
\begin{essyntax}
  \variant{1} analyze set chains \opt{\var{chain_start} \var{n_chains}
    \var{chain_length}}
...
Variant \variant{1} defines a set of \var{n_chains} chains of equal
length \var{chain_length} which start with the particle with particle
number \var{chain_start} and are consecutively numbered (\ie the last
particle in that topology has number $\var{chain_start} +
\var{n_chains}*\var{chain_length}$). Variant \variant{2} will return
...
stored configurations (see section \vref{analyze:append}).
...
configurations (see section \vref{analyze:append}).
...
chains in the system; hence, it gives the same result as \var{idf(0)}
from \lit{analyze internal_dist}. If you want to look only at specific
...
(see section \vref{analyze:append}).
...
\begin{essyntax}
 analyze rdfchain \var{r_min} \var{r_max} \var{r_bins} 
 \opt{\var{chain_start} \var{n_chains} \var{chain_length}}
\end{essyntax}
...
given by \var{r_min} and \var{r_max} and it is divided into
\var{r_bins} equidistant bins.
...
\item the mean-square displacement of the beads in the
  chain (\lit{<g1>})
\item the mean-square displacement in the center of
  gravity of the chain itself (\lit{<g2>})
...
possible to store configurations for later analysis (see section
\vref{analyze:append}).  Using this mechanism, the program is also
able to work quasi-offline by successively reading in previously saved
configurations and storing them to perform any analysis desired
afterwards.
...
\begin{essyntax}
  ...
  \var{col} \opt{\var{a_tau}} \opt{plot}

  \variant{2} uwerr \var{data} \var{nrep} 
  \var{f} \opt{\var{a_tau} \opt{\var{f_args}}} \opt{plot}
\end{essyntax}
...
\item[\var{f_args}] are further arguments to \var{f}.

\item[\var{s_tau}] is the estimate $S=\tau/tau_{\textrm{int}}$ as
