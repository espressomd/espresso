...
 :raw-latex:`\cite{Peskin2002, Crowl2010, KruegerThesis}`). When the
...
-  ibm\_triel is a discretized elastic force with the following syntax
   ...
   e.g. :raw-latex:`\cite{KruegerThesis}`).
...
-  ibm\_tribend is a discretized bending potential with the following
   ...
   where the first one () follows :raw-latex:`\cite{KruegerThesis}` and
   the second one () follows :raw-latex:`\cite{Gompper1996}`. In both
...
-  ibm\_volCons is a volume-conservation force. Without this correction,
   ...
   constant :raw-latex:`\cite{KruegerThesis}`.
