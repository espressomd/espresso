...
3 in .5, 1, 1.5, 2 in -, 0, (0,) ellipse ( and .5\*); (-,-) – (-,);
(+,-) – (+,); (0,0) node[dot,label=above:center] – (0,1.5) node[above]
direction; in 0, (2,) – (2,-); (2,-) – node[blue,below,rotate=-8]
bins\_axial (2,); in .5, 1, 1.5, 2 (,-) – (-.5,-); (2,-) –
node[red,above,rotate=80] bins\_radial (0,-);
...
:raw-latex:`\cite{kolb99a}`. Given no arguments this function calculates
...
in poor solvent :raw-latex:`\citep{limbach03a}`. The first three
...
:raw-latex:`\citet{schmitz00a}`. It identifies free space in the
...
Ref. :raw-latex:`\cite{thompson09a}`. A different formula is used to
...
:raw-latex:`\cite{allen06, bereau15}`. The configurational temperature,
...
book, e.g. :raw-latex:`\cite{rubinstein03a}`. If is used, the radius of
...
For more information, see Chapter 4 and equation 4.102
in :raw-latex:`\cite{doi86a}`.
...
arbitrary numerical time series according to
:raw-latex:`\citet{wolff04a}`.
...
section (3.3) of :raw-latex:`\cite{wolff04a}`. The default is 1.5 and it
