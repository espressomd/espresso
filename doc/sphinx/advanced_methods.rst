...
Their mathematical formulations were inspired by :cite:`dupin07`.

The interaction parameters are set up with :class:`object_in_fluid.OifCellType`::

    import espressomd
    import object_in_fluid
    system = espressomd.System(box_l=[20, 20, 20])
    # cell local and global forces
    cell_type = object_in_fluid.OifCellType(nodesfile="oif_nodes.dat",
        trianglesfile="oif_triangles.dat", system=system,
...
The meaning of the parameters ``ks``, ``kb``, ``kal``, ``kag``, ``kv``
...
any of :math:`k_s, k_{s,lin}, k_b, k_{al}` to zero, the corresponding modulus
can be turned off.

OIF local forces are asymmetric. Particles need to be mentioned in the
correct order. The orientation of a triangle ABC, defined by the normal
vector :math:`n_{ABC} = \overrightarrow{AB} \times \overrightarrow{AC}`,
must point to the inside of the immersed object.
...
Notice that concave objects can be defined if :math:`\theta^0` is
larger than :math:`\pi`.
...
The deviation of the triangle surface :math:`S_{ABC}` is computed from the
triangle surface in the resting shape
:math:`\Delta S_{ABC} = S_{ABC} - S_{ABC}^0`. The area constraint assigns
the following shrinking/expanding force to every vertex:

.. math:: F_{al}(A) = -k_{al}\frac{\Delta S_{ABC}}{\sqrt{S_{ABC}}}w_{A}

where :math:`k_{al}` is the area constraint coefficient, and :math:`w_{A}`
is the unit vector pointing from the centroid of triangle :math:`ABC` to the
vertex :math:`A`. Similarly the analogue forces are assigned to :math:`B`
and :math:`C`.
...
where :math:`S` denotes the current surface of the immersed object, :math:`S_0`
the surface in the relaxed state and :math:`\Delta S = S - S_0`.
