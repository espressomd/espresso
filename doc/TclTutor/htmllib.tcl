...
#  puts "FAMILY: $family - $size $weight $style $adjust_size"
	...
	if {[string match courier [string tolower $family]]} {
	    # puts -nonewline "COURIER: START: $size"
	    incr size 1;
	    # puts "COURIER: END: $size"
	}
...
proc HMtag_table {win param text} {
    ...
    upvar #0 HM$win var
    # puts "HMtag_table : WIN: $win PARAM: $param TEXT: $text "
...
#	    HMcurrent_tags $w
	    ...
	    foreach tag [$w tag names] {
		    ...
		    set realFont [eval HMx_font $font $var(S_adjust_size)]
		    $w tag configure $tag -font $realFont

		    if {[font metrics $realFont -fixed] == 1} {
		        set HMtable(hasFixed) 1.4
		    }

		    set ht [font metrics $realFont -linespace]
		    if {$ht > $max} {
		      set max $ht
		      set maxFont $realFont
		    }
		}
	    }
	    $w configure -font $maxFont
	}
    }

    set charWidth [font measure $maxFont "n"]
    
    # hasFixed will be set to a scaling value if the text widget has
    #  both fixed and variable width characters.  This will probably be
    #  too wide, but that's (usually) better than too narrow.

    if {[font metrics $maxFont -fixed] == 1} {
        set HMtable(hasFixed) 1
#	puts "SET HASFIXED TO 1"
    }
    
    catch {lindex [grid size $parentFrame] 0} columns

    # puts "TABLEsetWindowWidths: PARENT: $parentFrame $parentWin COL: $columns :: CHARWIDTH: $charWidth"
	...
	foreach txtwin $txtwinLst($c) {
	    set fs -1;
	    foreach txt [split [$txtwin get 0.0 end] \n] {
	      set len [string length $txt]
	      ;# The length in the text widget is one larger than the string.
	      incr len
	      if {$len > $txtSize($c)} {
	       set txtSize($c) [expr int($len * $HMtable(hasFixed))]
	       }

	       foreach ww [split $txt] {
	           set sl [string length $ww]
		   if {$sl > $fs} {set fs $sl}
	       }
	      if {[expr $HMtable(hasFixed) * $fs] > $firstSpace($c)} {
	           set firstSpace($c) [expr round($fs * $HMtable(hasFixed))]
		   # puts "COL: $c FS: $fs firstsp: $firstSpace($c)"
	      }
	    }
	}
    }
    ...
    set parentPixelWidth [winfo width $parentFrame]
    # puts "PPWIDTH: $parentPixelWidth "
    set parentPixelWidth [winfo width $parentWin]
    # puts "PPWIDTH: $parentPixelWidth "
#    set parentPixelWidth [expr int($parentPixelWidth *.8) ]
...
#     }
	...
	if {$right > $parentPixelWidth} {
	    set cut [expr int($right - $parentPixelWidth) / ($charWidth)]
	    incr newcolWidth -$cut
            $txtwin configure -width $newcolWidth
#	    update idle; update;
#            puts "$txtwin :: NOW: [$txtwin configure -width] :: $parentPixelWidth -- $newcolWidth :: $right $w $x :: CUT: $cut"
#            foreach {w h x y} [split [winfo geometry $txtwin] "+x"] {}
#  	    set right [expr $w + $x]
	}
    }
...
# puts "ROW - ..$row.. [grid size $parentFrame]"
  ...
  for {set r 0} {$r <= $row} {incr r} {
    set windows ""
    set max -1;
    # puts "THIS ROW: $r :: [grid slaves $parentFrame -row $r]"
    foreach w [grid slaves $parentFrame -row $r] {
        if {[string match [winfo class $w] "Text"]} {
            $w configure -height 99
            update; update idle;

	    set dl1 [$w dlineinfo 1.1]
	    set id [$w index {end - 3 chars}]
            update; update idle;
	    set dl2 [$w dlineinfo $id]
	    # puts "W: $w DL1: $dl1 -- DL2: ..$dl2.. ID: $id [$w get 0.0 end] :: [string length [$w get 0.0 end]]"
	    if {[string match $dl2 ""]} {
               set dl2 $dl1
	    }
	    foreach {x1 y1 w1 h1 b} $dl1 {};
	    foreach {x2 y2 w2 h2 b} $dl2 {};
	    set lines [expr ($y2 - $y1 + $h1) / $h1]
	    lappend windows $w
	 ...
	 foreach w $windows {
	    ...
	    $w configure -font {arial 16 normal roman}
#	    puts "SET HEIGHT: $max :: $y1 $y2 $h1 $h2 [$w get 0.0 end]"
#	    puts "$w: [$w dump 0.0 end]"
#	    puts "$w: [$w configure]"
	}
    }
    ...
    if {![string match $parentWin $win]} {
        # puts "PARENTHT: $parentht   :: $parentWin :: $win"
	    set dl1 [$parentWin dlineinfo 1.1]
	    set id [$parentWin index {end - 3 chars}]
            update; update idle;
	    set dl2 [$parentWin dlineinfo $id]
	    foreach {x1 y1 w1 h1 b} $dl1 {};
	    foreach {x2 y2 w2 h2 b} $dl2 {};
	    set lines [expr ($y2 - $y1 + $h1) / $h1]
	incr parentht $lines
        $parentWin configure -height $parentht    
    }
}
...
proc HMtag_td {win param text} {
    ...
    # puts "HMtag_td : WIN: $win PARAM: $param TEXT: $text :: $t"
...
#    upvar #0 HM$parentWin masterVar
   ...
   #  inserted with appropriate tags.
    ...
    set curFont [HMx_font arial 16 medium r $var(S_adjust_size)]
...
#    $newTable configure -background $col($HMtable(unique))
    ...
    update; update idle;
    # puts "$newTable :: $HMtable(row) -column $HMtable(column) -sticky nsew"
