...
pre-calculated LJ parameters \emph{epsilon}, \emph{sigma} and \emph{cutoff}. With \pypressoinline{shift="auto"},
...
\pypressoinline{system.analysis.energy()}, calculate the temperature from the ideal part and 
...
Make sure that the feature for external forces is compiled in by defining \emph{EXTERNAL\_FORCES} in your myconfig.hpp.
...
ELC is also added to the \emph{actors} of the system and takes \emph{gap size} and \emph{maximum
