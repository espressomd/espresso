...
Espresso can be used both, with python 2 and python 3. In these tutorials, we use python 3 print syntax. If you are on python 2, issue the following command to actuvate it:
\begin{lstlisting}
...
In the energy.dat file we print out the values for kinetic
...
Here, the equations of motion are integrated, and measurements are taken. The radial distribution function is averaged over several measurements, to reduce noise.
\begin{pypresso}
...
For the Lennard-Jones parameters used here, the system is in a liquid state, and particle interactions are significant, as can be seen from the fact that correlations are still visible at several diameters.
...
\begin{itemize}
\item Edit the \texttt{lj\_tutorial.py} file such that half of the particles are placed with type 1. Type 0 is implied for the remaining particles
\item Specify Lennard-Jones interactions for interactions of type 0 particles with other type 0 particles, of type 1 particles with other type 1 particles, and of type 0 particles with type 1 particles. Use the same parameters for interactions within a component, but use a different \texttt{lj\_cut\_mixed} parameter for the cutoff in the Lennard-Jones interaction between type 0 and type 1. Set this parameter to $2^{\frac16}\sigma$ to get de-mixing or to $2.5\sigma$ to get mixing between the two components
\item Record the radial distribution functions separately for components 0 and 0, 1 and 1, and 0 and 1. This can be done by changing the \texttt{type\_list} arguments of the respective analysis command. You can record all three radial distribution functions in a single simulation. It is also possible to write them as several columns into a single file.
\item Plot the radial distribution functions for all three combinations of particle types. The mixed case will differ significantly, depending on your choice of \texttt{lj\_cut\_mixed}.
\end{itemize}
