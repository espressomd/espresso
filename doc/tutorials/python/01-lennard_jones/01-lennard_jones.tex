...
The scripting interface (Python) is used to setup the system (particles, boundary onditions, interactions, ...), control the simulation, run analysis, and store and load results.
...
\section{Overview over a simulatino script}
...
crucial system parameters like the time step and the size of the simulatio box (\texttt{time\_step}, and \texttt{box\_l}). At any time, only one instance of the System class can exist.
\begin{lstlisting}
...
In this tutorail, the NVE-ensemble will be maintained by means of the Langevin thermostat.

In \es{}, the teermostat is set as follows:
{\small\vspace{0,2cm}
\begin{lstlisting}
...
Non-bonded interactions act between all particls of a given combination of particle types.
In this tutorial, we use the Lennard-Jones non-bonded interaction.
The interactaion of two particles of type 0 can be setup as follows:
{\small\vspace{0,2cm}
\begin{lstlisting}
lj1_eps     = 1.0
lj1_sig     = 1.0
...
This is done by limiting the maximum force between two particles, integrating the equations of motion, and increasing the limit step by step.
...
\end{lstlistingg}
In this code fragmetn, you can also see, how the analysis routines can be used to obtain information about the simulation system, and how to integrate the equation of motion.
...
\subsection{Integrating the equaitons of motion, taking measurements}
\noindent After we have set the necessary environment and warmed up our system, we can now start with the actual 
simulation. To analyse our data after the simulation has completed, we open some files for writing the data in. 
