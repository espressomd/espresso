...
require_feature "CATALYTIC_REACTIONS"
...
set x0pnt [expr 0.5*$box_l]
set y0pnt [expr 0.5*$box_l]
set z0pnt [expr 0.5*$box_l]
...
set nB [expr floor($csmall*pow($box_l,3))]
set nA $nB

puts "Number of reactive A particles:  $nB"
puts "Number of reactive B particles:  $nA"
...
if { $active == 1 } {
    reaction reactant_type 1 catalyzer_type 0 product_type 2 range $cat_range ct_rate $cat_rate swap on
}
...
for { set cnt 0 } { $cnt < 5 } { incr cnt } {
    ...
    # (AVACF) correlator
    set ang_id [observable new particle_body_angular_momentum id $cent]
