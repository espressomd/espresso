...
myconfig.hpp to be set throughout the whole tutorial:
...
box_l = 15        #side length of our cubic
...
\end{verbatim}
You can switch from one method to the other simply by changing the ``mode'' parameter from ``reaction\textunderscore ensemble'' to ``constant\textunderscore pH\textunderscore ensemble''. Now the system is ready to be simulated.
...
\section{Example two: linear weak polyelectrolyes}
...
\begin{figure}[tb]
	...
	\caption{Titration curves for a weak polyelectrolyte chain composed by $N_0=50$ titratable monomers. Yellow squares: Constant pH method; light blue trianges: Reaction Ensemble method:; blue solid line: ideal behavior.}
	\label{titration}
\end{figure}
