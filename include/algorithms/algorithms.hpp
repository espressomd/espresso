...
template <class ParticleRange, class ValueOp, class WeightOp>
auto weighted_average(ParticleRange const &particles, ValueOp &&value_op,
                      WeightOp &&weight_op) {

  auto const ws =
      detail::weighted_sum(particles, std::forward<ValueOp>(value_op),
                           std::forward<WeightOp>(weight_op));
  return ws.first / ws.second;
}

template <class ParticleRange, class ValueOp>
auto average(ParticleRange const &particles, ValueOp value_op) {
  auto one = [](auto const &p) { return 1; };
  return weighted_average(particles, value_op, one);
}
