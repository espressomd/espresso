...
/////////// Generate createPart
  ...
  for (int i = firstPartId; i < mesh_nnodes+firstPartId; i++){
...
// Generation of bending force bonds:
	...
	for (int i = 0; i < n_BenBond; i++) { // Run over all edges
		p2id = mesh_edges[i][0]; // Put IDs of points to p2id,p3id
		p3id = mesh_edges[i][1]; 
		for(int k = 0; k < 3; k++){	// Put coordinates of the edges's points
		...
		for (int k = 0; k < mesh_ntriangles; k++) { // Run over all triangles and determine the two triangles with the common current edge
...
// Generation of local area force bonds:
	...
	for (int i = 0; i < n_localAreaBond; i++) {
		for(int k = 0; k < 3; k++){		
