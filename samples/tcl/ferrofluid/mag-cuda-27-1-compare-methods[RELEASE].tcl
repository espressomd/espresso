...
if {$rnd_val_for_p_type < $PHI_l} {
    #only single component should be nonzero
    set dx 0.0
    set dy 0.0
    set dz [expr 1.0 * ($m_l/$m0)]
	set massval_large [expr $M_l/$M0]
    set I_val_large [expr 0.1*($M_l/$M0)*pow($d_l_hydrod/$SIGMA,2.0)]
    ...
    set dx 0.0
    set dy 0.0
    set dz [expr 1.0 * ($m_mean/$m0)]
	set massval_small [expr 1.0]
	set I_val_small [expr 0.1]
...
puts "====================="
puts "In silico time units: $t0 s"
puts "Small nanoparticles"
puts "d_small_hydrod=$d_mean_hydrod m"
