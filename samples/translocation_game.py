...
camera_position = [snake_startpos[0], snake_startpos[1], system.box_l[2]*zoom],
...
harmonic_head = espressomd.interactions.HarmonicBond(k=harmonic_k, r_0=0.5*(snake_head_sigma + snake_bead_sigma))
harmonic_bead = espressomd.interactions.HarmonicBond(k=harmonic_k, r_0=snake_bead_sigma)
...
for i in range(snake_n):
    if i == 0:
        p_head = system.part.add(pos = snake_startpos,type = snake_head_type, fix = [0,0,1], mass = snake_head_mass, temp = temperature_snake, gamma = gamma_snake_head)
#    elif i == snake_n - 1:
#        system.part.add(pos = snake_startpos,type = snake_head_type, fix = [0,0,1], mass = snake_head_mass, bonds = (harmonic_bead if (i > 1) else harmonic_head, i - 1))
        ...
        system.part.add(pos = snake_startpos + np.array([0,-1,0]) * (0.5*(snake_head_sigma + snake_bead_sigma) + (i-1) * snake_bead_sigma), bonds = (harmonic_bead if (i > 1) else harmonic_head, i - 1), type = snake_bead_type, fix = [0,0,1], mass = snake_bead_mass, temp = temperature_snake, gamma = gamma_snake_bead)
...
system.non_bonded_inter[snake_head_type, snake_head_type].lennard_jones.set_params(epsilon=1.0, sigma=snake_head_sigma, cutoff= WCA_cut * snake_head_sigma, shift="auto")

sm = 0.5*(snake_head_sigma+snake_bead_sigma)
system.non_bonded_inter[snake_bead_type, snake_head_type].lennard_jones.set_params(epsilon=1.0, sigma=sm, cutoff= WCA_cut * sm, shift="auto")

system.non_bonded_inter[snake_bead_type, snake_bead_type].lennard_jones.set_params(epsilon=1.0, sigma=snake_bead_sigma, cutoff= WCA_cut * snake_bead_sigma, shift="auto")

sm = 0.5*(snake_head_sigma+cylinder_sigma)
system.non_bonded_inter[snake_head_type, cylinder_type].lennard_jones.set_params(epsilon=10.0, sigma=sm, cutoff= WCA_cut * sm, shift="auto")

sm = 0.5*(snake_bead_sigma+cylinder_sigma)
system.non_bonded_inter[snake_bead_type, cylinder_type].lennard_jones.set_params(epsilon=10.0, sigma=sm, cutoff= WCA_cut * sm, shift="auto")

sm = 0.5*(bubble_sigma+snake_bead_sigma)
system.non_bonded_inter[snake_bead_type, bubble_type].lennard_jones.set_params(epsilon=bubble_snake_eps, sigma=sm, cutoff= 2.5 * sm, shift="auto")

sm = 0.5*(bubble_sigma+snake_head_sigma)
system.non_bonded_inter[snake_head_type, bubble_type].lennard_jones.set_params(epsilon=1.0, sigma=sm, cutoff= WCA_cut * sm, shift="auto")

sm = 0.5*(bubble_sigma+cylinder_sigma)
system.non_bonded_inter[bubble_type, cylinder_type].lennard_jones.set_params(epsilon=1000.0, sigma=sm, cutoff= 2.5 * sm, shift="auto")

system.non_bonded_inter[bubble_type, bubble_type].lennard_jones.set_params(epsilon=bubble_bubble_eps, sigma=bubble_sigma, cutoff= 2.5 * bubble_sigma, shift="auto")
...
system.constraints.add(shape=Wall(dist = -box[1], normal=[0, -1, 0]), particle_type=cylinder_type, penetrable=True)

system.constraints.add(shape=SimplePore(center=0.5*box, axis=[1, 0, 0], length=pore_length, radius= pore_radius, smoothing_radius=5), particle_type=cylinder_type, penetrable=True)
...
while (n < bubbles_n):
    ...
    bpos = [np.random.random() * (pore_xl - snake_head_sigma*4) + snake_head_sigma * 2, np.random.random() * box[1], box[2]*0.5]
    system.part.add(pos = bpos, type = bubble_type, fix = [0,0,1], mass = bubble_mass, temp = temperature_bubbles, gamma = gamma_bubbles)
...
bpos = [np.random.random() * (pore_xl - snake_head_sigma*4) + snake_head_sigma * 2, np.random.random() * box[1], box[2]*0.5]
p_temp_inc = system.part.add(pos = bpos, type = temp_change_inc_type, fix = [1,1,1])
...
def main():
    ...
    while(True):
        ...
        # CAMERA TRACKING
        zoom_a = (z_eq - zoom) * 0.2 - zoom_v * 0.8 + v_f * 0.005*np.linalg.norm(system.part[0].v)
        ...
        if ud_cnt > 100:
            ud_cnt = 0
            pl = system.part.select(lambda p: p.pos[0]<pore_xl and p.type == bubble_type)
            pr = system.part.select(lambda p: p.pos[0]>pore_xr and p.type == bubble_type)
            Nl = len(pl)
            Nr = len(pr)
            for p in pl:
                p.temp = temp_l
        ...
        if repos_temp_inc or tincF > 5000:
            ...
            if np.random.random() < 0.5:
                p_temp_inc.pos = [np.random.random() * (pore_xl - snake_head_sigma*4) + snake_head_sigma * 2, np.random.random() * box[1], box[2]*0.5]
            else:
                p_temp_inc.pos = [pore_xr +  np.random.random() * (pore_xr - pore_xl - snake_head_sigma*4) + snake_head_sigma * 2, np.random.random() * box[1], box[2]*0.5]

        if repos_temp_dec or tdecF > 5000:
            tdecF = 0
            if np.random.random() < 0.5:
                p_temp_dec.pos = [np.random.random() * (pore_xl - snake_head_sigma*4) + snake_head_sigma * 2, np.random.random() * box[1], box[2]*0.5]
            else:
                p_temp_dec.pos = [pore_xr +  np.random.random() * (pore_xr - pore_xl - snake_head_sigma*4) + snake_head_sigma * 2, np.random.random() * box[1], box[2]*0.5]
