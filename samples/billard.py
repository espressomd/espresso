...
visualizer = openGLLive(system,
                        ...
                        particle_type_colors=[[1, 1, 1, 1], [
                            1, 0, 0, 1], [0, 0, 1, 1], [0.2, 0.2, 0.2, 1]],
...
def main():
    ...
    system.constraints.add(shape=Wall(dist=table_h, normal=[
                           0.0, 1.0, 0.0]), particle_type=types['table'], penetrable=1)

    system.constraints.add(shape=Wall(
        dist=0.01, normal=[1.0, 0.0, 0.0]), particle_type=types['wall'], penetrable=1)
    system.constraints.add(shape=Wall(
        dist=-(table_dim[0] - 0.01), normal=[-1.0, 0.0, 0.0]), particle_type=types['wall'], penetrable=1)
    system.constraints.add(shape=Wall(
        dist=0.01, normal=[0.0, 0.0, 1.0]), particle_type=types['wall'], penetrable=1)
    system.constraints.add(shape=Wall(dist=-(table_dim[1] - 0.01), normal=[
                           0.0, 0.0, -1.0]), particle_type=types['wall'], penetrable=1)
# system.constraints.add(shape=Wall(dist=table_dim[0],normal=[0.0,0.0,-1.0]),particle_type=2)
    for h in hole_pos:
        system.constraints.add(shape=Cylinder(center=(np.array(h) - np.array([0, table_h * 0.5, 0])).tolist(), axis=[
                               0, 1, 0], radius=hole_rad, length=0.51 * table_h, direction=1), particle_type=types['hole'], penetrable=1)
    ...
    system.part.add(id=0, pos=ball_start_pos,
                    type=types['cue_ball'], q=charge[0], mass=mass[0])
    spawnpos = []
    ...
    for i in range(5):
        for j in range(i + 1):
            ...
            system.part.add(id=pid, pos=pos,
                            q=charge[0], mass=mass[0], type=t, fix=[0, 1, 0])
            spawnpos.append(pos)
            pid += 1
