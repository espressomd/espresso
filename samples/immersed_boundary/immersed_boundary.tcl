...
# puts "N_drag = $N_drag "
puts "Radius = $r"
puts "Shear Rate =  [expr ($G * $kS)] / [expr ($nu * $r)] =  [expr ( ($G * $kS) / ($nu * $r) )]"
...
puts $simInfo "Radius = $r"
puts $simInfo "Shear Rate =  [expr ($G * $kS)] / [expr ($nu * $r)] =  [expr ( ($G * $kS) / ($nu * $r) )]"
puts $simInfo "u = [expr  ($shear_rate * ($boxz - $H)/2)]"
...
lbboundary wall dist [expr $H/2] normal 0. 0. 1. velocity [expr $u] 0 0 
lbboundary wall dist [expr -$boxz + ($H/2)] normal 0. 0. -1. velocity [expr -$u] 0 0
