...
for i in range(n_ionpairs):
    system.part.add(id=rid, type=types["PF6"],  pos=np.random.random(3) * box_l, q=charges["PF6"], mass = masses["PF6"])
...
for i in range(n_ionpairs):
    ...
    system.part.add(id = rid, type = types["BMIM_COM"], pos = pos_com, mass = masses["BMIM_COM"], rinertia = [646.284, 585.158, 61.126], temp = 0, gamma = 0, rotation=[1,1,1])
    cation_com_ids.append(rid)
    com_id = rid
    rid+=1
    system.part.add(id = rid, type = types["BMIM_C1"], pos = pos_com + [0, -0.527, 1.365], q = charges["BMIM_C1"])
    ...
    if args.drude:
        ...
        rid += 1
    system.part.add(id = rid, type = types["BMIM_C2"], pos = pos_com + [0, 1.641, 2.987], q = charges["BMIM_C2"])
    system.part[rid].vs_auto_relate_to(com_id)
    ...
    if args.drude:
        ...
        rid += 1
    system.part.add(id = rid, type = types["BMIM_C3"], pos = pos_com + [0, 0.187, -2.389], q = charges["BMIM_C3"])
    system.part[rid].vs_auto_relate_to(com_id)
...
system.minimize_energy.init(f_max = 5.0, gamma = 0.01, max_steps = 100000, max_displacement= 0.01)
...
if args.drude:
    ...
    for i in anion_ids:
        drude_helpers.add_drude_particle_to_core(system, harmonic_bond, thermalized_dist_bond, system.part[i], i+1, types["PF6_D"], polarizations["PF6"], args.mass_drude, coulomb_prefactor)
    for i in cation_c1_ids:
        drude_helpers.add_drude_particle_to_core(system, harmonic_bond, thermalized_dist_bond, system.part[i], i+1, types["BMIM_C1_D"], polarizations["BMIM_C1"], args.mass_drude, coulomb_prefactor)
    for i in cation_c2_ids:
        drude_helpers.add_drude_particle_to_core(system, harmonic_bond, thermalized_dist_bond, system.part[i], i+1, types["BMIM_C2_D"], polarizations["BMIM_C2"], args.mass_drude, coulomb_prefactor)
    for i in cation_c3_ids:
        drude_helpers.add_drude_particle_to_core(system, harmonic_bond, thermalized_dist_bond, system.part[i], i+1, types["BMIM_C3_D"], polarizations["BMIM_C3"], args.mass_drude, coulomb_prefactor)
    ...
    if args.intra_ex:
        ...
        for i in cation_c1_ids:
            drude_helpers.add_intramol_exclusion_bonds(system, [i+1,i+3,i+5], [i,i+2,i+4])
