...
class Controls(HasTraits):
    ...
    view = View(
        Group(
            Item('temperature', editor=RangeEditor(low_name='min_temp', high_name='max_temp')),
            ...
            Item('number_of_particles', editor=RangeEditor(low_name='min_n', high_name='max_n', is_float=False)),
    ...
    def _temperature_fired(self):
        ...
        data1 = int((self.temperature - self.min_temp) / (self.max_temp - self.min_temp) * 127)
        data2 = data1
        if self.midi_output is not None:
    ...
    def _pressure_fired(self):
        ...
        if pressure_log_flag:
            data1 = limit_range(int(127 * (numpy.log(self.pressure) - numpy.log(self.min_press))/(numpy.log(self.max_press) - numpy.log(self.min_press))), minval=0, maxval=127)
        else:
            data1 = limit_range(int((self.pressure - self.min_press) / (self.max_press - self.min_press) * 127), minval=0, maxval=127)
        data2 = data1
        if self.midi_output is not None:
            self.midi_output.write_short(status, data1, data2)
            ...
            if controls.midi_input is not None and controls.midi_input.poll():
                ...
                for event in events:
                    ...
                    if status == controls.MIDI_NUM_TEMPERATURE:
                        temperature = data2 * (controls.max_temp - controls.min_temp) / 127 + controls.min_temp
                        controls.temperature = limit_range(temperature, controls.min_temp, controls.max_temp)
                    elif status == controls.MIDI_NUM_VOLUME:
                        volume = data2 * \
                            (controls.max_vol - controls.min_vol) / \
                            127 + controls.min_vol
                        controls.volume = limit_range(volume, controls.min_vol, controls.max_vol)
                        controls.ensemble = 'NVT'
                    elif status == controls.MIDI_NUM_PRESSURE:
                        pressure = pressure_from_midi_val(data2, controls.min_press, controls.max_press)
                        ...
                        controls.number_of_particles = limit_range(npart, controls.min_n, controls.max_n)
                        ...
                        if data2 < 65:
                            ...
                            mayavi_rotation_angle -= mayavi_rotation_angle_step*(data2-64)
...
def update_plot():
    ...
    data_len = numpy.array([len(plt1_x_data), len(plt1_y_data), len(plt2_x_data), len(plt2_y_data)]).min()
