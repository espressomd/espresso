...
# ::system_generation::find_fudgearea_cyl --
#
...
#
proc ::system_generation::find_fudgearea_cyl { r1  box_l init_area nlipids } {
    set init_step 0.01

    set init_count [find_n_cyl $r1 $box_l $init_area ]
    set init_diff [expr $init_count - $nlipids]
    mmsg::send [namespace current] "init: A: $init_area R: $r1 diff: $init_diff cnt: [find_n_cyl $r1 $box_l $init_area]"
    ...
    mmsg::send [namespace current] "bracket: low: $lower_b [find_n_cyl $r1 $box_l $lower_b] up: $upper_b [find_n_cyl $r1 $box_l $upper_b] "
	...
	set diff [expr  [find_n_cyl $r1 $box_l $area ]  - $nlipids]
	mmsg::send [namespace current] "bracket: low: $lower_b [find_n_cyl $r1 $box_l $lower_b] up: $upper_b [find_n_cyl $r1 $box_l $upper_b] new: $area [find_n_cyl $r1 $box_l $area] "
	if { $diff > 0 } {
	    set lower_b $area
	} elseif { $diff < 0 } {
	    set upper_b $area
...
# ::system_generation::uniform_list --
#
#
proc ::system_generation::uniform_list_cyl { num range } {
...
#
# ::system_generation::create_cylinder --
#
...
#
proc ::system_generation::create_cylinder { topo box_l args } {
	...
	{random    "whether to use random placement of molecules" }
	{startmol.arg     0   "starting val for molecule types"  }
	{bondl.arg     1.0   "bond length between atoms"  }
    }
    set usage "Usage: create_uniform_cylinder \[center:initarea:random:startmol:bondl]: "
    ...
    set ellz [lindex $box_l 2]		;# length of box in z-direction
    ...
    # in the outer layer.
    set fudgedlipidarea [ find_fudgearea_cyl $rad $box_l $marea $n_mols_outer ]
    ...
    set exmols [expr  [find_n_cyl $rad $box_l $fudgedlipidarea ]  - $n_mols_outer]
    if { $exmols > 0 } {
	if { $nmoltypes > 1 } {
    ...
    set fudgedlipidarea [ find_fudgearea_cyl $rad $box_l $marea $n_mols_inner ]
    set exmols [expr  [find_n_cyl $rad $box_l $fudgedlipidarea ]  - $n_mols_inner]
    if { $exmols > 0 } {
	if { $nmoltypes > 1 } {
	    mmsg::warn [namespace current] "molecule proportions may be incorrect!!"
	}
