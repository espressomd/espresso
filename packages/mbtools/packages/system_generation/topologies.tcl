...
#----------------------------------------------------------#
# ::system_generation::create_simple_topo --
#
# Creates a basic topology from a single molecule type
#
proc ::system_generation::create_simple_topo {n_mols beads_per_mol  args } {
...
# 
# ::system_generation::shuffle_topo--
#  
...
proc ::system_generation::shuffle_topo {topo} {
    mmsg::send [namespace current] "shuffling topology"

    set startmol [minmoltype $topo ]
    set startpart [minpartid $topo ]

    set full_n_molslist [listnmols $topo]
    ...
    set molsizes [listmollengths $topo ]
    set pnum $startpart
...
#
# ::system_generation::sort_topo -- 
#
# Sort a topology into ascending molecule type order
#
#
proc ::system_generation::sort_topo { topo } {    
    set n_molstotal [llength $topo]
    set maxtp [maxmoltypeid $topo]
...
#
# ::system_generation::join_topos--
#
# Join two topologies
#
proc ::system_generation::join_topos { topo1 topo2 args } {
