...
# ::system_generation::find_fudgearea --
# 
...
#
proc ::system_generation::find_fudgearea { r1  init_area nlipids } {
    set init_step 0.01

    set init_count [find_n $r1 $init_area ]
    set init_diff [expr $init_count - $nlipids] 
    mmsg::send [namespace current] "init: A: $init_area R: $r1 diff: $init_diff cnt: [find_n $r1 $init_area]"
    ...
    mmsg::send [namespace current] "bracket: low: $lower_b [find_n $r1 $lower_b] up: $upper_b [find_n $r1 $upper_b] "
	...
	set diff [expr  [find_n $r1 $area ]  - $nlipids]
	mmsg::send [namespace current] "bracket: low: $lower_b [find_n $r1 $lower_b] up: $upper_b [find_n $r1 $upper_b] new: $area [find_n $r1 $area] "
	if { $diff > 0 } {
	    set lower_b $area
	} elseif { $diff < 0 } {
	    set upper_b $area
...
#
#
proc ::system_generation::uniform_list { num range } {
...
#
# ::system_generation::create_sphere --
#
...
#
proc ::system_generation::create_sphere { topo box_l args } { 
    ...
    set usage "Usage: create_uniform_sphere \[center:initarea:random:startmol:bondl]: "
