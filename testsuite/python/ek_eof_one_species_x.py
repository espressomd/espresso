...
# Set up the (LB) electrokinetics fluid
    ...
    counterions = electrokinetics.Species(density=density_counterions, D=0.3, valency=valency, ext_force = [force,0,0])
    ek.add_species(counterions)


# Set up the walls confining the fluid and carrying charge

    ek_wall1 = electrokinetics.EKBoundary(charge_density=sigma/(agrid*padding),shape=shapes.Wall(normal=[0, 0, 1], dist=padding))
    system.ekboundaries.add(ek_wall1)
    ek_wall2 = electrokinetics.EKBoundary(charge_density=sigma/(agrid*padding),shape=shapes.Wall(normal=[0, 0, -1], dist=-(padding+width)))
...
# obtain the desired xi value
    ...
    for i in range(int(box_z/agrid)):
      ...
      # density
        measured_density = counterions[int(box_x/(2*agrid)), int(box_y/(2*agrid)),i].density
      ...
      # velocity
        measured_velocity = ek[int(box_x/(2*agrid)), int(box_y/(2*agrid)), i].velocity[0]
      ...
      # diagonal pressure tensor
        ...
        measured_pressure_xx = ek[int(box_x/(2*agrid)), int(box_y/(2*agrid)), i].pressure[(0,0)]
        calculated_pressure_xx = hydrostatic_pressure(ek,position, xi, bjerrum_length, (0,0),box_x,box_y,box_z,agrid)
        measured_pressure_yy = ek[int(box_x/(2*agrid)), int(box_y/(2*agrid)), i].pressure[(1,1)]
        calculated_pressure_yy = hydrostatic_pressure(ek,position, xi, bjerrum_length, (1,1),box_x,box_y,box_z,agrid)
        measured_pressure_zz = ek[int(box_x/(2*agrid)), int(box_y/(2*agrid)), i].pressure[(2,2)]
        ...
        pressure_difference_zz = abs(measured_pressure_zz - calculated_pressure_zz)
      ...
      # xy component pressure tensor
        measured_pressure_xy = ek[int(box_x/(2*agrid)), int(box_y/(2*agrid)), i].pressure[(0,1)]
      ...
      # yz component pressure tensor
        measured_pressure_yz = ek[int(box_x/(2*agrid)), int(box_y/(2*agrid)), i].pressure[(1,2)]
      ...
      # xz component pressure tensor
        measured_pressure_xz = ek[int(box_x/(2*agrid)), int(box_y/(2*agrid)), i].pressure[(0,2)]
        calculated_pressure_xz = pressure_tensor_offdiagonal(position, xi, bjerrum_length, force)
    ...
    print("Pressure deviation xx component: {}".format(total_pressure_difference_xx))
    print("Pressure deviation yy component: {}".format(total_pressure_difference_yy))
    print("Pressure deviation zz component: {}".format(total_pressure_difference_zz))
    print("Pressure deviation xy component: {}".format(total_pressure_difference_xy))
    ...
    self.assertTrue(total_density_difference < 5.0e-05,"Density accuracy not achieved")
    self.assertTrue(total_velocity_difference < 5.0e-06,"Velocity accuracy not achieved")
    self.assertTrue(total_pressure_difference_xx < 5.0e-05,"Pressure accuracy xx component not achieved")
    self.assertTrue(total_pressure_difference_yy < 5.0e-07,"Pressure accuracy yy component not achieved")
    self.assertTrue(total_pressure_difference_zz < 5.0e-07,"Pressure accuracy zz component not achieved")
    self.assertTrue(total_pressure_difference_xy < 5.0e-10,"Pressure accuracy xy component not achieved")
    self.assertTrue(total_pressure_difference_yz < 5.0e-10,"Pressure accuracy yz component not achieved")
    self.assertTrue(total_pressure_difference_xz < 7.5e-06,"Pressure accuracy xz component not achieved")
