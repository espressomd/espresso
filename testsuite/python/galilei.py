...
class Galilei(ut.TestCase):
  ...
  def test_kill_particle_motion(self):
	  ...
	  np.testing.assert_array_equal(np.copy(self.system.part[:].v), np.zeros((N_PART, 3))) 

  def test_kill_particle_forces(self):
	  g = GalileiTransform()
	  g.kill_particle_forces()

	  np.testing.assert_array_equal(np.copy(self.system.part[:].f), np.zeros((N_PART, 3))) 
  ...
  def test_cms(self):
	  ...
	  com = np.sum(np.multiply(parts.mass.reshape((N_PART, 1)), parts.pos), axis=0) / total_mass
  ...
  def test_cms_velocity(self):
	  ...
	  g = GalileiTransform()
          total_mass = np.sum(parts.mass)
	  com_v = np.sum(np.multiply(parts.mass.reshape((N_PART, 1)), parts.v), axis=0) / total_mass
  ...
  def test_galilei_transform(self):
	  ...
	  np.testing.assert_allclose(np.copy(g.system_CMS_velocity()), np.zeros((3,)),atol=1e-15) 
