...
class NPTThermostat:
    ...
    def test_pressure_with_dp3m(self):
        """
        Test for NpT with DP3M. NPT only ever accumulates a scalar
        (trace/3) virial, via an older energy-proxy approximation for
        the dipolar real- and k-space contributions (see
        ``npt_add_virial_contribution`` in ``dp3m.hpp`` /
        ``dp3m_heffte.impl.hpp``), predating and independent of the
        full anisotropic pressure-tensor calculation used by
        ``system.analysis.pressure_tensor()``. This test only checks
        that the two scalar estimates of the total virial agree, the
        same way :func:`test_pressure_with_p3m` does for electrostatic
        P3M; it does not exercise the anisotropic tensor (see
        ``dp3m_magnetostatics_pressure.py`` for that).
        """
        np.random.seed(42)
        box_l = 6.0
        num_part = 40
        ...
        # fixed (untuned) P3M parameters: DP3M's tuner is prone to
        # "root must be bracketed" failures for such a small, sparse
        # system, so parameters pre-tuned for this configuration are
        # hardcoded here for reproducibility; NPT rescales r_cut and
        # alpha with the box automatically (see DipolarP3M::on_boxl_change)
