...
class CollisionDetection(ut.TestCase):
    ...
    def test_bonds_slice_assignment_removes_mirror_across_ranks(self):
        ...
        n_nodes = int(np.prod(default_node_grid))
        system.collision_detection.protocol = espressomd.collision_detection.Off()
        system.part.clear()
        try:
            ...
            p1 = system.part.add(pos=(0.5, 0.5, boundary_z + 0.01), id=1)
            if n_nodes > 1:
            ...
            system.integrator.run(1, recalc_forces=True)
            self.assertGreaterEqual(len(p0.bonds) + len(p1.bonds), 1)
        finally:
            system.collision_detection.protocol = espressomd.collision_detection.Off()
            system.part.clear()
            system.cell_system.node_grid = default_node_grid
