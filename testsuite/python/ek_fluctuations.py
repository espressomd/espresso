...
class EKFluctuations(ut.TestCase):
    ...
    def detail_test_fluctuation(self, single_precision: bool):
        decimal_precision: int = 3 if single_precision else 10
        ...
        species = espressomd.electrokinetics.EKSpecies(
            ...
            single_precision=single_precision, tau=self.TAU, thermalized=True)
        ...
        for _ in range(sample_steps):
            ...
            new_hist, _ = np.histogram(density, bins=int(
                (n_max - n_min) / bin_size), range=(n_min, n_max), density=False)
            if hist is None:
                hist = new_hist
            else:
                hist += new_hist
