...
class PropagationNPT:
    ...
    def test_00_propagation(self):
        ...
        system.integrator.set_isotropic_npt(ext_pressure=0.01, piston=1e16, barostat=self.barostat)
        positions = []
        modes_trans = [
            Propagation.NONE,
    ...
    def test_07__virtual(self):
        ...
        system.thermostat.set_npt(kT=0., gamma0=GAMMA0, gammav=1e-6, seed=42)
        system.integrator.set_isotropic_npt(ext_pressure=0.01, piston=1e6, barostat=self.barostat)
    ...
    def test_09_integrator_recovery(self):
        ...
        if self.barostat == "Andersen":
            npt_params = {'ext_pressure': 0.01, 'piston': 0.001, 'barostart': 'Andersen'}
        elif self.barostat == "MTK":
            npt_params = {'ext_pressure': 0.01, 'piston': 4.0, 'barostart': 'MTK'}
        system.box_l = [8] * 3
        ...
        with self.assertRaises(RuntimeError):
            system.integrator.set_isotropic_npt(ext_pressure=-1, piston=100, barostat=self.barostat)
        with self.assertRaises(RuntimeError):
            system.integrator.set_isotropic_npt(ext_pressure=100, piston=-1, barostat=self.barostat)
    ...
    def run_with_p3m(self, container, p3m, method):
        system = self.system
        if self.barostat == "Andersen":
            npt_kwargs = {'ext_pressure': 0.001, 'piston': 0.001, 'barostart': 'Andersen'}
        elif self.barostat == "MTK":
            npt_kwargs = {'ext_pressure': 0.001, 'piston': 4.0, 'barostart': 'MTK'}
