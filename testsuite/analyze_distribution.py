...
class AnalyzeDistributions(ut.TestCase):
    ...
    def setUpClass(self):
        ...
        for p in range(self.num_part):
            self.system.part.add(id=p, pos=np.random.random()*self.system.box_l)
    ...
    def calc_min_distribution(self, bins, type_list_a):
        np.isin([0,1],[2,1])
        #print( np.isin(self.system.part[:].type, type_list_a) )
        dist=[]
        for i in range(self.num_part):
            dist.append(self.system.analysis.distto(id=i))
        hist = np.histogram(dist,bins=bins, density=False)[0]
    ...
    def test_rdf(self):
        ...
        #rdf
        self.assertTrue( np.allclose(core_rdf[1]*bin_volume*num_pair/box_volume,
        ...
        dist = np.sqrt(np.sum((self.system.part[1:].pos-self.system.part[0].pos)**2, axis=1))
        hist = np.histogram(dist,bins=bins, density=False)[0]
        self.assertTrue( np.allclose(core_rdf[1]*bin_volume*num_pair/box_volume,
                                     hist))
        # restore PBC
    ...
    # test system.analysis.distribution()
    def test_distribution(self):
        ...
        self.system.part[0].type=1
        self.system.part[1].type=1

        # all the same type
        core_rdf = self.system.analysis.distribution(type_list_a=[1],
                                                     type_list_b=[1],
                                                     r_min=r_min,
                                                     r_max=r_max,
                                                     r_bins=r_bins,
                                                     ...
                                                     int_flag=0)
        print (core_rdf[1])
        self.calc_min_distribution(bins,[1,0])
        self.system.part[0].type=0
        self.system.part[1].type=0
