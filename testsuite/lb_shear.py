...
class LBShearCommon(object):
    ...
    def prepare(self):
        ...
        wall1 = espressomd.lbboundaries.LBBoundary(
            shape=wall_shape1, velocity=[0, 0, +1])
        wall2 = espressomd.lbboundaries.LBBoundary(
            shape=wall_shape2, velocity=[0, 0, -1])

        self.system.lbboundaries.add(wall1)
        self.system.lbboundaries.add(wall2)

        quater_indices = [int((self.system.box_l[0] / AGRID) / 4),
                          int((self.system.box_l[1] / AGRID) / 4),
                          int((self.system.box_l[2] / AGRID) / 4)]
        diff = float("inf")
        old_val = self.lbf[quater_indices].velocity[2]
        while diff > 0.01:
            self.system.integrator.run(100)
            new_val = self.lbf[quater_indices].velocity[2]
    ...
    def test_profile(self):
        ...
        H = self.system.box_l[0] - 2.0 * AGRID
        v_expected = (1. - 2. * (x - AGRID) / H)
