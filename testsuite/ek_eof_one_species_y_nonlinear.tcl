...
for {set i 0} {$i < [expr $box_x/$agrid]} {incr i} {
  ...
  if {[expr $i*$agrid] >= $padding && [expr $i*$agrid] < [expr $box_x - $padding] } {
    ...
    set measured_pressure_xx [lindex [lbnode $i [expr int($box_y/(2*$agrid))] [expr int($box_z/(2*$agrid))] print pi_neq] 0]
    set calculated_pressure_xx [hydrostatic_pressure $position $xi $bjerrum_length 0]
    set measured_pressure_yy [lindex [lbnode $i [expr int($box_y/(2*$agrid))] [expr int($box_z/(2*$agrid))] print pi_neq] 2]
    set calculated_pressure_yy [hydrostatic_pressure $position $xi $bjerrum_length 2]
    set measured_pressure_zz [lindex [lbnode $i [expr int($box_y/(2*$agrid))] [expr int($box_z/(2*$agrid))] print pi_neq] 5]
    ...
    set measured_pressure_xy [lindex [lbnode $i [expr int($box_y/(2*$agrid))] [expr int($box_z/(2*$agrid))] print pi_neq] 1]
    set calculated_pressure_xy [pressure_tensor_offdiagonal $position $xi $bjerrum_length $force]
    ...
    set measured_pressure_yz [lindex [lbnode $i [expr int($box_y/(2*$agrid))] [expr int($box_z/(2*$agrid))] print pi_neq] 4]
    ...
    set measured_pressure_xz [lindex [lbnode $i [expr int($box_y/(2*$agrid))] [expr int($box_z/(2*$agrid))] print pi_neq] 3]
...
if { $total_density_difference > 8.0e-06 } {
  error_exit "Density accuracy not achieved"
}
if { $total_velocity_difference > 7.0e-06 } {
  error_exit "Velocity accuracy not achieved"
}
if { $total_pressure_difference_xx > 6.0e-05 } {
  error_exit "Difference xx component too large"
}
if { $total_pressure_difference_yy > 5.0e-05 } {
...
if { $total_pressure_difference_xy > 1.0e-05 } {
  error_exit "Pressure accuracy xy component not achieved"
}
if { $total_pressure_difference_yz > 5.0e-11 } {
  error_exit "Pressure accuracy yz component not achieved"
}
if { $total_pressure_difference_xz > 5.0e-11 } {
