...
# function to read in configurations from .chk file
#
# The simulation "analysis_system.data.chk" contains a set of configurations
# of 20 polymer chains with 30 monomers each.

proc read_data { fname } {
  global observables listables

  puts -nonewline "Reading data from '$fname'... "; flush stdout

# open (old) checkpoint file
  if { [file exists "$fname.chk"] } { 
    set chk [open "$fname.chk" "r"] 
  } elseif { [file exists "$fname"] } {
    set chk [open "$fname" "r"] 
  } else { 
    puts "ERROR: Could not find checkpoint-list $fname!\nAborting..."; exit 
  }
...
# doing analysis
  ...
  set re "analyze re"; set re_av "analyze <re>"
  lappend get_listables $re $re_av
  
  set rg "analyze rg"; set rg_av "analyze <rg>"
