...
class CollisionDetection(ut.TestCase):
    ...
    def test_bind_at_point_of_collision_random(self):
        ...
        self.s.integrator.set_steepest_descent(f_max=0, gamma=1, max_displacement=0.001)
        ...
        # Tidy
        self.s.non_bonded_inter[0,0].lennard_jones.set_params(epsilon=0,sigma=0,cutoff=0)
    ...
    def test_glue_to_surface_random(self):
        ...
        # Add randomly placed particles
        self.s.part.add(pos=np.random.random((100,3)),type=100*[self.part_type_to_attach_vs_to])
        self.s.part.add(pos=np.random.random((100,3)),type=100*[self.part_type_to_be_glued])
        self.s.part.add(pos=np.random.random((100,3)),type=100*[self.other_type])
        ...
        to_attach_vs_to=self.s.part.select(type=self.part_type_to_attach_vs_to)
        ...
        for p in non_virtual:
            ...
            for bond in p.bonds:
                ...
                if p.type==self.part_type_after_glueing:
                    ...
                    if bond[0]==self.H2:
                        self.assertEqual(self.s.part[bond[1]].type,self.part_type_vs)
                    else:
                        self.assertEqual(self.s.part[bond[1]].type,self.part_type_to_attach_vs_to)
                elif p.type==self.part_type_to_attach_vs_to:
                    self.assertEqual(bond[0], self.H)
                    self.assertEqual(self.s.part[bond[1]].type,self.part_type_after_glueing)
                else:
