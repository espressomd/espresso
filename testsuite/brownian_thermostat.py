...
class BrownianThermostat(ut.TestCase):
    ...
    def test_brownian_per_particle(self):
        """Test for Brownian particle. Covers all combinations of
    ...
    def test_diffusion(self):
        ...
        if espressomd.has_features("ROTATION"):
            if espressomd.has_features("PARTICLE_ANISOTROPY"):
                ...
                s.thermostat.set_brownian(kT=kT,gamma=gamma,gamma_rotation=gamma_rot_a)
            else:
                # Rotation without particle anisotropy
                s.thermostat.set_brownian(kT=kT,gamma=gamma,gamma_rotation=gamma_rot_i)
        else:
            # No rotation
            s.thermostat.set_brownian(kT=kT,gamma=gamma)
        ...
        #    TODO: to return this after an implementation of the fine inertial BD
...
#            s.part[0].mass=3
#        if espressomd.has_features("PARTICLE_ANISOTROPY"):
#            s.thermostat.set_brownian(kT=0,gamma=gamma_t_a)
#        else:
#            s.thermostat.set_brownian(kT=0,gamma=gamma_t_i)
#
#        s.time=0
#        for i in range(100):
#            s.integrator.run(10)
...
#        if espressomd.has_features("PARTICLE_ANISOTROPY"):
#            s.thermostat.set_brownian(kT=0,gamma=gamma_t_a,gamma_rotation=gamma_r_a)
#        else:
#            s.thermostat.set_brownian(kT=0,gamma=gamma_t_i,gamma_rotation=gamma_r_i)
#
#        s.time=0
#        for i in range(100):
#            s.integrator.run(10)
