...
class test_ljCos2(unittest.TestCase):
	...
	def read(cls):
		...
		cls.system.box_l 		= map(float,[cls.data[5][1],cls.data[5][2],cls.data[5][3]])
	...
	def test_energy(self):
		...
		nonbondedEnergy = test_ljCos2.curEnergy['non_bonded',0,0] + test_ljCos2.curEnergy['non_bonded',0,1] + test_ljCos2.curEnergy['non_bonded',1,1]
		self.assertLessEqual(abs(totalEnergy - nonbondedEnergy), test_ljCos2.epsilon)
		

	def test_pressure(self):
		totalPressure 		= test_ljCos2.curPressure['total']
		nonbondedPressure	= test_ljCos2.curPressure['non_bonded',0,0] + test_ljCos2.curPressure['non_bonded',0,1] + test_ljCos2.curPressure['non_bonded',1,1]
		self.assertLessEqual(abs(totalPressure - nonbondedPressure), test_ljCos2.epsilon)
		

	def test_relative_energy(self):
		relativeEnergyDiff = abs((test_ljCos2.curEnergy['total'] - test_ljCos2.prevTotalEnergy)/test_ljCos2.prevTotalEnergy)
		self.assertLessEqual(relativeEnergyDiff, test_ljCos2.epsilon)


	def test_relative_pressure(self):
		relativePressureDiff = abs((test_ljCos2.curPressure['total'] - test_ljCos2.prevTotalPressure)/test_ljCos2.prevTotalPressure)
	...
	def test_max_force(self):
		...
		for i in range(0,test_ljCos2.numPart):
			dFx = abs(test_ljCos2.prevParticleData[i][5] - test_ljCos2.system.part[i].f[0])
			dFy = abs(test_ljCos2.prevParticleData[i][6] - test_ljCos2.system.part[i].f[1])
			dFz = abs(test_ljCos2.prevParticleData[i][7] - test_ljCos2.system.part[i].f[2])
			if(dFx>maxdFx):
				maxdFx = dFx
			if(dFy>maxdFy):
				maxdFy = dFy
