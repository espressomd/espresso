...
@ut.skipIf(not espressomd.has_features(["MASS","ROTATIONAL_INERTIA","LANGEVIN_PER_PARTICLE","BROWNIAN_DYNAMICS"]),
...
class ThermoTest(ut.TestCase):
    ...
    def run_test_case(self, test_case):
        ...
        if "PARTICLE_ANISOTROPY" in espressomd.features():
            self.es.thermostat.set_brownian(kT=kT, gamma=[gamma_global[0],gamma_global[1],gamma_global[2]])
        else:
        ...
        for i in range(n):
            for k in range(2):
                ...
                self.es.part.add(rotation=(1,1,1), id = ind, mass = mass, rinertia = J, pos = part_pos, v = part_v)
