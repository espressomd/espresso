...
for {set i 0} {$i < [expr $box_z/$agrid]} {incr i} {
  ...
  if {[expr $i*$agrid] >= $padding && [expr $i*$agrid] < [expr $box_z - $padding] } {
    ...
    set measured_stress_xy [lindex [lbnode [expr int($box_x/(2*$agrid))] [expr int($box_y/(2*$agrid))] $i print pi_neq] 1]
    ...
    set measured_stress_yz [lindex [lbnode [expr int($box_x/(2*$agrid))] [expr int($box_y/(2*$agrid))] $i print pi_neq] 4]
    ...
    set measured_stress_xz [lindex [lbnode [expr int($box_x/(2*$agrid))] [expr int($box_y/(2*$agrid))] $i print pi_neq] 3]
...
if { $total_density_difference > 5.0e-05 } {
  error_exit "Density accuracy not achieved"
}
if { $total_velocity_difference > 5.0e-06 } {
  error_exit "Velocity accuracy not achieved"
}
if { $total_stress_difference_xx > 5.0e-05 } {
...
if { $total_stress_difference_zz > 5.0e-07 } {
...
if { $total_stress_difference_xz > 7.5e-06 } {
