...
MDINLINE void add_subt_lj_fene_pair_force(Particle *p1, Particle *p2, int type_num)
{
  ...
  ONEPART_TRACE(if(p1->r.identity==check_id) fprintf(stderr,"%d: OPT: SUBT_LJ_FENE f = (%.3e,%.3e,%.3e) with part id=%d at dist %f fac %.3e\n",this_node,p1->f[0],p1->f[1],p1->f[2],p2->r.identity,sqrt(dist2),fac));
  ONEPART_TRACE(if(p2->r.identity==check_id) fprintf(stderr,"%d: OPT: SUBT_LJ_FENE f = (%.3e,%.3e,%.3e) with part id=%d at dist %f fac %.3e\n",this_node,p2->f[0],p2->f[1],p2->f[2],p1->r.identity,sqrt(dist2),fac));
}
